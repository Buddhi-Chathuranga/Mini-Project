-----------------------------------------------------------------------------
-- File    : UpgradeToTextApps9
--
-- Purpose : This script will convert App9 terms to App10 text and move App9 Text to App10
-----------------------------------------------------------------------------
...
BEGIN
   ...
   commit;  
   execute immediate 'create table temp_term_trans_def_tab as select term_id, usage_id, language_code, translated_basic_text,rowstate from term_translated_definition_tab@terms';
   commit;
...
DECLARE 
   ...
   CURSOR get_term_data___ (component_ VARCHAR2, input_main_type_ VARCHAR2) IS
      SELECT ctx.context_id,
             ...
             atr.name,
             atr.prog_text,
             t9_atr.term_usage_version_id,
             t9_atr.display_name_type,
             t9_atr.exclude_from_documentation,
             t9_atr.no_term_connection
       FROM  language_context_tab ctx,
             language_attribute_tab atr,             
             LANGUAGE_CONTEXT_TAB@TERMS t9_ctx,
             LANGUAGE_ATTRIBUTE_TAB@TERMS t9_atr
       WHERE ctx.context_id = atr.context_id
       and   t9_ctx.context_id = t9_atr.context_id 
       and   ctx.module = t9_ctx.module
       and   ctx.main_type = t9_ctx.main_type
       and   ctx.sub_type = t9_ctx.sub_type
       and   ctx.path = t9_ctx.path
       and   atr.name = t9_atr.name        
       and   NVL(t9_atr.connection_complete,0) = 1       
   ...
   IS
      ...
      CURSOR get_display_name IS
         ...
         WHERE  t.term_id = d.term_id
           ...
           AND  t.rowstate != 'Obsolete';

      CURSOR get_translation IS
         SELECT translated_text,DECODE(rowstate,'Translated','O','Revised','C','O') status
      ...
      CURSOR get_usage_translation IS
         SELECT translated_basic_text,DECODE(rowstate,'Translated','O','Revised','C','O') status
