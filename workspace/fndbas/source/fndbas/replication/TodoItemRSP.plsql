...
PROCEDURE Get_Info(     attr_           IN  VARCHAR2,
   ...
   stmt_          VARCHAR2(3000);  
BEGIN
   lu_ := Client_SYS.Get_Item_Value('BUSINESS_OBJECT', attr_);
   IF (lu_ IS NULL) THEN
      site_field_ := '*';
      key_fields_ := NULL;
   ELSE
...
IS
   ...
   init_       NUMBER := 1;
BEGIN
   -- Get url and lu from the new_attr_
   url_     := Client_SYS.Get_Item_Value('URL', attr_);
   lu_      := Client_SYS.Get_Item_Value('BUSINESS_OBJECT', attr_);
   -- Get base view
...
IS
   ...
   sync_sites_ VARCHAR2(2000);
BEGIN
   Get_Info(non_key_attr_,site_field_,key_fields_);
