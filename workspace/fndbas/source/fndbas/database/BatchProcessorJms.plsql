...
IS
   message_seq_no_ NUMBER       := batch_processor_queue_seq.NEXTVAL;
...
BEGIN
   ...
   ELSE
      is_topic_ := FALSE;
      node_id_  := CASE chosen_node_id_ IS NOT NULL WHEN TRUE THEN chosen_node_id_ ELSE Connect_Node_API.Choose_Node_(invoke_) END;
   END IF;

   INSERT INTO batch_processor_queue_tab
     (message_seq_no,
      cluster_name,
      node_id,
      timestamp,
   ...
   IF is_topic_ THEN
      Connect_Node_API.Notify_All_Nodes_(message_seq_no_);
   ELSIF invoke_ THEN
      NULL; -- For Invoke calls notification is delayed to COMMIT in Plsqlap_Server_API.Invoke_Aq___
   ELSIF node_id_ <> '?' THEN
      Connect_Node_API.Notify_Node_(node_id_, message_seq_no_);
   END IF;
