...
ROW_SYNC_PROPERTY_NAME CONSTANT VARCHAR2(30) := 'CONNECT_CONFIG_ROW_SYNC'; -- 'ON' | 'OFF'  (default: 'ON')
...
DB_MESSAGE_QUEUES     CONSTANT VARCHAR2(20) := Config_Instance_Group_API.DB_MESSAGE_QUEUES;
...
DB_TASK_TEMPLATES     CONSTANT VARCHAR2(20) := Config_Instance_Group_API.DB_TASK_TEMPLATES;
...
PROCEDURE Add_Params_To_Tmp_Table___ (
   params_ IN Connect_Runtime_Params_Type) IS
BEGIN
   INSERT INTO config_instance_param_tmp_tab(group_name, instance_name, parameter_name, parameter_value)
...
BEGIN
   ...
   Delete_Table___
    (table_name_  => 'config_instance_tab',
     key_columns_ => 'group_name, instance_name',
     key1_        => group_name_,
     key2_        => instance_name_);
...
BEGIN
   ...
   DELETE config_instance_tmp_tab;
   INSERT INTO config_instance_tmp_tab(group_name, instance_name, instance_type, description)
   VALUES (group_name_, instance_name_, instance_type_, description_);

   Sync_Table___ (
      src_table_     => 'config_instance_tmp_tab',
      dest_table_    => 'config_instance_tab',
...
BEGIN
   ...
   DELETE config_instance_param_tmp_tab;
   INSERT INTO config_instance_param_tmp_tab(group_name, instance_name, parameter_name, parameter_value)
   SELECT group_name, instance_name, parameter_name, parameter_value
     FROM TABLE(params_);

   Sync_Table___ (
      src_table_     => 'config_instance_param_tmp_tab',
      dest_table_    => 'config_instance_param_tab',
...
PROCEDURE Sync_Connect_Instances___ IS
   ...
   queues_       Connect_Queue_API.Connect_Queues;
   ...
   tasks_        Connect_Print_Agent_Task_API.Connect_Print_Agent_Tasks;

   params_  Connect_Runtime_Params_Type;
BEGIN
   Log___('Sync_Connect_Instances___:');
   --
   ...
   FROM config_runtime_instance; -- UNION of all client entities
   --
   ...
   -- ConnectQueue
   --
   SELECT * BULK COLLECT INTO queues_ FROM connect_queue_tab;
   Connect_Queue_API.To_Runtime_Params_(params_, queues_);
   Add_Params_To_Tmp_Table___(params_);
   ...
   --
   -- ConnectPrintAgentTask
   --
   SELECT * BULK COLLECT INTO tasks_ FROM connect_print_agent_task_tab;
   Connect_Print_Agent_Task_API.To_Runtime_Params_(params_, tasks_);
   Add_Params_To_Tmp_Table___(params_);
...
--
-- Disable synchronization of runtime tables on row level in the current session.
--
-- A procedure, like Import, that calls this procedure must:
--   (1) re-enable synchronization on row level
--   (2) call Sync_All_ procedure when all rows have been modified
--
-- BEGIN
--     Connect_Runtime_API.Disable_Row_Sync_;
--    -- import data
--     Connect_Runtime_API.Enable_Row_Sync_;
--     Connect_Runtime_API.Sync_All_;
-- EXCEPTION
--    WHEN OTHERS THEN
--       Connect_Runtime_API.Enable_Row_Sync_;
--       RAISE;
...
PROCEDURE Sync_Queue_ (
   queue_ IN connect_queue_tab%ROWTYPE)
IS
   params_        Connect_Runtime_Params_Type;
...
BEGIN
   IF Sync_Row_Enabled___ THEN
      ...
      Connect_Queue_API.To_Runtime_Params_(params_, Connect_Queue_API.Connect_Queues(queue_));
...
PROCEDURE Sync_Print_Agent_Task_ (
   task_ IN connect_print_agent_task_tab%ROWTYPE)
IS
   params_ Connect_Runtime_Params_Type;
BEGIN
   IF Sync_Row_Enabled___ THEN
      Sync_Instance___(DB_TASK_TEMPLATES, task_.instance_name, 'PrintAgent', task_.description);
      Connect_Print_Agent_Task_API.To_Runtime_Params_(params_, Connect_Print_Agent_Task_API.Connect_Print_Agent_Tasks(task_));
