-----------------------------------------------------------------------------
--
--  Logical unit: ReportMessageBody
--  Component:    FNDBAS
--
...
-------------------- PUBLIC DECLARATIONS ------------------------------------
PROCEDURE New (
   newrec_ IN OUT NOCOPY report_message_body_tab%ROWTYPE )
IS
BEGIN
   New___(newrec_);
END New;
...
IS 
   ...
   CURSOR check_in_bound IS
      ...
      FROM report_message_body t, 
           report_application_message a
   ...
   CURSOR get_details IS 
      ...
      FROM report_application_message a
