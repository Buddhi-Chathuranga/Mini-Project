...
--  Date        Sign    History
--  ----------  ------  -----------------------------------------------------
--  2014-05-01  madrse  Added method Run_Application_Server_Task
--  2014-09-15  madrse  Moved public methods to AppMessageProcessing.plsql
--  2014-09-30  madrse  Added validation of state transition
-----------------------------------------------------------------------------
...
PROCEDURE New (
   newrec_ IN OUT NOCOPY fndcn_application_message_tab%ROWTYPE )
IS
BEGIN
   New___(newrec_);
END New;
...
PROCEDURE Validate_State_Transition___ (
   ...
   newrec_     IN OUT fndcn_application_message_tab%ROWTYPE)
IS
...
BEGIN
   ...
   IF new_ = Message_State_Types_API.DB_RELEASED THEN
      newrec_.error_text := NULL;
   END IF;

   IF old_ = Message_State_Types_API.DB_PROCESSING THEN
      newrec_.finished_timestamp := systimestamp;
   END IF;

   IF newrec_.tag = 'BATCH' AND newrec_.message_function IN ('PrintAgent', 'Connectivity') THEN
      IF old_ IS NULL AND new_ IN (Message_State_Types_API.DB_SUSPENDED) THEN
         RETURN;
      ELSIF old_ = new_ THEN
         RETURN;
      END IF;
      Error_SYS.Appl_General(lu_name_, 'TEMPLATE_STATETRANSITION: Invalid state transition from [:P1] to [:P2] on application message template [:P3]', old_, new_, newrec_.application_message_id);
   END IF;

   IF old_ IS NULL AND new_ IN (Message_State_Types_API.DB_RELEASED, Message_State_Types_API.DB_SUSPENDED, Message_State_Types_API.DB_FAILED, Message_State_Types_API.DB_WAITING, Message_State_Types_API.DB_FINISHED) THEN
      RETURN;
   ELSIF old_ = new_ THEN
      RETURN;
   ELSIF old_ = Message_State_Types_API.DB_RELEASED   AND new_ IN (Message_State_Types_API.DB_SUSPENDED, Message_State_Types_API.DB_PROCESSING, Message_State_Types_API.DB_FAILED) THEN
      RETURN;
   ELSIF old_ = Message_State_Types_API.DB_PROCESSING AND new_ IN (Message_State_Types_API.DB_FINISHED, Message_State_Types_API.DB_FAILED, Message_State_Types_API.DB_WAITING, Message_State_Types_API.DB_SUSPENDED) THEN
      RETURN;
   ELSIF old_ = Message_State_Types_API.DB_FAILED     AND new_ IN (Message_State_Types_API.DB_RELEASED, Message_State_Types_API.DB_CANCELLED) THEN
      RETURN;
   ELSIF old_ = Message_State_Types_API.DB_WAITING    AND new_ IN (Message_State_Types_API.DB_RELEASED, Message_State_Types_API.DB_SUSPENDED, Message_State_Types_API.DB_CANCELLED) THEN
      RETURN;
   ELSIF old_ = Message_State_Types_API.DB_CANCELLED  AND new_ IN (Message_State_Types_API.DB_RELEASED) THEN
      RETURN;
   ELSIF old_ = Message_State_Types_API.DB_SUSPENDED  AND new_ IN (Message_State_Types_API.DB_RELEASED, Message_State_Types_API.DB_CANCELLED) THEN
      RETURN;
   END IF;
   Error_SYS.Appl_General(lu_name_, 'STATETRANSITION: Invalid state transition from [:P1] to [:P2] on application message [:P3]', old_, new_, newrec_.application_message_id);
...
PROCEDURE Update___ (
   objid_      IN     VARCHAR2,
   oldrec_     IN     fndcn_application_message_tab%ROWTYPE,
   newrec_     IN OUT fndcn_application_message_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2,
...
BEGIN
   ...
   --
   IF oldrec_.state = Message_State_Types_API.DB_PROCESSING AND newrec_.state = Message_State_Types_API.DB_PROCESSING AND oldrec_.queue <> newrec_.queue THEN
      newrec_.state := Message_State_Types_API.DB_SUSPENDED;
   --
   -- Suspend Waiting message moved from an InOrder queue to another queue. Send JMS message to wake up the InOrder queue.
   --
   ELSIF oldrec_.state = Message_State_Types_API.DB_WAITING AND newrec_.state = Message_State_Types_API.DB_WAITING AND oldrec_.queue <> newrec_.queue THEN
      ...
      IF lower(execution_mode_) = 'inorder' THEN
         newrec_.state := Message_State_Types_API.DB_SUSPENDED;
         App_Message_Processing_API.Release_Suspended_Messages_(oldrec_.queue);
      END IF;
   END IF;
...
PROCEDURE Raise_Record_Modified___ (
   rec_ fndcn_application_message_tab%ROWTYPE )
IS
BEGIN
   Error_SYS.Record_Modified(Application_Message_API.lu_name_, p1_ => rec_.application_message_id);
   super(rec_);
...
BEGIN
   ...
   Address_Label_API.New__(info_, objid_, objversion_, address_line_attr_, action_);
   Message_Body_API.New__(info_, objid_, objversion_, message_body_attr_, action_);
...
PROCEDURE Write_B_Message_Value__ (
   ...
   lob_loc_    IN     BLOB ) IS
BEGIN
   Message_Body_API.Write_Message_Value__(objversion_, rowid_, lob_loc_);
