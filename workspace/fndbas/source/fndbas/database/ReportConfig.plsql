...
BEGIN
   ...
   WHILE Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_) LOOP
      IF instr(sep_ || mandatory_attrs_, sep_ || name_ || sep_) > 0 AND value_ IS NULL THEN
         Error_SYS.Appl_General(Report_Config_API.lu_name_, 'MANDATORY: Missing value for mandatory attribute [:P1] in record [:P2]', name_, lu_name_);
      END IF;
   END LOOP;
...
BEGIN
   Ins_Util_API.Spool_Ins_File_Header (
      ...
      purpose_           => 'Install report application server tasks and task templates');

   Ins_Util_API.Generate_Ins_File (
      table_name_        => 'report_application_server_task',
      pk_column_list_    => Ins_Util_API.List('tag', 'subject'),
      order_by_          => 'subject',
      where_             => condition_,
      lu_package_        => 'Report_Application_Message_API',
      ...
      is_text_blob_func_ => 'Report_Config_API.Is_Text_Blob',
      wide_format_       => FALSE,
      debug_             => debug_);
