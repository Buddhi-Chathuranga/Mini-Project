...
--
-- Try to lock Batch Processor message. Called from MDB and Cleanup_Topics_.
--
...
BEGIN
   SELECT NULL
     INTO tmp_
     FROM batch_processor_queue_tab
...
BEGIN
   ...
   DELETE batch_processor_queue_tab
    WHERE message_seq_no = message_seq_no_;
   Trace___('Deleting Batch Processor messages sent: ' || message_seq_no_);
   DELETE batch_processor_queue_sent_tab
    WHERE message_seq_no = message_seq_no_;
END Remove_Message_;
...
BEGIN
   ...
   ELSE
      INSERT INTO batch_processor_queue_sent_tab(node_id, message_seq_no)
...
BEGIN
   ...
   DELETE batch_processor_queue_sent_tab S
    WHERE message_seq_no IN (SELECT S.message_seq_no FROM batch_processor_queue_sent_tab S
                             MINUS
                             SELECT Q.message_seq_no FROM batch_processor_queue_tab Q);
