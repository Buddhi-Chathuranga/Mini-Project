...
--  Date        Sign    History
--  ----------  ------  -------------------------------------------------------
--  2021-04-07  didslk  OR21R2-86: Created for IFS Cloud 21R2
-----------------------------------------------------------------------------
-- Note: is_invoke_ is never used.
...
BEGIN
   RETURN 'IFS_REPORTING' || CASE is_invoke_ WHEN TRUE THEN '_INVOKE' ELSE '' END || '$' || node_id_;
...
BEGIN
   ...
   INSERT INTO report_node_tab (node_id, cluster_name, lock_handle, invoke_lock_handle, created_timestamp, timestamp, load_factor, rowversion)
...
BEGIN
   ...
   DELETE report_node_tab WHERE node_id = node_id_;
...
--
-- Called on reporting node startup
--
...
--
-- Called on reporting node shutdown
--
...
IS
   ...
   load_factor_    NUMBER;
   load_sent_mul_  NUMBER;
BEGIN
   SELECT n.node_id, 
         (n.load_factor * (1 + NVL(s.sent_count, 0) * 0.1)) load_sent_mult, -- Assumption, next queued item will increase the load by 10%
          n.load_factor 
     INTO node_id_, load_sent_mul_, load_factor_
     FROM report_node n, (SELECT   node_id, count(*) sent_count
                          FROM     report_batch_pr_queue_sent_tab t
                          GROUP BY node_id) s
    WHERE cluster_name = 'REP'
      AND n.node_id = s.node_id (+)
      AND n.age_in_seconds <= max_age_
    ORDER BY 2, 3, Dbms_Random.Value
    FETCH FIRST ROW ONLY;

   Trace___('Chose node [' || node_id_ || '] with load_factor * (total_sent + 1) [' || load_sent_mul_ || '] with load_factor [' || load_factor_ || ']');
   RETURN node_id_;
EXCEPTION
   WHEN no_data_found THEN
      Trace___('WARNING! There is no running IFS Reporting container');
...
@UncheckedAccess
FUNCTION Is_Node_Deletable (
   node_id_    IN VARCHAR2) RETURN VARCHAR2
IS
   node_rec_ Report_Node_API.Public_Rec;
   created_time_ DATE;
   current_time_ DATE := SYSDATE;
   time_difference_  NUMBER;
BEGIN
   node_rec_ := Report_Node_API.Get(node_id_);
   created_time_ := node_rec_.created_timestamp;
   time_difference_ := (round(to_number(current_time_-(created_time_))*1440)); -- time difference in minutes
   IF (time_difference_ > 60) THEN
      RETURN 'TRUE';
   ELSE
      RETURN 'FALSE';
   END IF;
END Is_Node_Deletable;