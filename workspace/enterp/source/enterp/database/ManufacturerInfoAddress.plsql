-----------------------------------------------------------------------------
--
--  Logical unit: ManufacturerInfoAddress
--  Component:    ENTERP
--
...
--  090922  Nsillk  Issue Id EAFH-124 Removed unnecessary COMMIT statements
...
--  131111  Isuklk  PBFI-2169 Refactoring and Split ManufacturerInfoAddress.entity
...
--  160415  reanpl  STRLOC-117, Added handling of new attributes address3, address4, address5, address6, removed New_Address, Modify_Address
...
PROCEDURE Get_Manufacturer_Party___ (
   newrec_ IN OUT manufacturer_info_address_tab%ROWTYPE )
IS
BEGIN
   IF (newrec_.party IS NULL) THEN
      newrec_.party := Manufacturer_Info_API.Get_Party(newrec_.manufacturer_id);      
   END IF;
END Get_Manufacturer_Party___;


PROCEDURE Check_Ean_Location___ (
   rec_ IN manufacturer_info_address_tab%ROWTYPE )
IS
   objid_        VARCHAR2(100);
...
BEGIN
   Get_Id_Version_By_Keys___(objid_, objversion_, rec_.manufacturer_id, rec_.address_id);
   IF (rec_.ean_location IS NOT NULL) THEN
      FOR a IN Check_Exist LOOP
         Error_SYS.Record_Exist(lu_name_, 'EANEXIST: This Manufacturer''s Own Address ID already exists on another address for this manufacturer, or on another manufacturer.');
      END LOOP;
   END IF;
...
PROCEDURE Check_Common___ (
   oldrec_ IN     manufacturer_info_address_tab%ROWTYPE,
   newrec_ IN OUT manufacturer_info_address_tab%ROWTYPE,
...
IS
   ...
   CURSOR get_address_types(manufacturer_id_ IN VARCHAR2, address_id_ IN VARCHAR2) IS
      SELECT t.rowid, t.manufacturer_id, Address_Type_Code_API.Decode(t.address_type_code) AS address_type_code, t.def_address
      FROM   manuf_info_addr_type_tab t
      WHERE  t.manufacturer_id = manufacturer_id_
...
BEGIN
   ...
   IF (NVL(newrec_.valid_from, Database_Sys.Get_First_Calendar_Date()) != NVL(oldrec_.valid_from, Database_Sys.Get_First_Calendar_Date())) OR 
      ...
      FOR c1 IN get_address_types(newrec_.manufacturer_id, newrec_.address_id) LOOP
         Manuf_Info_Addr_Type_API.Check_Def_Address_Exist(validation_result_, 
                                                          validation_flag_, 
                                                          c1.manufacturer_id, 
                                                          c1.def_address, 
...
IS
BEGIN
   Get_Manufacturer_Party___(newrec_);
   ...
   IF (NVL(newrec_.valid_from, Database_Sys.Get_First_Calendar_Date()) != NVL(oldrec_.valid_from, Database_Sys.Get_First_Calendar_Date())) OR 
      ...
      FOR c1 IN get_address_types(newrec_.manufacturer_id, newrec_.address_id) LOOP
         Manuf_Info_Addr_Type_API.Check_Def_Address_Exist(validation_result_, 
                                                          ...
                                                          newrec_.valid_to);
         IF (c1.def_address = 'TRUE' AND (validation_result_ = 'FALSE')) THEN
            Manuf_Info_Addr_Type_API.Check_Def_Addr_Temp(c1.manufacturer_id, 
                                                         c1.address_type_code, 
                                                         c1.def_address,
                                                         c1.rowid, 
                                                         newrec_.valid_from, 
...
PROCEDURE Check_Delete___ (
   remrec_ IN manufacturer_info_address_tab%ROWTYPE )
IS
   ...
   CURSOR get_address_types (manufacturer_id_ VARCHAR2, address_id_ VARCHAR2) IS
      SELECT *
      FROM   manuf_info_addr_type
      WHERE  manufacturer_id = manufacturer_id_
      AND    address_id = address_id_;
   CURSOR get_def_address_types_count (manufacturer_id_ VARCHAR2, address_id_ VARCHAR2) IS
      SELECT COUNT(*)
      FROM   manuf_info_addr_type
      WHERE  manufacturer_id = manufacturer_id_
      AND    address_id = address_id_
...
BEGIN
   OPEN get_def_address_types_count(remrec_.manufacturer_id, remrec_.address_id);
   FETCH get_def_address_types_count INTO addr_type_count_;
   CLOSE get_def_address_types_count;
   FOR rec_ IN get_address_types(remrec_.manufacturer_id, remrec_.address_id) LOOP
      Manuf_Info_Addr_Type_API.Check_Def_Address_Exist(validation_result_, validation_flag_, rec_.manufacturer_id, rec_.def_address, rec_.address_type_code_db, rec_.objid, remrec_.valid_from, remrec_.valid_to);
   ...
   IF (addr_type_code_list_ IS NOT NULL) THEN
      Client_SYS.Add_Warning(lu_name_, 'REMOVEADDRWITHDEFADDRTYPES: This is the default :P1 Address Type(s) for Manufacturer :P2. If removed, there will be no default address for this Address Type(s).', addr_type_code_list_, remrec_.manufacturer_id);
   END IF;
...
PROCEDURE Remove__ (
   ...
   action_     IN  VARCHAR2 )
IS
   remrec_ manufacturer_info_address_tab%ROWTYPE;
BEGIN
   IF (action_ = 'DO') THEN
      remrec_ := Get_Object_By_Id___(objid_);
      Cascade_Delete_Comm_Method__(remrec_.manufacturer_id, remrec_.address_id);
   END IF;   
...
IS
   country_       manufacturer_info_address.country%TYPE;
   address_pres_  Address_Presentation_API.Public_Rec_Address;
   line_          NUMBER;
   row1_          VARCHAR2(100);
   row2_          VARCHAR2(100);
   ...
   row5_          VARCHAR2(100);
BEGIN
   country_ := Get_Country(manufacturer_id_, address_id_);
   address_pres_ := Address_Presentation_API.Get_Address_Record(Iso_Country_API.Encode(country_));
   line_ := line_no_;
   ...
   IF (address_pres_.address1_row = line_) THEN   
      IF (address_pres_.address1_order = 1) THEN
         row1_ := Get_Address1(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address1_order = 2) THEN
         row2_ := Get_Address1(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address1_order = 3) THEN
         row3_ := Get_Address1(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address1_order = 4) THEN
         row4_ := Get_Address1(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address1_order = 5) THEN
         row5_ := Get_Address1(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Address1(manufacturer_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.address2_row = line_) THEN   
      IF (address_pres_.address2_order = 1) THEN
         row1_ := Get_Address2(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address2_order = 2) THEN
         row2_ := Get_Address2(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address2_order = 3) THEN
         row3_ := Get_Address2(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address2_order = 4) THEN
         row4_ := Get_Address2(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address2_order = 5) THEN
         row5_ := Get_Address2(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Address2(manufacturer_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.address3_row = line_) THEN
      IF (address_pres_.address3_order = 1) THEN
         row1_ := Get_Address3(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address3_order = 2) THEN
         row2_ := Get_Address3(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address3_order = 3) THEN
         row3_ := Get_Address3(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address3_order = 4) THEN
         row4_ := Get_Address3(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address3_order = 5) THEN
         row5_ := Get_Address3(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Address3(manufacturer_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.address4_row = line_) THEN
      IF (address_pres_.address4_order = 1) THEN
         row1_ := Get_Address4(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address4_order = 2) THEN
         row2_ := Get_Address4(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address4_order = 3) THEN
         row3_ := Get_Address4(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address4_order = 4) THEN
         row4_ := Get_Address4(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address4_order = 5) THEN
         row5_ := Get_Address4(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Address4(manufacturer_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.address5_row = line_) THEN
      IF (address_pres_.address5_order = 1) THEN
         row1_ := Get_Address5(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address5_order = 2) THEN
         row2_ := Get_Address5(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address5_order = 3) THEN
         row3_ := Get_Address5(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address5_order = 4) THEN
         row4_ := Get_Address5(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address5_order = 5) THEN
         row5_ := Get_Address5(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Address5(manufacturer_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.address6_row = line_) THEN
      IF (address_pres_.address6_order = 1) THEN
         row1_ := Get_Address6(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address6_order = 2) THEN
         row2_ := Get_Address6(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address6_order = 3) THEN
         row3_ := Get_Address6(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address6_order = 4) THEN
         row4_ := Get_Address6(manufacturer_id_, address_id_);
      ELSIF (address_pres_.address6_order = 5) THEN
         row5_ := Get_Address6(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Address6(manufacturer_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.zip_code_row = line_) THEN   
      IF (address_pres_.zip_code_order = 1) THEN
         row1_ := Get_Zip_Code(manufacturer_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 2) THEN
         row2_ := Get_Zip_Code(manufacturer_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 3) THEN
         row3_ := Get_Zip_Code(manufacturer_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 4) THEN
         row4_ := Get_Zip_Code(manufacturer_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 5) THEN
         row5_ := Get_Zip_Code(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_Zip_Code(manufacturer_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.city_row = line_) THEN   
      IF (address_pres_.city_order = 1) THEN
         row1_ := Get_City(manufacturer_id_, address_id_);
      ELSIF (address_pres_.city_order = 2) THEN
         row2_ := Get_City(manufacturer_id_, address_id_);
      ELSIF (address_pres_.city_order = 3) THEN
         row3_ := Get_City(manufacturer_id_, address_id_);
      ELSIF (address_pres_.city_order = 4) THEN
         row4_ := Get_City(manufacturer_id_, address_id_);
      ELSIF (address_pres_.city_order = 5) THEN
         row5_ := Get_City(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_City(manufacturer_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.county_row = line_) THEN   
      IF (address_pres_.county_order = 1) THEN
         row1_ := Get_County(manufacturer_id_, address_id_);
      ELSIF (address_pres_.county_order = 2) THEN
         row2_ := Get_County(manufacturer_id_, address_id_);
      ELSIF (address_pres_.county_order = 3) THEN
         row3_ := Get_County(manufacturer_id_, address_id_);
      ELSIF (address_pres_.county_order = 4) THEN
         row4_ := Get_County(manufacturer_id_, address_id_);
      ELSIF (address_pres_.county_order = 5) THEN
         row5_ := Get_County(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_County(manufacturer_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.state_row = line_) THEN   
      IF (address_pres_.state_order = 1) THEN
         row1_ := Get_State(manufacturer_id_, address_id_);
      ELSIF (address_pres_.state_order = 2) THEN
         row2_ := Get_State(manufacturer_id_, address_id_);
      ELSIF (address_pres_.state_order = 3) THEN
         row3_ := Get_State(manufacturer_id_, address_id_);
      ELSIF (address_pres_.state_order = 4) THEN
         row4_ := Get_State(manufacturer_id_, address_id_);
      ELSIF (address_pres_.state_order = 5) THEN
         row5_ := Get_State(manufacturer_id_, address_id_);
      ELSE
         row1_ := Get_State(manufacturer_id_, address_id_);
      END IF;
   END IF;   
...
IS
   ...
   cnt_   NUMBER := 1;
BEGIN
   addr_ := RTRIM(REPLACE(Get_Address(manufacturer_id_, address_id_), CHR(13), ''), CHR(10));
   LOOP
      ptr_ := INSTR(addr_, CHR(10));
      IF (ptr_ > 0 AND ptr_ < LENGTH(addr_)) THEN
         cnt_ := cnt_ + 1;
...
IS
   ...
   CURSOR get_id_by IS
      SELECT a.address_id
      FROM   manufacturer_info_address_tab a, manuf_info_addr_type_tab t
      WHERE  a.manufacturer_id   = manufacturer_id_
      AND    t.manufacturer_id   = a.manufacturer_id
...
IS
   newrec_       manufacturer_info_address_tab%ROWTYPE;
   address1_     manufacturer_info_address_tab.address1%TYPE;
   address2_     manufacturer_info_address_tab.address2%TYPE;
   address3_     manufacturer_info_address_tab.address3%TYPE;
   address4_     manufacturer_info_address_tab.address4%TYPE;
   address5_     manufacturer_info_address_tab.address5%TYPE;
   address6_     manufacturer_info_address_tab.address6%TYPE;
   zip_code_     manufacturer_info_address_tab.zip_code%TYPE;
   city_         manufacturer_info_address_tab.city%TYPE;
   county_       manufacturer_info_address_tab.city%TYPE;
   state_        manufacturer_info_address_tab.state%TYPE;
BEGIN
   ...
   Client_SYS.Add_Info(lu_name_, 'MANADDR: The address might have been stored with an invalid format. Please check the address :P1 for manufacturer :P2', address_id_, manufacturer_id_);
END New;
...
PROCEDURE Modify (
   ...
   valid_to_        IN DATE DEFAULT NULL )
IS
   oldrec_       manufacturer_info_address_tab%ROWTYPE;
   newrec_       manufacturer_info_address_tab%ROWTYPE;
   address1_     manufacturer_info_address_tab.address1%TYPE;
   address2_     manufacturer_info_address_tab.address2%TYPE;
   address3_     manufacturer_info_address_tab.address3%TYPE;
   ...
   zip_code_     manufacturer_info_address_tab.zip_code%TYPE;
   city_         manufacturer_info_address_tab.city%TYPE;
   county_       manufacturer_info_address_tab.county%TYPE;
   state_        manufacturer_info_address_tab.state%TYPE;
   country_temp_ manufacturer_info_address.country%TYPE;
BEGIN
   oldrec_ := Get_Object_By_Keys___(manufacturer_id_, address_id_);
   newrec_ := oldrec_;
   newrec_.address      := address_;
   newrec_.country      := Iso_Country_API.Encode(country_);
   newrec_.ean_location := ean_location_;    
...
IS
   ...
   objversion_  VARCHAR2(200);
BEGIN
   Get_Id_Version_By_Keys___(objid_, objversion_, manufacturer_id_, address_id_);
...
BEGIN
   FOR t IN get_def_address LOOP
      IF (Manuf_Info_Addr_Type_API.Is_Default(manufacturer_id_, t.address_id, address_type_code_) = 'TRUE') THEN
         RETURN t.address_id;
      END IF;
   END LOOP;
   RETURN NULL;
...
IS
   ...
   name_            manufacturer_info_tab.name%TYPE;
...
IS
   address_       manufacturer_info_address_tab.address%TYPE;
...
BEGIN
   ...
   FOR rec_ IN get_records LOOP
      ...
      IF (address_ != rec_.address) THEN
         UPDATE manufacturer_info_address_tab
