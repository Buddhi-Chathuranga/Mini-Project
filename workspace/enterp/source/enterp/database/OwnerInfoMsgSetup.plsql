-----------------------------------------------------------------------------
--
--  Logical unit: OwnerInfoMsgSetup
--  Component:    ENTERP
--
...
PROCEDURE Check_Default___ (
   newrec_ IN owner_info_msg_setup_tab%ROWTYPE,
   objid_  IN VARCHAR2 )
IS
   dummy_ NUMBER;
   CURSOR meth_def IS
      SELECT 1
      FROM   owner_info_msg_setup
      WHERE  owner_id     = newrec_.owner_id
...
PROCEDURE Insert___ (
   ...
   newrec_     IN OUT owner_info_msg_setup_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2 )
IS
BEGIN
...
PROCEDURE Update___ (
   objid_      IN     VARCHAR2,
   oldrec_     IN     owner_info_msg_setup_tab%ROWTYPE,
   newrec_     IN OUT owner_info_msg_setup_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2,
...
FUNCTION Get_Default_Media_Code (
   ...
   message_class_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   media_code_ owner_info_msg_setup.media_code%TYPE;
   CURSOR get_media IS
      SELECT media_code
      FROM   owner_info_msg_setup
      WHERE  owner_id = owner_id_
