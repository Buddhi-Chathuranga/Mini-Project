-----------------------------------------------------------------------------
--
--  Logical unit: ForwarderInfoMsgSetup
--  Component:    ENTERP
--
...
--  ------  ------  ---------------------------------------------------------
--  981124  Camk    Created
--  131107  Isuklk  PBFI-2160 Refactoring and Split ForwarderInfoMsgSetup.entity
-----------------------------------------------------------------------------
...
PROCEDURE Check_Default___ (
   newrec_ IN forwarder_info_msg_setup_tab%ROWTYPE,
   objid_  IN VARCHAR2 )
IS
   dummy_ NUMBER;
   CURSOR meth_def IS
      SELECT 1
      FROM   forwarder_info_msg_setup
      WHERE  forwarder_id     = newrec_.forwarder_id
...
PROCEDURE Insert___ (
   ...
   newrec_     IN OUT forwarder_info_msg_setup_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2 )
IS
BEGIN
...
PROCEDURE Update___ (
   objid_      IN     VARCHAR2,
   oldrec_     IN     forwarder_info_msg_setup_tab%ROWTYPE,
   newrec_     IN OUT forwarder_info_msg_setup_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2,
...
FUNCTION Get_Default_Media_Code (
   ...
   message_class_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   media_code_ forwarder_info_msg_setup.media_code%TYPE;
   CURSOR get_media IS
      SELECT media_code
      FROM   forwarder_info_msg_setup
      WHERE  forwarder_id = forwarder_id_
