-----------------------------------------------------------------------------
--
--  Logical unit: ForwarderInfoAddress
--  Component:    ENTERP
--
...
--  060406  Sacalk  Bug 56972, Added function Sync_Addr, Modified New_ and Modify_ methods   
------------------------------------ 1.9.0 ----------------------------------
...
--  090922  Nsillk  Issue Id EAFH-123 Removed unnecessary COMMIT statements
...
-----------------------------------------------------------------------------  
--  131106  Isuklk  PBFI-2414 Refactoring in ForwarderInfoAddress.entity
...
--  160418  reanpl  STRLOC-120, Added handling of new attributes address3, address4, address5, address6, removed New_Address, Modify_Address
...
PROCEDURE Get_Forwarder_Party___ (
   newrec_ IN OUT forwarder_info_address_tab%ROWTYPE )
IS
BEGIN
...
PROCEDURE Check_Ean_Location___ (
   rec_ IN forwarder_info_address_tab%ROWTYPE )
IS
   objid_        VARCHAR2(100);
   objversion_   VARCHAR2(200);
BEGIN   
   Get_Id_Version_By_Keys___(objid_, objversion_, rec_.forwarder_id, rec_.address_id);
   IF (rec_.ean_location IS NOT NULL) THEN
      FOR a IN (SELECT 1
                FROM   forwarder_info_address
                WHERE  ean_location = rec_.ean_location
                AND    objid||''   <> NVL(objid_,CHR(0)) )
      LOOP
         Error_SYS.Record_Exist(lu_name_, 'EANEXIST: This Forwarder''s Own Address ID already exists on another address for this forwarder, or on another forwarder.');
      END LOOP;
   END IF;
...
PROCEDURE Check_Common___ (
   oldrec_ IN     forwarder_info_address_tab%ROWTYPE,
   newrec_ IN OUT forwarder_info_address_tab%ROWTYPE,
...
IS
   ...
   CURSOR get_address_types(forwarder_id_ IN VARCHAR2, address_id_ IN VARCHAR2) IS
      SELECT t.rowid, t.forwarder_id, Address_Type_Code_API.Decode(t.address_type_code) AS address_type_code, t.def_address
      FROM   forw_info_addr_type_tab t
      WHERE  t.forwarder_id = forwarder_id_
...
BEGIN
   ...
   IF (NVL(newrec_.valid_from, Database_Sys.Get_First_Calendar_Date()) != NVL(oldrec_.valid_from, Database_Sys.Get_First_Calendar_Date())) OR 
      ...
      FOR c1 IN get_address_types(newrec_.forwarder_id, newrec_.address_id) LOOP
         Forw_Info_Addr_Type_API.Check_Def_Address_Exist(validation_result_, 
                                                         validation_flag_, 
                                                         c1.forwarder_id, 
                                                         c1.def_address, 
   ...
   IF (NVL(newrec_.valid_from, Database_Sys.Get_First_Calendar_Date()) != NVL(oldrec_.valid_from, Database_Sys.Get_First_Calendar_Date())) OR 
      ...
      FOR c1 IN get_address_types(newrec_.forwarder_id, newrec_.address_id) LOOP
         Forw_Info_Addr_Type_API.Check_Def_Address_Exist(validation_result_, 
                                                         ...
                                                         newrec_.valid_to);
         IF (c1.def_address = 'TRUE' AND (validation_result_ = 'FALSE')) THEN
            Forw_Info_Addr_Type_API.Check_Def_Addr_Temp(c1.forwarder_id, 
                                                        c1.address_type_code, 
                                                        c1.def_address,
                                                        c1.rowid, 
                                                        newrec_.valid_from, 
...
PROCEDURE Check_Delete___ (
   remrec_ IN forwarder_info_address_tab%ROWTYPE )
IS
   ...
   CURSOR get_address_types (forwarder_id_ VARCHAR2, address_id_ VARCHAR2) IS
      SELECT *
      FROM   forw_info_addr_type
      WHERE  forwarder_id = forwarder_id_
      AND    address_id = address_id_;
   CURSOR get_def_address_types_count (forwarder_id_ VARCHAR2, address_id_ VARCHAR2) IS
      SELECT COUNT(*)
      FROM   forw_info_addr_type
      WHERE  forwarder_id = forwarder_id_
      AND    address_id = address_id_
...
BEGIN
   OPEN get_def_address_types_count(remrec_.forwarder_id, remrec_.address_id);
   FETCH get_def_address_types_count INTO addr_type_count_;
   CLOSE get_def_address_types_count;
   FOR rec_ IN get_address_types(remrec_.forwarder_id, remrec_.address_id) LOOP
      Forw_Info_Addr_Type_API.Check_Def_Address_Exist(validation_result_, validation_flag_, rec_.forwarder_id, rec_.def_address, rec_.address_type_code_db, rec_.objid, remrec_.valid_from, remrec_.valid_to);
   ...
   IF (addr_type_code_list_ IS NOT NULL) THEN
      Client_SYS.Add_Warning(lu_name_, 'REMOVEADDRWITHDEFADDRTYPES: This is the default :P1 Address Type(s) for Forwarder :P2. If removed, there will be no default address for this Address Type(s).', addr_type_code_list_, remrec_.forwarder_id);
   END IF;
...
PROCEDURE Remove__ (
   ...
   action_     IN  VARCHAR2 )
IS
   remrec_ forwarder_info_address_tab%ROWTYPE;
BEGIN
   IF (action_ = 'DO') THEN
      remrec_ := Get_Object_By_Id___(objid_);
      Cascade_Delete_Comm_Method__(remrec_.forwarder_id, remrec_.address_id);
   END IF;   
...
IS
   country_       forwarder_info_address.country%TYPE;
   address_pres_  Address_Presentation_API.Public_Rec_Address;
   line_          NUMBER;
   row1_          VARCHAR2(100);
   row2_          VARCHAR2(100);
   ...
   row5_          VARCHAR2(100);
BEGIN
   country_ := Get_Country(forwarder_id_, address_id_);
   address_pres_ := Address_Presentation_API.Get_Address_Record(Iso_Country_API.Encode(country_));
   line_ := line_no_;
   ...
   IF (address_pres_.address1_row = line_) THEN   
      IF (address_pres_.address1_order = 1) THEN
         row1_ := Get_Address1(forwarder_id_, address_id_);
      ELSIF (address_pres_.address1_order = 2) THEN
         row2_ := Get_Address1(forwarder_id_, address_id_);
      ELSIF (address_pres_.address1_order = 3) THEN
         row3_ := Get_Address1(forwarder_id_, address_id_);
      ELSIF (address_pres_.address1_order = 4) THEN
         row4_ := Get_Address1(forwarder_id_, address_id_);
      ELSIF (address_pres_.address1_order = 5) THEN
         row5_ := Get_Address1(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Address1(forwarder_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.address2_row = line_) THEN   
      IF (address_pres_.address2_order = 1) THEN
         row1_ := Get_Address2(forwarder_id_, address_id_);
      ELSIF (address_pres_.address2_order = 2) THEN
         row2_ := Get_Address2(forwarder_id_, address_id_);
      ELSIF (address_pres_.address2_order = 3) THEN
         row3_ := Get_Address2(forwarder_id_, address_id_);
      ELSIF (address_pres_.address2_order = 4) THEN
         row4_ := Get_Address2(forwarder_id_, address_id_);
      ELSIF (address_pres_.address2_order = 5) THEN
         row5_ := Get_Address2(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Address2(forwarder_id_, address_id_);
      END IF;
   END IF; 
   IF (address_pres_.address3_row = line_) THEN
      IF (address_pres_.address3_order = 1) THEN
         row1_ := Get_Address3(forwarder_id_, address_id_);
      ELSIF (address_pres_.address3_order = 2) THEN
         row2_ := Get_Address3(forwarder_id_, address_id_);
      ELSIF (address_pres_.address3_order = 3) THEN
         row3_ := Get_Address3(forwarder_id_, address_id_);
      ELSIF (address_pres_.address3_order = 4) THEN
         row4_ := Get_Address3(forwarder_id_, address_id_);
      ELSIF (address_pres_.address3_order = 5) THEN
         row5_ := Get_Address3(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Address3(forwarder_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.address4_row = line_) THEN
      IF (address_pres_.address4_order = 1) THEN
         row1_ := Get_Address4(forwarder_id_, address_id_);
      ELSIF (address_pres_.address4_order = 2) THEN
         row2_ := Get_Address4(forwarder_id_, address_id_);
      ELSIF (address_pres_.address4_order = 3) THEN
         row3_ := Get_Address4(forwarder_id_, address_id_);
      ELSIF (address_pres_.address4_order = 4) THEN
         row4_ := Get_Address4(forwarder_id_, address_id_);
      ELSIF (address_pres_.address4_order = 5) THEN
         row5_ := Get_Address4(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Address4(forwarder_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.address5_row = line_) THEN
      IF (address_pres_.address5_order = 1) THEN
         row1_ := Get_Address5(forwarder_id_, address_id_);
      ELSIF (address_pres_.address5_order = 2) THEN
         row2_ := Get_Address5(forwarder_id_, address_id_);
      ELSIF (address_pres_.address5_order = 3) THEN
         row3_ := Get_Address5(forwarder_id_, address_id_);
      ELSIF (address_pres_.address5_order = 4) THEN
         row4_ := Get_Address5(forwarder_id_, address_id_);
      ELSIF (address_pres_.address5_order = 5) THEN
         row5_ := Get_Address5(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Address5(forwarder_id_, address_id_);
      END IF;
   END IF;
   IF (address_pres_.address6_row = line_) THEN
      IF (address_pres_.address6_order = 1) THEN
         row1_ := Get_Address6(forwarder_id_, address_id_);
      ELSIF (address_pres_.address6_order = 2) THEN
         row2_ := Get_Address6(forwarder_id_, address_id_);
      ELSIF (address_pres_.address6_order = 3) THEN
         row3_ := Get_Address6(forwarder_id_, address_id_);
      ELSIF (address_pres_.address6_order = 4) THEN
         row4_ := Get_Address6(forwarder_id_, address_id_);
      ELSIF (address_pres_.address6_order = 5) THEN
         row5_ := Get_Address6(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Address6(forwarder_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.zip_code_row = line_) THEN   
      IF (address_pres_.zip_code_order = 1) THEN
         row1_ := Get_Zip_Code(forwarder_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 2) THEN
         row2_ := Get_Zip_Code(forwarder_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 3) THEN
         row3_ := Get_Zip_Code(forwarder_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 4) THEN
         row4_ := Get_Zip_Code(forwarder_id_, address_id_);
      ELSIF (address_pres_.zip_code_order = 5) THEN
         row5_ := Get_Zip_Code(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_Zip_Code(forwarder_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.city_row = line_) THEN   
      IF (address_pres_.city_order = 1) THEN
         row1_ := Get_City(forwarder_id_, address_id_);
      ELSIF (address_pres_.city_order = 2) THEN
         row2_ := Get_City(forwarder_id_, address_id_);
      ELSIF (address_pres_.city_order = 3) THEN
         row3_ := Get_City(forwarder_id_, address_id_);
      ELSIF (address_pres_.city_order = 4) THEN
         row4_ := Get_City(forwarder_id_, address_id_);
      ELSIF (address_pres_.city_order = 5) THEN
         row5_ := Get_City(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_City(forwarder_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.county_row = line_) THEN   
      IF (address_pres_.county_order = 1) THEN
         row1_ := Get_County(forwarder_id_, address_id_);
      ELSIF (address_pres_.county_order = 2) THEN
         row2_ := Get_County(forwarder_id_, address_id_);
      ELSIF (address_pres_.county_order = 3) THEN
         row3_ := Get_County(forwarder_id_, address_id_);
      ELSIF (address_pres_.county_order = 4) THEN
         row4_ := Get_County(forwarder_id_, address_id_);
      ELSIF (address_pres_.county_order = 5) THEN
         row5_ := Get_County(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_County(forwarder_id_, address_id_);
      END IF;
   END IF;   
   IF (address_pres_.state_row = line_) THEN   
      IF (address_pres_.state_order = 1) THEN
         row1_ := Get_State(forwarder_id_, address_id_);
      ELSIF (address_pres_.state_order = 2) THEN
         row2_ := Get_State(forwarder_id_, address_id_);
      ELSIF (address_pres_.state_order = 3) THEN
         row3_ := Get_State(forwarder_id_, address_id_);
      ELSIF (address_pres_.state_order = 4) THEN
         row4_ := Get_State(forwarder_id_, address_id_);
      ELSIF (address_pres_.state_order = 5) THEN
         row5_ := Get_State(forwarder_id_, address_id_);
      ELSE
         row1_ := Get_State(forwarder_id_, address_id_);
      END IF;
   END IF;   
...
IS
   ...
   cnt_   NUMBER := 1;
BEGIN
   addr_ := RTRIM(REPLACE(Get_Address(forwarder_id_,address_id_), CHR(13), ''), CHR(10));
   LOOP
      ptr_ := INSTR(addr_, CHR(10));
      IF (ptr_ > 0 AND ptr_ < LENGTH(addr_)) THEN
         cnt_ := cnt_ + 1;
...
IS
   ...
   CURSOR get_id_by IS
     SELECT A.address_id
       FROM   forwarder_info_address a, forw_info_addr_type t
       WHERE  a.forwarder_id = forwarder_id_
       AND    t.forwarder_id = a.forwarder_id
...
IS
   newrec_      forwarder_info_address_tab%ROWTYPE;
   address1_    forwarder_info_address_tab.address1%TYPE;
   address2_    forwarder_info_address_tab.address2%TYPE;
   address3_    forwarder_info_address_tab.address3%TYPE;
   address4_    forwarder_info_address_tab.address4%TYPE;
   address5_    forwarder_info_address_tab.address5%TYPE;
   address6_    forwarder_info_address_tab.address6%TYPE;
   zip_code_    forwarder_info_address_tab.zip_code%TYPE;
   city_        forwarder_info_address_tab.city%TYPE;
   county_      forwarder_info_address_tab.county%TYPE;
   state_       forwarder_info_address_tab.state%TYPE;  
BEGIN  
   ...
   Client_SYS.Add_Info(lu_name_, 'FORWADDR: The address might have been stored with an invalid format. Please check the address :P1 for forwarder :P2', address_id_, forwarder_id_);
END New;
...
PROCEDURE Modify (
   ...
   valid_to_     IN DATE DEFAULT NULL )
IS
   oldrec_       forwarder_info_address_tab%ROWTYPE;
   newrec_       forwarder_info_address_tab%ROWTYPE;
   address1_     forwarder_info_address_tab.address1%TYPE;
   address2_     forwarder_info_address_tab.address2%TYPE;
   address3_     forwarder_info_address_tab.address3%TYPE;
   ...
   county_       forwarder_info_address_tab.county%TYPE;
   state_        forwarder_info_address_tab.state%TYPE;
   country_temp_ forwarder_info_address.country%TYPE;
BEGIN
   oldrec_ := Get_Object_By_Keys___(forwarder_id_, address_id_);
   newrec_ := oldrec_;
   newrec_.address      := address_;
   newrec_.country      := Iso_Country_API.Encode(country_);
   newrec_.ean_location := ean_location_;    
...
IS
   ...
   objversion_   VARCHAR2(200);
BEGIN
   Get_Id_Version_By_Keys___(objid_, objversion_, forwarder_id_, address_id_);
...
BEGIN
   FOR t IN get_def_address LOOP
      IF (Forw_Info_Addr_Type_API.Is_Default(forwarder_id_, t.address_id, address_type_code_) = 'TRUE') THEN
         RETURN t.address_id;
      END IF;
   END LOOP;
   RETURN NULL;
...
IS
   ...
   name_            Forwarder_Info_TAB.name%TYPE;
...
IS
   address_    forwarder_info_address_tab.address%TYPE;
...
BEGIN
   ...
   FOR rec_ IN get_records LOOP
      ...
      IF (address_ != rec_.address) THEN
         UPDATE forwarder_info_address_tab
