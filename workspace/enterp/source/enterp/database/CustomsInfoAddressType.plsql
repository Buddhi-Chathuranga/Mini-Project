-----------------------------------------------------------------------------
--
--  Logical unit: CustomsInfoAddressType
--  Component:    ENTERP
--
...
--  011011  Thsrlk  corrected bug # 24654 in Unpack_Check_Insert___ 
...
--  131108  Isuklk  PBFI-3344 Refactoring in CustomsInfoAddressType.entity
--  141122  AmThLK  Added Check_Insert___ to validate default_address_type 
...
--  210203  Hecolk  FISPRING20-8730, Get rid of string manipulations in db - Modified in New methods
--  210105  Smallk  FIZ-11253, Merged Bug 157213. 
-----------------------------------------------------------------------------
...
PROCEDURE Check_Def_Address___ (
   customs_id_         IN customs_info_address_type_tab.customs_id%TYPE,
   address_type_code_  IN customs_info_address_type_tab.address_type_code%TYPE,
   def_address_        IN customs_info_address_type_tab.def_address%TYPE,
   objid_              IN VARCHAR2,
   valid_from_         IN DATE,
   valid_to_           IN DATE )
IS
   oldrec_ customs_info_address_type_tab%ROWTYPE;   
   CURSOR def_addr IS
      SELECT b.ROWID, TO_CHAR(b.rowversion) AS objversion
      FROM   customs_info_address_tab a, customs_info_address_type_tab b
      WHERE  a.customs_id        =  b.customs_id
      AND    a.address_id        =  b.address_id
      AND    b.customs_id        =  customs_id_   
...
BEGIN
   IF (def_address_  = 'TRUE') THEN
      FOR d IN def_addr LOOP
         ...
         UPDATE customs_info_address_type_tab
...
PROCEDURE Check_Common___ (
   oldrec_ IN     customs_info_address_type_tab%ROWTYPE,
   newrec_ IN OUT customs_info_address_type_tab%ROWTYPE,
   indrec_ IN OUT Indicator_Rec,
   attr_   IN OUT VARCHAR2 )
IS
   address_rec_             Customs_Info_Address_API.Public_Rec;
   objid_                   VARCHAR2(100);
...
BEGIN
   ...
   is_invalid_address_  := INSTR(Address_Type_Code_API.Get_Addr_Typs_For_Party_Type('CUSTOMS'), newrec_.address_type_code) = 0;
   IF (is_invalid_address_) THEN
      Error_SYS.Record_General(lu_name_, 'ADDRTYPINVCUSTOMS: Address type :P1 is invalid for Customs.', address_type_code_);
   END IF;
   address_rec_ := Customs_Info_Address_API.Get(newrec_.customs_id, newrec_.address_id);
   Get_Id_Version_By_Keys___(objid_, objversion_, newrec_.customs_id, newrec_.address_id, newrec_.address_type_code);
   Check_Def_Address_Exist(validation_result_, validation_flag_, newrec_.customs_id, newrec_.def_address, address_type_code_, objid_, address_rec_.valid_from, address_rec_.valid_to);
   ...
   IF (newrec_.def_address = 'FALSE' AND (validation_flag_ = 'TRUE')) THEN
      Client_SYS.Add_Warning(lu_name_, 'REMOVEADDRTYPE: This is the default :P1 Address Type for Customs :P2. If removed, there will be no default address for this Address Type.', address_type_code_, newrec_.customs_id);
   END IF; 
END Check_Common___;
...
PROCEDURE Check_Delete___ (
   remrec_ IN customs_info_address_type_tab%ROWTYPE )
IS
   address_rec_             Customs_Info_Address_API.Public_Rec;
   objid_                   VARCHAR2(100);
   objversion_              VARCHAR2(200);
   ...
   CURSOR get_count (customs_id_ VARCHAR2, address_id_ VARCHAR2) IS
      SELECT COUNT(*) 
...
BEGIN  
   ...
   address_rec_ := Customs_Info_Address_API.Get(remrec_.customs_id,  remrec_.address_id);
   Get_Id_Version_By_Keys___(objid_, objversion_, remrec_.customs_id, remrec_.address_id, remrec_.address_type_code);
   OPEN get_count(remrec_.customs_id, remrec_.address_id);
   FETCH get_count INTO add_type_count_;
   CLOSE get_count;
   IF (remrec_.def_address = 'TRUE') THEN
      Check_Def_Address_Exist(validation_result_, validation_flag_, remrec_.customs_id, remrec_.def_address, address_type_code_, objid_, address_rec_.valid_from, address_rec_.valid_to);
      IF (validation_result_ = 'TRUE') THEN
         Client_SYS.Clear_Info();
         Client_SYS.Add_Warning(lu_name_, 'REMOVEADDRTYPE: This is the default :P1 Address Type for Customs :P2. If removed, there will be no default address for this Address Type.', address_type_code_, remrec_.customs_id);
      END IF;
   END IF;
   IF (add_type_count_ = 1) THEN
      Client_SYS.Add_Warning(lu_name_, 'REMOVELASTADDTYPE: This is the last address type(s) for address identity :P1 of Customs :P2. If removed, there will not be any address type(s) for this address ID.', remrec_.address_id, remrec_.customs_id);
   END IF;     
   super(remrec_);
END Check_Delete___;
...
BEGIN
   ...
   Check_Def_Address_Exist(validation_result_, validation_flag_, newrec_.customs_id, newrec_.def_address, address_type_code_, objid_, address_rec_.valid_from, address_rec_.valid_to);
   IF (newrec_.def_address = 'TRUE' AND (validation_result_ = 'FALSE')) THEN
      Check_Def_Addr_Temp(newrec_.customs_id, address_type_code_, newrec_.def_address, objid_, address_rec_.valid_from, address_rec_.valid_to);
   END IF;
END Insert___;
...
IS
   ...
   CURSOR get_def IS
      ...
      FROM   customs_info_address_type_tab b
      WHERE  b.customs_id          = customs_id_
      AND    b.address_type_code   = Address_Type_Code_API.Encode(address_type_code_)
      AND    b.ROWID||''           = NVL(objid_,CHR(0));
   dummy_      VARCHAR2(20);
BEGIN
...
BEGIN
   Check_Def_Address___(customs_id_, address_type_code_, def_address_, objid_, valid_from_, valid_to_);  
...
IS
BEGIN
   IF (Check_Exist___(customs_id_, address_id_, address_type_code_)) THEN
      RETURN 'TRUE';
   ELSE
      RETURN 'FALSE';
   END IF;
...
IS
   newrec_       customs_info_address_type_tab%ROWTYPE;
BEGIN
...
PROCEDURE Remove (
   ...
   address_type_code_ IN VARCHAR2 )
IS
   remrec_       customs_info_address_type_tab%ROWTYPE;
   objid_        VARCHAR2(100);
   objversion_   VARCHAR2(200);
BEGIN
   Get_Id_Version_By_Keys___(objid_, objversion_, customs_id_, address_id_, Address_Type_Code_API.Encode(address_type_code_));
...
BEGIN
   addr_types_ := Address_Type_Code_API.Get_Addr_Typs_For_Party_Type('CUSTOMS');
   IF (addr_types_ IS NOT NULL) THEN
      ...
      FOR addr_type_ IN 1 .. addr_types_count_ LOOP
         def_exist_ := Default_Exist(customs_id_, address_id_, Address_Type_Code_API.Decode(TRIM(addr_types_table_(addr_type_))));
         IF (def_exist_ = 'TRUE') THEN
            def_address_ := 'FALSE';
         ELSE
            def_address_ := 'TRUE';
         END IF;
         New(customs_id_, address_id_, Address_Type_Code_API.Decode(TRIM(addr_types_table_(addr_type_))), def_address_);
      END LOOP;
   END IF;
