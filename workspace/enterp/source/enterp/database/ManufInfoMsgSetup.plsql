-----------------------------------------------------------------------------
--
--  Logical unit: ManufInfoMsgSetup
--  Component:    ENTERP
--
...
PROCEDURE Check_Default___ (
   newrec_ IN manuf_info_msg_setup_tab%ROWTYPE,
   objid_ IN VARCHAR2 )
IS
   dummy_ NUMBER;
   CURSOR meth_def IS
      SELECT 1
      FROM   manuf_info_msg_setup_tab
      WHERE  manufacturer_id     = newrec_.manufacturer_id
...
PROCEDURE Insert___ (
   ...
   newrec_     IN OUT manuf_info_msg_setup_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2 )
IS
BEGIN
...
PROCEDURE Update___ (
   objid_      IN     VARCHAR2,
   oldrec_     IN     manuf_info_msg_setup_tab%ROWTYPE,
   newrec_     IN OUT manuf_info_msg_setup_tab%ROWTYPE,
   attr_       IN OUT VARCHAR2,
...
IS
   media_code_ manuf_info_msg_setup.media_code%TYPE;
   CURSOR get_media IS
      SELECT media_code
      FROM   manuf_info_msg_setup_tab
