...
--  171123  CKumlk  STRSC-13401, Added new methods Data_Capt_Item_Value_Exist, Get_Data_Capt_Item_Value_Desc.
--  171122  SURBLK  Created, added method Create_Data_Capture_Lov.
-----------------------------------------------------------------------------
...
-- This method is used by DataCaptCountHandlUnit 
@ServerOnlyAccess 
...
BEGIN
   IF (data_item_value_ = 'EXISTS') THEN
      description_ :=  SUBSTR(Get_Client_Value(0),1,50);
   ELSIF (data_item_value_ = 'DOES_NOT_EXIST') THEN
      description_ :=  SUBSTR(Get_Client_Value(1),1,50);
   ELSIF (data_item_value_ = 'COUNT_PART') THEN
