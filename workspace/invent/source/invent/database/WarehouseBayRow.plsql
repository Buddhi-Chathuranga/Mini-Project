...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  211211  JaThlk  SC21R2-2932, Added new function Get_Default_Row_Id. 
...
--                  Validation from FALSE to TRUE on the row level.
...
--  140220  AwWelk  PBSC-7300, Removed the overtake in Get_Bin_Volume_Capacity().
--  130625  Matkse  Modified Get_Attr___ to handle availability_control_id.
--  130620  Matkse  Modified Clear_Storage_Chars__, Clear_Bin_Storage_Chars__ and Clear_Bin_Storage_Chars___ to accept flag for availability_control_id.
--  130521  Erlise  Added availability_control_id.
--  121115  MaEelk  Made a call to Storage_Zone_Detail_API.Remove_Row from Delete___ that would remove the relevent record when remind a warehouse 
...
--  120605  Matkse  Modified Delete___ by adding calls to Site_Putaway_Zone_API.Remove_Row and Invent_Part_Putaway_Zone_API.Remove_Row
...
--  120309          Get_Bin_Min_Humidity_Source_Db, Get_Bin_Max_Humidity_Source_Db, Get_Row_Carry_Cap_Source_Db.
--  120309  Matkse  Added new LOV view WAREHOUSE_BAY_ROW_LOV2
...
--  110707  MaEelk  Added user allowed site filter to WAREHOUSE_BAY_ROW and WAREHOUSE_BAY_ROW_LOV.
...
--  091106  NaLrlk  Added view WAREHOUSE_BAY_ROW_LOV.
...
--  091008  NaLrlk  Modified the WAREHOUSE_BAY_ROW view ref column to CASCADE check.
--  090907  ShKolk  Renamed method Create_Default_Row to New.
...
--  090819  NaLrlk  Added function Get_Row_Carry_Capacity_Source.
...
default_row_id_ CONSTANT VARCHAR2(3)  := '  -';
...
PROCEDURE Get_Bin_Vol_Cap_And_Src_Db___ (
   bin_volume_capacity_           OUT WAREHOUSE_BAY_ROW_TAB.bin_volume_capacity%TYPE,
   bin_volume_capacity_source_db_ OUT VARCHAR2,
   contract_                      IN WAREHOUSE_BAY_ROW_TAB.contract%TYPE,
   warehouse_id_                  IN WAREHOUSE_BAY_ROW_TAB.warehouse_id%TYPE,
   bay_id_                        IN WAREHOUSE_BAY_ROW_TAB.bay_id%TYPE,
   row_id_                        IN WAREHOUSE_BAY_ROW_TAB.row_id%TYPE,
   get_capacity_                  IN BOOLEAN,
   get_source_                    IN BOOLEAN )
IS
   bin_height_capacity_ WAREHOUSE_BAY_ROW_TAB.bin_height_capacity%TYPE;
   bin_width_capacity_  WAREHOUSE_BAY_ROW_TAB.bin_width_capacity%TYPE;
   bin_dept_capacity_   WAREHOUSE_BAY_ROW_TAB.bin_dept_capacity%TYPE;
BEGIN
   IF (Site_Invent_Info_API.Get_Volume_Uom(contract_) IS NOT NULL) THEN
      Update_Cache___(contract_, warehouse_id_, bay_id_, row_id_);
      IF ( micro_cache_value_.bin_volume_capacity IS NULL) THEN
         IF ((micro_cache_value_.bin_width_capacity IS NULL) OR
            ...
            END IF;
         ELSE
            bin_height_capacity_ := Get_Bin_Height_Capacity(contract_, warehouse_id_, bay_id_, row_id_);
            IF (bin_height_capacity_ IS NOT NULL) THEN
               bin_width_capacity_  := Get_bin_Width_Capacity(contract_, warehouse_id_, bay_id_, row_id_);
               IF (bin_width_capacity_ IS NOT NULL) THEN
                  bin_dept_capacity_   := Get_Bin_Dept_Capacity(contract_, warehouse_id_, bay_id_, row_id_);
                  IF (bin_dept_capacity_ IS NOT NULL) THEN
                     bin_volume_capacity_ := bin_height_capacity_ * bin_width_capacity_ * bin_dept_capacity_;
                     bin_volume_capacity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
                  END IF;
               END IF;
            END IF;
         END IF;
...
PROCEDURE Insert___ (
   ...
   newrec_     IN OUT WAREHOUSE_BAY_ROW_TAB%ROWTYPE,
   attr_       IN OUT VARCHAR2 )
IS
BEGIN
   ...
   Check_Bin_Temperature_Range(newrec_.contract, newrec_.warehouse_id, newrec_.bay_id, newrec_.row_id);
   Check_Bin_Humidity_Range   (newrec_.contract, newrec_.warehouse_id, newrec_.bay_id, newrec_.row_id);
...
PROCEDURE Update___ (
   objid_      IN     VARCHAR2,
   oldrec_     IN     WAREHOUSE_BAY_ROW_TAB%ROWTYPE,
   newrec_     IN OUT WAREHOUSE_BAY_ROW_TAB%ROWTYPE,
   attr_       IN OUT VARCHAR2,
...
BEGIN
   ...
   IF ((NVL(newrec_.bin_min_temperature, number_null_) != NVL(oldrec_.bin_min_temperature, number_null_)) OR
      ...
      -- This checks needs to be performed after updating the record
...
PROCEDURE Delete___ (
   objid_  IN VARCHAR2,
   remrec_ IN WAREHOUSE_BAY_ROW_TAB%ROWTYPE )
IS
BEGIN
   super(objid_, remrec_);
   Storage_Zone_Detail_API.Remove_Row(remrec_.contract, remrec_.warehouse_id, remrec_.bay_id, remrec_.row_id);   
END Delete___;


FUNCTION Get_Attr___ (
   lu_rec_                     IN WAREHOUSE_BAY_ROW_TAB%ROWTYPE,
...
BEGIN
   ...
   FOR rec_ IN get_capabilities LOOP
      Warehouse_Row_Capability_API.Copy__(from_contract_,
                                          from_warehouse_id_,
                                          from_bay_id_,
                                          from_row_id_,
...
BEGIN
   IF NOT (indrec_.route_order) THEN
      newrec_.route_order := newrec_.row_id;
   END IF;
   ...
   IF (newrec_.row_id != default_row_id_) THEN
      Error_SYS.Check_Valid_Key_String('ROW_ID', newrec_.row_id);
   END IF;
   
   IF (newrec_.route_order != default_row_id_) THEN
      Error_SYS.Check_Valid_Key_String('ROUTE_ORDER', newrec_.route_order);
   END IF;
   
   IF (UPPER(newrec_.row_id) != newrec_.row_id) THEN
      Error_SYS.Record_General(lu_name_,'UPPERCASE: The Row ID must be entered in upper-case.');
   END IF;
...
BEGIN
   ...
   IF (oldrec_.receipts_blocked = Fnd_Boolean_API.db_false) AND (newrec_.receipts_blocked = Fnd_Boolean_API.db_true) THEN
      -- Receipts Blocked has been changed from FALSE to TRUE on the Row level.
      IF (Get_Receipts_Blocked_Db(newrec_.contract,
                                  newrec_.warehouse_id,
                                  newrec_.bay_id,
                                  newrec_.row_id) = Fnd_Boolean_API.db_true) THEN
         -- The Row is already blocked because whole bay is blocked.
   ...
   IF (oldrec_.receipt_to_occupied_blocked = Fnd_Boolean_API.db_false) AND (newrec_.receipt_to_occupied_blocked = Fnd_Boolean_API.db_true) THEN
      -- Receipt to occupied blocked has been changed from FALSE to TRUE on the Row level.
      IF (Get_Receipt_To_Occup_Blockd_Db(newrec_.contract,
                                         newrec_.warehouse_id,
                                         newrec_.bay_id,
                                         newrec_.row_id) = Fnd_Boolean_API.db_true) THEN
         -- The Row is already blocked because the whole bay is blocked.
   ...
   IF (oldrec_.mix_of_part_number_blocked = Fnd_Boolean_API.db_false) AND (newrec_.mix_of_part_number_blocked = Fnd_Boolean_API.db_true) THEN
      -- Mix of Part Numbers Blocked has been changed from FALSE to TRUE on the Row level.
   ...
   IF (oldrec_.mix_of_cond_codes_blocked = Fnd_Boolean_API.db_false) AND (newrec_.mix_of_cond_codes_blocked = Fnd_Boolean_API.db_true) THEN
      -- Mix of Condition Codes Blocked has been changed from FALSE to TRUE on the Row level.
   ...
   IF (oldrec_.mix_of_lot_batch_no_blocked = Fnd_Boolean_API.db_false) AND (newrec_.mix_of_lot_batch_no_blocked = Fnd_Boolean_API.db_true) THEN
      -- Mix of Lot Batch Numbers Blocked has been changed from FALSE to TRUE on the Row level.
   ...
   IF (oldrec_.exclude_storage_req_val = Fnd_Boolean_API.db_false) AND (newrec_.exclude_storage_req_val = Fnd_Boolean_API.db_true) THEN
      -- Exclude Storage Requirement Validation has been changed from FALSE to TRUE on the Row level.
      IF (Get_Exclude_Storage_Req_Val_Db(newrec_.contract,
                                         newrec_.warehouse_id,
                                         newrec_.bay_id,
                                         newrec_.row_id) = Fnd_Boolean_API.db_true) THEN
         -- The Row is already excluded because whole Warehouse is excluded.
...
IS
   newrec_     WAREHOUSE_BAY_ROW_TAB%ROWTYPE;
   indrec_     Indicator_Rec;
   empty_rec_  WAREHOUSE_BAY_ROW_TAB%ROWTYPE;
   attr_       VARCHAR2(2000);
   objid_      VARCHAR2(2000);
   objversion_ VARCHAR2(2000);
BEGIN

   Exist(from_contract_, from_warehouse_id_, from_bay_id_, from_row_id_);
   Warehouse_Bay_API.Exist(to_contract_, to_warehouse_id_, to_bay_id_);

   IF NOT (Check_Exist___(to_contract_, to_warehouse_id_, to_bay_id_, to_row_id_)) THEN
...
IS
   newrec_     WAREHOUSE_BAY_ROW_TAB%ROWTYPE;
   oldrec_     WAREHOUSE_BAY_ROW_TAB%ROWTYPE;
   indrec_     Indicator_Rec;
   attr_       VARCHAR2(2000);
   objid_      VARCHAR2(2000);
   objversion_ VARCHAR2(2000);
BEGIN
   ...
   oldrec_ := Lock_By_Keys___(contract_, warehouse_id_, bay_id_, row_id_);
   newrec_ := oldrec_;
   Unpack___(newrec_, indrec_, attr_);
   ...
   IF (capabilities_db_) THEN
      Warehouse_Row_Capability_API.Clear_Storage_Capabilities__(contract_, 
                                                                warehouse_id_, 
                                                                bay_id_,
                                                                row_id_,
                                                                all_capabilities_db_,
...
BEGIN
   ...
   IF (capabilities_ = 1) THEN
      IF (all_capabilities_ = 0) THEN 
         capability_tab_ := Warehouse_Bin_Capability_API.Get_Operative_Capabilities(contract_, warehouse_id_, bay_id_, NULL, row_id_, NULL);
      END IF;
   END IF;
...
IS
   ...
   newrec_     WAREHOUSE_BAY_ROW_TAB%ROWTYPE;
   indrec_     Indicator_Rec;
   objid_      WAREHOUSE_BAY_ROW.objid%TYPE;
   objversion_ WAREHOUSE_BAY_ROW.objversion%TYPE;
BEGIN
   ...
   Client_SYS.Add_To_Attr('ROUTE_ORDER', row_id_, attr_);
...
FUNCTION Get_Bin_Height_Capacity (
   ...
   warehouse_id_ IN VARCHAR2,
   bay_id_       IN VARCHAR2,
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_height_capacity_ WAREHOUSE_BAY_ROW_TAB.bin_height_capacity%TYPE;
BEGIN
   bin_height_capacity_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
IS
   bin_width_capacity_ WAREHOUSE_BAY_ROW_TAB.bin_width_capacity%TYPE;
BEGIN
   bin_width_capacity_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
IS
   bin_dept_capacity_ WAREHOUSE_BAY_ROW_TAB.bin_dept_capacity%TYPE;
BEGIN
   bin_dept_capacity_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
FUNCTION Get_Bin_Volume_Capacity (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_volume_capacity_ WAREHOUSE_BAY_ROW_TAB.bin_volume_capacity%TYPE;
   dummy_               VARCHAR2(20);
BEGIN
...
IS
   ...
   dummy_                    WAREHOUSE_BAY_ROW_TAB.bin_volume_capacity%TYPE;
BEGIN
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Volume_Cap_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
FUNCTION Get_Bin_Carrying_Capacity (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_carrying_capacity_ WAREHOUSE_BAY_ROW_TAB.bin_carrying_capacity%TYPE;
BEGIN
   bin_carrying_capacity_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
FUNCTION Get_Row_Carrying_Capacity (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   row_carrying_capacity_ WAREHOUSE_BAY_ROW_TAB.row_carrying_capacity%TYPE;
BEGIN
   row_carrying_capacity_ := super(contract_, warehouse_id_, bay_id_, row_id_);

   IF (row_carrying_capacity_ IS NULL) THEN
      row_carrying_capacity_ := Warehouse_Bay_API.Get_Row_Carrying_Capacity(contract_, warehouse_id_, bay_id_);
   END IF;
...
FUNCTION Get_Bin_Min_Temperature (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_min_temperature_ WAREHOUSE_BAY_ROW_TAB.bin_min_temperature%TYPE;
BEGIN
   bin_min_temperature_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
FUNCTION Get_Bin_Max_Temperature (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_max_temperature_ WAREHOUSE_BAY_ROW_TAB.bin_max_temperature%TYPE;
BEGIN
   bin_max_temperature_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
FUNCTION Get_Bin_Min_Humidity (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_min_humidity_ WAREHOUSE_BAY_ROW_TAB.bin_min_humidity%TYPE;
BEGIN
   bin_min_humidity_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
FUNCTION Get_Bin_Max_Humidity (
   ...
   row_id_       IN VARCHAR2 ) RETURN NUMBER
IS
   bin_max_humidity_ WAREHOUSE_BAY_ROW_TAB.bin_max_humidity%TYPE;
BEGIN
   bin_max_humidity_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
BEGIN
   ...
   ELSE
      bin_height_capacity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Height_Cap_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_width_capacity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Width_Cap_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_dept_capacity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Dept_Cap_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_carry_capacity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Carry_Cap_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_min_temperature_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Min_Temp_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_max_temperature_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Max_Temp_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_min_humidity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Min_Humidity_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      bin_max_humidity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Bin_Max_Humidity_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   IF (micro_cache_value_.row_carrying_capacity IS NULL) THEN
      row_carry_capacity_source_db_ := Warehouse_Bay_API.Get_Row_Carry_Cap_Source_Db(contract_, warehouse_id_, bay_id_);
   ELSE
      row_carry_capacity_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Row_Carry_Cap_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
FUNCTION Get_Receipts_Blocked (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   receipts_blocked_ WAREHOUSE_BAY_ROW_TAB.receipts_blocked%TYPE;
BEGIN
   receipts_blocked_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
BEGIN
   ...
   ELSE
      receipts_blocked_source_ := Warehouse_Structure_Level_API.Decode(Warehouse_Structure_Level_API.DB_ROW);
   END IF;
...
FUNCTION Get_Receipt_To_Occup_Blockd (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   receipt_to_occupied_blocked_ WAREHOUSE_BAY_ROW_TAB.receipt_to_occupied_blocked%TYPE;
BEGIN
   receipt_to_occupied_blocked_ := super(contract_, warehouse_id_, bay_id_, row_id_);
   ...
   IF (receipt_to_occupied_blocked_ = Fnd_Boolean_API.db_false) THEN
      receipt_to_occupied_blocked_ := Warehouse_Bay_API.Get_Receipt_To_Occup_Blockd(contract_, warehouse_id_, bay_id_);
   END IF;
...
BEGIN
   ...
   ELSE
      receipt_to_occup_blkd_src_ := Warehouse_Structure_Level_API.Decode(Warehouse_Structure_Level_API.DB_ROW);
   END IF;
...
FUNCTION Get_Hide_In_Whse_Navigator (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   hide_in_whse_navigator_ WAREHOUSE_BAY_ROW_TAB.hide_in_whse_navigator%TYPE;
BEGIN
   hide_in_whse_navigator_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
IS
   hide_in_whse_navigator_ WAREHOUSE_BAY_ROW_TAB.hide_in_whse_navigator%TYPE := Fnd_Boolean_API.db_false;
...
IS
BEGIN
   RETURN (Check_Exist___(contract_, warehouse_id_, bay_id_, row_id_));
END Check_Exist;
...
BEGIN
   ...
   bin_min_temperature_ := Get_Bin_Min_Temperature(contract_, warehouse_id_, bay_id_, row_id_);
   bin_max_temperature_ := Get_Bin_Max_Temperature(contract_, warehouse_id_, bay_id_, row_id_);

   IF (Part_Catalog_Invent_Attrib_API.Incorrect_Temperature_Range(bin_min_temperature_, bin_max_temperature_)) THEN
      Error_SYS.Record_General(lu_name_, 'TEMPRANGE: Incorrect Operative Temperature Range for Row :P1 of Bay :P2 in Warehouse :P3.', row_id_, bay_id_, warehouse_id_);
   END IF;
...
BEGIN
   ...
   bin_min_humidity_ := Get_Bin_Min_Humidity(contract_, warehouse_id_, bay_id_, row_id_);
   bin_max_humidity_ := Get_Bin_Max_Humidity(contract_, warehouse_id_, bay_id_, row_id_);

   IF (Part_Catalog_Invent_Attrib_API.Incorrect_Humidity_Range(bin_min_humidity_, bin_max_humidity_)) THEN
      Error_SYS.Record_General(lu_name_, 'HUMRANGE: Incorrect Operative Humidity Range for Row :P1 of Bay :P2 in Warehouse :P3.', row_id_, bay_id_, warehouse_id_);
   END IF;
...
FUNCTION Get_Mix_Of_Part_Number_Blocked (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   mix_of_part_number_blocked_ WAREHOUSE_BAY_ROW_TAB.mix_of_part_number_blocked%TYPE;
BEGIN
   mix_of_part_number_blocked_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
BEGIN
   ...
   ELSE
      mix_of_part_blocked_source_ := Warehouse_Structure_Level_API.Decode(Warehouse_Structure_Level_API.DB_ROW);
   END IF;
...
FUNCTION Get_Mix_Of_Cond_Codes_Blocked (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   mix_of_cond_codes_blocked_ WAREHOUSE_BAY_ROW_TAB.mix_of_cond_codes_blocked%TYPE;
BEGIN
   mix_of_cond_codes_blocked_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
BEGIN
   ...
   IF (micro_cache_value_.mix_of_cond_codes_blocked = Fnd_Boolean_API.db_false) THEN
      mix_of_cond_blocked_source_ := Warehouse_Bay_API.Get_Mix_Of_Cond_Blocked_Source(contract_, warehouse_id_, bay_id_);
   ELSE
      mix_of_cond_blocked_source_ := Warehouse_Structure_Level_API.Decode(Warehouse_Structure_Level_API.DB_ROW);
   END IF;
...
FUNCTION Get_Mix_Of_Lot_Batch_Blocked (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   mix_of_lot_batch_no_blocked_ WAREHOUSE_BAY_ROW_TAB.mix_of_lot_batch_no_blocked%TYPE;
BEGIN
   mix_of_lot_batch_no_blocked_ := super(contract_, warehouse_id_, bay_id_, row_id_);   
...
BEGIN
   ...
   ELSE
      mix_of_lot_blocked_source_ := Warehouse_Structure_Level_API.Decode(Warehouse_Structure_Level_API.DB_ROW);
   END IF;
...
FUNCTION Get_Exclude_Storage_Req_Val (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   exclude_storage_req_val_ WAREHOUSE_BAY_ROW_TAB.exclude_storage_req_val%TYPE;
BEGIN
   exclude_storage_req_val_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
BEGIN
   ...
   ELSE
      exclude_storage_req_val_src_ := Warehouse_Structure_Level_API.Decode(Warehouse_Structure_Level_API.DB_ROW);
   END IF;
...
BEGIN
   total_carrying_capacity_ := NVL(Get_Row_Carrying_Capacity(contract_,
                                                          warehouse_id_,
                                                          bay_id_,
                                                          row_id_), Inventory_Putaway_Manager_API.positive_infinity_);
...
PROCEDURE Lock_By_Keys_Wait (
   ...
   row_id_       IN VARCHAR2 )
IS
   dummy_ WAREHOUSE_BAY_ROW_TAB%ROWTYPE;
BEGIN

   dummy_ := Lock_By_Keys___(contract_, warehouse_id_, bay_id_, row_id_);
...
FUNCTION Get_Availability_Control_Id (
   ...
   row_id_       IN VARCHAR2 ) RETURN VARCHAR2
IS
   availability_control_id_ WAREHOUSE_BAY_ROW_TAB.availability_control_id%TYPE;
BEGIN
   availability_control_id_ := super(contract_, warehouse_id_, bay_id_, row_id_);
...
IS
BEGIN
   RETURN (Warehouse_Structure_Level_API.Decode(Get_Avail_Control_Id_Source_Db(contract_, warehouse_id_, bay_id_, row_id_)));
...
BEGIN
   ...
   ELSE
      avail_control_id_source_db_ := Warehouse_Structure_Level_API.DB_ROW;
   END IF;
...
@UncheckedAccess
FUNCTION Get_Default_Row_Id RETURN VARCHAR2
IS
BEGIN
   RETURN default_row_id_;
END Get_Default_Row_Id;
