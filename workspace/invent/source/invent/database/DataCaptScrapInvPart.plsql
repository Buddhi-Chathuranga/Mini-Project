-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptScrapInvPart
--  Component:    INVENT
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: SCRAP_INVENTORY_PART
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  211222  BwItlK  Bug 161776 (SCZ-17073), Added check in Validate_Data_Item on data items that could be checked with barcode_id_.
...
--  171102  ChFolk  STRSC-14018, Changed message constants NEGATIVEORZEROQTY in Validate_Data_Item as NEGATIVEORZEROSCRAPQTY and NEGATIVEORZEROINPUTQTY 
--  171102          to distinguish the different message texts.
--  171102  DaZase  STRSC-13069, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
--  171102          of anything found later in that method. Changed size to 4000 on value_ in Execute_Process and detail_value_ in Add_Unique_Data_Item_Detail___.
--  171101  CKumlk  STRSC-13711, Changed size of scrap_note_ to 2000 in Execute_Process.
--  170912  SWiclk  STRSC-11289, Modified Get_Automatic_Data_Item_Value() in order to handle INPUT_UOM  and INPUT_QUANTITY when 
--  170912          inventory part has a value for Input UoM Group.
--  170904  SWiclk  STRSC-11289, Implemented GTIN support, hence handled GTIN, INPUT_UOM and INPUT_QUANTITY data items and INPUT_CONV_FACTOR as feedback item
--  170904          and GTIN_IS_MANDATORY as detail item. Added Get_Input_Uom_Sql_Whr_Exprs___() and added GTIN to Get_Filter_Keys___().
...
--  170330  SWiclk  Bug 134403, Modified Get_Filter_Keys___() in order to fetch predicted value for barcode_id regardless whether the barcode_id usage is mandatory or not.
...
--  151123  Chfose  LIM-5033, Removed pallet_id (NULL) from call to Inventory_Part_In_Stock_API.Scrap_Part.
--  151104  KhVeSe  LIM-4291, Added handling for new data items HANDLING_UNIT_ID, SSCC and ALT_HANDLING_UNIT_LABEL_ID.
--  151104          Also added calls to Data_Capture_Invent_Util_API.Add_Details_For_Hand_Unit_Type, 
--  151104          Data_Capture_Invent_Util_API.Add_Details_For_Handling_Unit in Add_Details_For_Latest_Item.
...
--  150728  ShKolk  Bug 123092, Modified Print_Report___() to set COPIES option to support StreamServe reports.
--  150622  LEPESE  LIM-3106, removed method Print_Report___ and added print_serviceability_tag_ as parameter to Inventory_Part_In_Stock_API.Scrap_Part.
--  150413  Chfose  LIM-979, Fixed calls to Inventory_Part_In_Stock_API by including handling_unit_id where applicable.
--  141117  DaZase  Some refactoring in some methods.
--  140728  BudKlk  Created.
-----------------------------------------------------------------------------
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      -- Pass the serial_no as '*' if the in inventory is not checked.
...
BEGIN
   sql_where_expression_  := ' AND freeze_flag_db = ''N''
                               ...
                               AND location_type_db IN (''PICKING'',''F'',''SHIPMENT'',''MANUFACTURING'') ';
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      Data_Capture_Invent_Util_API.Validate_Data_Item(capture_session_id_,
                                                   data_item_id_,
                                                   data_item_value_);
      IF (data_item_id_ IN ('SCRAPPING_CAUSE', 'QUANTITY_TO_SCRAP')) THEN
      ...
      END IF;
       ...
       ELSE 
         ...
         IF (data_item_id_ IN ('PART_NO', 'LOCATION_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'ENG_CHG_LEVEL', 'WAIV_DEV_REJ_NO',
                               'ACTIVITY_SEQ', 'HANDLING_UNIT_ID', 'SSCC','ALT_HANDLING_UNIT_LABEL_ID')) THEN
            IF (barcode_id_ IS NOT NULL AND data_item_id_ IN ('PART_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'ENG_CHG_LEVEL', 'WAIV_DEV_REJ_NO', 'ACTIVITY_SEQ')) THEN
               Inventory_Part_Barcode_API.Record_With_Column_Value_Exist(barcode_id_         => barcode_id_,
                                                                         contract_           => contract_,
                                                                         part_no_            => part_no_,
                                                                         configuration_id_   => configuration_id_,
                                                                         lot_batch_no_       => lot_batch_no_,
         ...
         ELSIF (data_item_id_ = 'BARCODE_ID') THEN            
            IF (data_item_value_ IS NOT NULL) THEN
               -- We need a complete set of filter keys fetched with unique handling so we can find a possible unique barcode and filter it correctly
               Inventory_Part_Barcode_API.Record_With_Column_Value_Exist(barcode_id_         => data_item_value_,
                                                                         contract_           => contract_,
                                                                         part_no_            => part_no_,
                                                                         configuration_id_   => configuration_id_,
                                                                         lot_batch_no_       => lot_batch_no_,
                                                                         ...
                                                                         column_description_ => data_item_description_);
            END IF;
         ELSIF (data_item_id_ IN ('QUANTITY_TO_SCRAP' )) THEN
            scrap_qty_ := data_item_value_;
            IF (Part_Catalog_API.Get_Rcpt_Issue_Serial_Track_Db(part_no_) = Fnd_Boolean_API.DB_TRUE ) THEN
               IF (scrap_qty_>1) THEN 
                  Error_SYS.Record_General(lu_name_, 'WRONGSCRAPQTY: Quantity to be scrapped cannot be more than one for the serial parts.');
               END IF;
            END IF;
            IF (scrap_qty_ <= 0) THEN
               Error_SYS.Record_General(lu_name_, 'NEGATIVEORZEROSCRAPQTY: Zero or negative scrap quantity is not allowed.');
            END IF;
            ...
            IF (qty_onhand_ IS NOT NULL AND qty_reserved_ IS NOT NULL ) THEN
               ...
               IF (qty_available_ <= 0 ) THEN
                  Error_SYS.Record_General(lu_name_, 'NOQTYONHAND: There is no quantity available at the location :P1 for the part :P2.',location_no_,part_no_);
               ELSIF (scrap_qty_ > qty_available_) THEN
                  Error_SYS.Record_General(lu_name_, 'TOMUCHQTYSCRAP: Quantity scrapped cannot be greater than the quantity available.');
               END IF;
            END IF;
         ELSIF (data_item_id_ = 'INPUT_QUANTITY') THEN
            IF (data_item_value_ <= 0) THEN
               Error_SYS.Record_General(lu_name_, 'NEGATIVEORZEROINPUTQTY: Zero or negative input quantity is not allowed.');              
            END IF;   
         ...
         ELSIF (data_item_id_ = 'SERIAL_NO') THEN
            IF (Part_Catalog_API.Get_Rcpt_Issue_Serial_Track_Db(part_no_) = Fnd_Boolean_API.DB_TRUE) THEN 
               IF (Part_Catalog_API.Get_Serial_Tracking_Code_Db(part_no_) = Part_Serial_Tracking_API.DB_SERIAL_TRACKING) THEN
                  IF (barcode_id_ IS NOT NULL) THEN
                     -- BARCODE_ID is used for this item, then validate it against the barcode table
                     Inventory_Part_Barcode_API.Record_With_Column_Value_Exist(barcode_id_         => barcode_id_,
                                                                               contract_           => contract_,
                                                                               part_no_            => part_no_,
                                                                               configuration_id_   => configuration_id_,
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ = 'SCRAPPING_CAUSE') THEN
      ...
      ELSE
         ...
         ELSIF (data_item_id_ IN ('PART_NO', 'LOCATION_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'SERIAL_NO', 'ENG_CHG_LEVEL', 'WAIV_DEV_REJ_NO', 
                                  'ACTIVITY_SEQ', 'HANDLING_UNIT_ID', 'SSCC','ALT_HANDLING_UNIT_LABEL_ID')) THEN
...
BEGIN
   IF no_of_records_handled_ = 1 THEN
      message_ :=  Language_SYS.Translate_Constant(lu_name_, 'SCRAPPART: The inventory part was scrapped.');
   ELSE
      message_ := Language_SYS.Translate_Constant(lu_name_, 'SCRAPPARTS: :P1 inventory parts were scrapped.',NULL, no_of_records_handled_);
   END IF;
   RETURN message_;
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN   
      ...
      IF (automatic_value_ IS NULL) THEN
         ...
         IF (data_item_id_ NOT IN ('SCRAPPING_CAUSE', 'PRINT_SERVICEABILITY_TAG')) THEN
            session_rec_ := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
            IF (data_item_id_ = 'BARCODE_ID') THEN
               -- We need a complete set of filter keys fetched with unique handling so we can find a possible unique barcode and filter it correctly
               Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, 
            ...
            ELSIF (barcode_id_ IS NOT NULL AND  
               ...
               END IF;
            ELSIF (data_item_id_ IN ('PART_NO', 'LOCATION_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'ENG_CHG_LEVEL', 
                                     'WAIV_DEV_REJ_NO', 'ACTIVITY_SEQ', 'SERIAL_NO', 'HANDLING_UNIT_ID', 'SSCC','ALT_HANDLING_UNIT_LABEL_ID')) THEN
            ...
            ELSIF (data_item_id_= 'GTIN') THEN        
               ...
               IF (part_no_ IS NOT NULL AND automatic_value_ IS NULL) THEN            
                  automatic_value_ := 'NULL';                     
               END IF;
            ELSIF (data_item_id_ = 'QUANTITY_TO_SCRAP') THEN   
               IF (Part_Catalog_API.Get_Rcpt_Issue_Serial_Track_Db(part_no_) = Fnd_Boolean_API.DB_TRUE) THEN
                  automatic_value_ := '1'; 
               ELSE
                  input_uom_ := Data_Capture_Session_API.Get_Latest_A_If_Before_B(capture_session_id_ => capture_session_id_, 
         ...
         FOR i IN conf_item_detail_tab_.FIRST..conf_item_detail_tab_.LAST LOOP
            ...
            ELSE  -- FEEDBACK ITEMS AS DETAILS
               IF (conf_item_detail_tab_(i).data_item_detail_id IN ('WAREHOUSE_ID', 'BAY_ID', 'TIER_ID', 'ROW_ID','BIN_ID', 'RECEIPTS_BLOCKED',
                                                                    'LOCATION_NO_DESC', 'LOCATION_TYPE')) THEN
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('PART_DESCRIPTION', 'CATCH_UNIT_MEAS', 'CATCH_UNIT_MEAS_DESCRIPTION', 'UNIT_MEAS', 
                                                                       ...
                                                                       'ACCOUNTING_GROUP_DESCRIPTION', 'PRODUCT_CODE', 'PRODUCT_CODE_DESCRIPTION',  
                                                                       'PRODUCT_FAMILY', 'PRODUCT_FAMILY_DESCRIPTION', 'SERIAL_TRACKING_RECEIPT_ISSUE',
                                                                       'SERIAL_TRACKING_INVENTORY', 'LOT_BATCH_TRACKING', 'ACCOUNTING_GROUP',
                                                                       'GTIN_IDENTIFICATION', 'GTIN_DEFAULT', 'INPUT_CONV_FACTOR')) THEN
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('QTY_ONHAND_NOT_RESERVED', 'CATCH_QTY_ONHAND_NOT_RESERVED', 'AVAILABILITY_CONTROL_ID', 
                                                                       'AVAILABILITY_CONTROL_DESCRIPTION', 'EXPIRATION_DATE')) THEN
...
BEGIN
   ...
   WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
      ...
      ELSIF (name_ = 'QUANTITY_TO_SCRAP') THEN
         scrap_qty_ := value_;
      ELSIF (name_ = 'CATCH_QUANTITY') THEN
         scrap_catch_qty_ := value_;
      ELSIF (name_ = 'SCRAPPING_CAUSE') THEN
         scrap_cause_ := value_;
      ELSIF (name_ = 'SCRAP_NOTE') THEN
         scrap_note_ := value_;
      ELSIF (name_ = 'PRINT_SERVICEABILITY_TAG') THEN
   ...
   -- Create serial tracking session for receipt and issue serial tracked part that is not tracked in inventory
   IF (Part_Catalog_API.Get_Rcpt_Issue_Serial_Track_Db(part_no_) = Fnd_Boolean_API.DB_TRUE AND serial_no_ != '*' AND
      Part_Catalog_API.Get_Serial_Tracking_Code_Db(part_no_) = Part_Serial_Tracking_API.DB_NOT_SERIAL_TRACKING) THEN
      part_tracking_session_id_ := Temporary_Part_Tracking_API.Get_Next_Session_Id();
      Temporary_Part_Tracking_API.New(part_tracking_session_id_, serial_no_, scrap_catch_qty_, contract_, part_no_, 'OTHER');
      -- Set serial no to '*' to reduce the scrap_qty_ from the part_no_.
      serial_no_ := '*';
   END IF;
   
   Inventory_Part_In_Stock_API.Scrap_Part(catch_quantity_              => scrap_catch_qty_,
                                          contract_                    => contract_,
                                          part_no_                     => part_no_,
                                          configuration_id_            => configuration_id_,
                                          location_no_                 => location_no_,
...
BEGIN
   -- Check to see that API method Inventory_Part_In_Stock_API.Scrap_Part is granted thru following projections/actions
   IF (Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseHandling', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseTierHandling', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('WarehouseNavigatorHandling', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('ScrapInventoryPart', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseRowHandling', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseLocHandling', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('InventoryPartInStockHandling', 'ScrapPart') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseBayHandling', 'ScrapPart')) THEN
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ = 'QUANTITY_TO_SCRAP') THEN       
