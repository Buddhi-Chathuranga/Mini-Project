-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptAttachPartHu
--  Component:    INVENT
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: ATTACH_PART_TO_HANDLING_UNIT
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  200813  DaZase  SC2020R1-7510, Replaced Is_Method_Available with Is_Proj_Action_Available in Is_Process_Available to support new projection security concept.
--  180904  ErRalk  Bug 143631, Modified Get_Automatic_Data_Item_Value method to fetch available quantity as an automatic value to 'QUANTITY' data item id.  
...
--  171017  DaZase  STRSC-12997, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
...
--  170914  SURBLK  STRSC-11275, Implemented GTIN support, Adding GTIN, INPUT_UOM and INPUT_QUANTITY data items and INPUT_CONV_FACTOR.
--  170512  DaZase  STRSC-7734, Added automatic handling of CATCH_QUANTITY so stock CATCH_QTY_ONHAND will be fetched if onhand quantity is the same as the attach quantity.
...
--  170407  LEPESE  LIM-11349, Removed fetching of top parent HU ID before fetching location no for handling unit since now location_no is added to complete structure.
--  170224  SWiclk  Bug 134403, Modified Get_Filter_Keys___() in order to fetch predicted value for barcode_id regardless whether the barcode_id usage is mandatory or not.
--  170217  DaZase  Changes in Get_Sql_Hu_Where_Expression___ and Create_List_Of_Values due to unique Handling_Unit_API
...
--  161221  SWiclk  LIM-9700, Added condition code as a data item. Modified Get_Sql_Stock_Where_Expr___() by adding capture_session_id_ and data_item_id_
--  161221          as parameters in order to in order to handle CONDITION_CODE. Added capture_session_id_ as a parameter to Get_Unique_Data_Item_Value___().
--  161128  SWiclk  LIM-9831, Modified Get_Filter_Keys___() in order to consider the top parent handling unit id when fetching the location_no.
--  161125  SWiclk  LIM-9830, Modified Get_Filter_Keys___(), Create_List_Of_Values(), Validate_Data_Item() and Execute_Process()
--  161125          in order to support serial parts that are not inventory tracked.
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      ELSIF (wanted_data_item_id_ IN ('HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID')) THEN
         unique_value_ := Handling_Unit_API.Get_Column_Value_If_Unique(no_unique_value_found_      => dummy_,
                                                                       ...
                                                                       column_name_                => wanted_data_item_id_,
                                                                       sql_where_expression_       => Get_Sql_Hu_Where_Expression___(contract_, location_no_));
      ELSE
         unique_value_ := Inventory_Part_In_Stock_API.Get_Column_Value_If_Unique(no_unique_value_found_      => dummy_,
                                                                                 ...
                                                                                 column_name_                => wanted_data_item_id_,
                                                                                 sql_where_expression_       => Get_Sql_Stock_Where_Expr___(capture_session_id_,
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      END IF;
      IF (location_no_ IS NULL AND handling_unit_id_ IS NOT NULL) THEN
         location_no_ := Handling_Unit_API.Get_Location_No(handling_unit_id_);
      END IF;

      -- if alt_handling_unit_label_id_/sscc comes after current data item, we exchange the parameter with % since this column in the view can be NULL 
      -- so we need to specifiy that we have to compare to all alternative handling unit label ids/ssccs in the table
      ...
      IF (Part_Catalog_API.Serial_Trak_Only_Rece_Issue_Db(part_no_) = Fnd_Boolean_API.DB_TRUE) THEN
         serial_no_ := NULL;
      END IF;  
      ...
      IF use_unique_values_ THEN
         ...
         IF (part_no_ IS NULL) THEN
            part_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'PART_NO', capture_session_id_);
         END IF;
         IF (configuration_id_ IS NULL) THEN
            configuration_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'CONFIGURATION_ID', capture_session_id_);
         END IF;
         IF (lot_batch_no_ IS NULL) THEN
            lot_batch_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'LOT_BATCH_NO', capture_session_id_);
         END IF;
         IF (serial_no_ IS NULL) THEN
            serial_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'SERIAL_NO', capture_session_id_);
         END IF;
         IF (eng_chg_level_ IS NULL) THEN
            eng_chg_level_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'ENG_CHG_LEVEL', capture_session_id_);
         END IF;
         IF (waiv_dev_rej_no_ IS NULL) THEN
            waiv_dev_rej_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'WAIV_DEV_REJ_NO', capture_session_id_);
         END IF;
         IF (activity_seq_ IS NULL) THEN
            activity_seq_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'ACTIVITY_SEQ', capture_session_id_);
         END IF;
         IF (location_no_ IS NULL) THEN
            location_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'LOCATION_NO', capture_session_id_);
         END IF;
         IF (handling_unit_id_ IS NULL) THEN
            handling_unit_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'HANDLING_UNIT_ID', capture_session_id_);
         END IF;
         IF (alt_handling_unit_label_id_ IS NULL) THEN
            alt_handling_unit_label_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'ALT_HANDLING_UNIT_LABEL_ID', capture_session_id_);
         END IF;
         IF (barcode_id_ IS NULL AND Data_Capture_Invent_Util_API.Inventory_Barcode_Enabled(session_rec_.capture_process_id, session_rec_.capture_config_id)) THEN
            barcode_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'BARCODE_ID', capture_session_id_);
         END IF;
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      /*IF (detail_value_ IS NULL AND data_item_detail_id_ IN ()) THEN
         detail_value_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, data_item_detail_id_);
      END IF;*/
...
-- Using similar where statement as the IEE client tbwHUInvPartInStockUnattached. That client is also adding 
-- contract/location_no where statements depending on if they have values on the parent client but here this 
-- will already been solved here depending in which order the filter keys comes. 
FUNCTION Get_Sql_Stock_Where_Expr___ (
   capture_session_id_          IN     NUMBER,
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      sql_where_expression_ := ' AND HANDLING_UNIT_ID = 0 AND (QTY_ONHAND - QTY_RESERVED) > 0 AND LOCATION_TYPE_DB NOT IN (''ARRIVAL'', ''QA'') ';
      session_rec_     := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
      process_package_ := Data_Capture_Process_API.Get_Process_Package(session_rec_.capture_process_id);
      condition_code_  := Data_Capture_Session_API.Get_Predicted_Data_Item_Value(capture_session_id_      => capture_session_id_,
                                                                                 current_data_item_id_    => data_item_id_,
                                                                                 current_data_item_value_ => NULL,
                                                                                 wanted_data_item_id_     => 'CONDITION_CODE',
                                                                                 session_rec_             => session_rec_,
                                                                                 process_package_         => process_package_); 
      IF ((condition_code_ IS NULL) AND NOT Data_Capt_Conf_Data_Item_API.Is_A_Before_B(session_rec_.capture_process_id, session_rec_.capture_config_id, 'CONDITION_CODE', data_item_id_)) THEN 
         condition_code_ := '%';
      END IF;
      sql_where_expression_ := sql_where_expression_ || ' AND (CONDITION_CODE = ''' || condition_code_ || ''' OR (CONDITION_CODE IS NULL AND ''' || condition_code_ || ''' IS NULL)  OR ''' || condition_code_ || ''' = ''%'') ';                      
...
END Get_Sql_Stock_Where_Expr___;


FUNCTION Get_Sql_Hu_Where_Expression___ (
   contract_     VARCHAR2,
   location_no_  VARCHAR2 ) RETURN VARCHAR2
IS
   sql_where_expression_         VARCHAR2(1000);
BEGIN
   IF (location_no_ IS NOT NULL) THEN
      sql_where_expression_ := ' AND Handling_Unit_API.Is_In_Inventory_Transit(handling_unit_id) = ''FALSE'' AND SHIPMENT_ID IS NULL AND (CONTRACT IS NULL OR (CONTRACT = ''' || contract_ || ''' AND LOCATION_NO = ''' || location_no_ || ''')) ';
   ELSE
      sql_where_expression_ := ' AND Handling_Unit_API.Is_In_Inventory_Transit(handling_unit_id) = ''FALSE'' AND SHIPMENT_ID IS NULL AND (CONTRACT IS NULL OR CONTRACT = ''' || contract_ || ''') ';
   END IF;
   RETURN sql_where_expression_;
END Get_Sql_Hu_Where_Expression___;
...
IS
   ...
   session_rec_                  Data_Capture_Common_Util_API.Session_Rec;
   process_package_              VARCHAR2(30);
   handling_unit_location_no_    VARCHAR2(35);
   hu_location_description_      VARCHAR2(50) := Language_SYS.Translate_Constant(lu_name_, 'HULOCATION: Handling Unit Location');  
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      -- Calling Invent Util for some validations (especially for BARCODE_ID)
      Data_Capture_Invent_Util_API.Validate_Data_Item(capture_session_id_, data_item_id_, data_item_value_);
      session_rec_     := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
      process_package_ := Data_Capture_Process_API.Get_Process_Package(session_rec_.capture_process_id);
      IF (data_item_id_ IN ('BARCODE_ID', 'HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID')) THEN
         ...
         IF ((data_item_id_ = 'BARCODE_ID' AND data_item_value_ IS NOT NULL) OR (data_item_id_ IN ('HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID'))) THEN 
            ...
            -- The same for the handling unit keys since if we get a unique location that could narrow down the filtering of these hu keys.
            Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, 
                               waiv_dev_rej_no_, activity_seq_, barcode_id_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, gtin_no_,
      ...
      IF (data_item_id_ = 'SERIAL_NO') AND (Part_Catalog_API.Serial_Trak_Only_Rece_Issue_Db(part_no_) = Fnd_Boolean_API.DB_TRUE) THEN
      ...
      ELSIF (data_item_id_ IN ('PART_NO', 'LOCATION_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'SERIAL_NO', 'ENG_CHG_LEVEL', 
                            'WAIV_DEV_REJ_NO', 'ACTIVITY_SEQ', 'CONDITION_CODE')) THEN
         ...
         ELSE
            IF (data_item_id_ IN ('CONDITION_CODE')) THEN
      ...
      ELSIF (data_item_id_ = 'BARCODE_ID') THEN 
         -- This will validate barcode if the detail barcode_id_is_mandatory is set even if value is NULL here
         Data_Capture_Invent_Util_API.Validate_Data_Item(capture_session_id_, data_item_id_, data_item_value_);
      ...
      ELSIF (data_item_id_ IN ('HANDLING_UNIT_ID', 'SSCC','ALT_HANDLING_UNIT_LABEL_ID')) THEN
         ...
         handling_unit_location_no_ := Handling_Unit_API.Get_Location_No(handling_unit_id_);
         IF (handling_unit_location_no_ IS NOT NULL AND (part_no_ IS NOT NULL OR 
                                                         configuration_id_ IS NOT NULL OR
                                                         lot_batch_no_ IS NOT NULL OR
                                                         serial_no_ IS NOT NULL OR
                                                         eng_chg_level_ IS NOT NULL OR
                                                         waiv_dev_rej_no_ IS NOT NULL OR
                                                         activity_seq_ IS NOT NULL)) THEN
            -- If some of the stock keys already have been scanned before check if this handling unit location will match the stock keys.
            -- This is the easiest way to handle this, another approach would be to join hu and stock view together so only locations filtered 
            -- from the stock items would be visible together with all empty HUs for the LOV/Automatic handling but that would be harder and 
            -- possible not so good perfomance wise since both these tables are very large and doing this for data items that will be 
            -- scanned 99% of the time in the real world feels unnessary.
            Inventory_Part_In_Stock_API.Record_With_Column_Value_Exist(contract_                   => contract_,
                                                                       part_no_                    => part_no_,
                                                                       configuration_id_           => configuration_id_,
                                                                       location_no_                => NULL,
                                                                       ...
                                                                       column_value_               => handling_unit_location_no_,
                                                                       column_description_         => hu_location_description_,
                                                                       sql_where_expression_       => Get_Sql_Stock_Where_Expr___(capture_session_id_,
                                                                                                                                  data_item_id_));
         END IF;
      ...
      ELSIF (data_item_id_ IN ('QUANTITY')) THEN
         IF (data_item_value_ <= 0) THEN
            Error_SYS.Record_General(lu_name_, 'NEGATIVEORZEROQTY: The quantity must be greater then 0.');
         END IF;
         ...
         IF (qty_onhand_ IS NOT NULL AND qty_reserved_ IS NOT NULL ) THEN
            ...
            IF (data_item_value_ IS NOT NULL AND data_item_value_ > qty_available_) THEN
               Error_SYS.Record_General(lu_name_, 'TOMUCHQTYTOATTACH: Maximum available stock record quantity to be attached is :P1.', Data_Capture_Common_Util_API.Get_Leading_Zero_For_Decimals(qty_available_));
            END IF;
         END IF;
...
IS
   ...
   sql_where_expression_         VARCHAR2(500);   
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ IN ('BARCODE_ID', 'HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID')) THEN
         -- We need a complete set of filter keys fetched with unique handling so we can find a possible unique barcode and filter it correctly
         -- The same for the handling unit keys since if we get a unique location that could narrow down the filtering of these hu keys.
         Get_Filter_Keys___(dummy_contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                            eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, handling_unit_id_, sscc_, 
                            alt_handling_unit_label_id_, gtin_no_, capture_session_id_, 
                            data_item_id_, data_item_value_ => NULL, use_unique_values_ => TRUE);
      ELSE
         Get_Filter_Keys___(dummy_contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                            eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, handling_unit_id_, sscc_, 
                            alt_handling_unit_label_id_, gtin_no_, capture_session_id_, data_item_id_);
      END IF;
      ...
      IF ((data_item_id_ = 'BARCODE_ID') OR
         (barcode_id_ IS NOT NULL AND data_item_id_ IN ('PART_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'SERIAL_NO', 
                                                        'ENG_CHG_LEVEL', 'WAIV_DEV_REJ_NO', 'ACTIVITY_SEQ'))) THEN
      ...
      ELSIF (data_item_id_ = 'SERIAL_NO' AND Part_Catalog_API.Serial_Trak_Only_Rece_Issue_Db(part_no_) = Fnd_Boolean_API.DB_TRUE) THEN
      ...
      ELSIF (data_item_id_ IN ('PART_NO', 'CONFIGURATION_ID', 'LOCATION_NO', 'LOT_BATCH_NO', 'SERIAL_NO', 
                               'ENG_CHG_LEVEL', 'WAIV_DEV_REJ_NO', 'ACTIVITY_SEQ', 'CONDITION_CODE')) THEN
      ...
      ELSIF (data_item_id_ IN ('HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID')) THEN
         IF (location_no_ IS NOT NULL) THEN
            -- We got a unique location from earlier scanned items so we can use that to filter down the first LOV a bit
            sql_where_expression_ := ' AND Handling_Unit_API.Is_In_Inventory_Transit(handling_unit_id) = ''FALSE'' AND SHIPMENT_ID IS NULL AND CONTRACT = ''' || contract_ || ''' AND LOCATION_NO = ''' || location_no_ || ''' ';
         ELSE
            sql_where_expression_ := ' AND Handling_Unit_API.Is_In_Inventory_Transit(handling_unit_id) = ''FALSE'' AND SHIPMENT_ID IS NULL AND CONTRACT = ''' || contract_ || ''' ';
         END IF;
         -- To make it a bit easer to use the LOV for these items we will first add handling units connected to current site and 
         -- then add handlings units not connected to any site (empty HU's), this is more of test help since in a real case 
         -- these items should always be scanned
         ...
         Handling_Unit_API.Create_Data_Capture_Lov(handling_unit_id_           => handling_unit_id_,
                                                   ...
                                                   column_name_                => data_item_id_,
                                                   lov_type_db_                => lov_type_db_,
                                                   sql_where_expression_       => ' AND Handling_Unit_API.Is_In_Inventory_Transit(handling_unit_id) = ''FALSE'' AND SHIPMENT_ID IS NULL AND CONTRACT IS NULL ');
      ELSIF (data_item_id_ IN ('INPUT_UOM')) THEN  
         gtin_no_ := Data_Capture_Session_API.Get_Latest_A_If_Before_B(capture_session_id_ => capture_session_id_,
                                                                       data_item_id_a_     => 'GTIN',
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (automatic_value_ IS NULL) THEN
         ...
         IF (data_item_id_ IN ('BARCODE_ID', 'HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID')) THEN
            -- We need a complete set of filter keys fetched with unique handling so we can find a possible unique barcode and filter it correctly
            -- The same for the handling unit keys since if we get a unique location that could narrow down the filtering of these hu keys.
            Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                               eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, handling_unit_id_, sscc_, 
                               alt_handling_unit_label_id_, gtin_no_, capture_session_id_, data_item_id_, 
         ...
         ELSIF ((data_item_id_ = 'CATCH_QUANTITY')) THEN
            IF (Fnd_Boolean_API.DB_FALSE = Part_Catalog_API.Get_Catch_Unit_Enabled_Db(part_no_)) THEN
               -- if part is not catch unit enabled catch_quantity should be null
               automatic_value_ := 'NULL';
            ELSIF (Fnd_Boolean_API.DB_TRUE = Part_Catalog_API.Get_Catch_Unit_Enabled_Db(part_no_)) THEN
               quantity_ := Data_Capture_Session_API.Get_Latest_A_If_Before_B(capture_session_id_, 'QUANTITY', 'CATCH_QUANTITY');
               stock_qty_onhand_ := Inventory_Part_In_Stock_API.Get_Column_Value_If_Unique(no_unique_value_found_      => dummy_,
                                                                                           contract_                   => contract_,
                                                                                           part_no_                    => part_no_,
                                                                                           configuration_id_           => configuration_id_,
                                                                                           location_no_                => location_no_,                                                                                 
                                                                                           ...
                                                                                           sql_where_expression_       => Get_Sql_Stock_Where_Expr___(capture_session_id_,
                                                                                                                                                       data_item_id_));
               -- If quantity = stock onhand quantity then return the whole catch quantity onhand as automatic value for catch quantity
               IF (quantity_ = stock_qty_onhand_) THEN
                  automatic_value_ := Inventory_Part_In_Stock_API.Get_Column_Value_If_Unique(no_unique_value_found_      => dummy_,
                                                                                             contract_                   => contract_,
                                                                                             part_no_                    => part_no_,
                                                                                             configuration_id_           => configuration_id_,
         ...
         ELSIF data_item_id_= 'QUANTITY' THEN
            ...
            ELSE 
               ...
               qty_reserved_ := Inventory_Part_In_Stock_API.Get_Column_Value_If_Unique(no_unique_value_found_                => dummy_,
                                                                                                 ...
                                                                                                 sql_where_expression_       => Get_Sql_Stock_Where_Expr___(capture_session_id_,
                                                                                                                                                             data_item_id_));                                            
               automatic_value_ :=  stock_qty_onhand_ - qty_reserved_ ;
            END IF;                
         ELSIF (data_item_id_= 'INPUT_UOM') THEN
            input_uom_group_id_ := Inventory_Part_API.Get_Input_Unit_Meas_Group_Id(contract_, part_no_);
...
BEGIN
   -- Check to see that API method Inventory_Part_In_Stock_API.Change_Handling_Unit_Id is granted thru any of following projections/actions
   IF (Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseHandling', 'ExecuteAttachToHandlingUnit') OR
       ...
       Security_SYS.Is_Proj_Action_Available('ReceiptHandling', 'ExecuteAttachToHandlingUnit') OR 
       Security_SYS.Is_Proj_Action_Available('HandlingUnitHandling', 'ConnectToHandlingUnit')) THEN
      process_available_ := Fnd_Boolean_API.DB_TRUE;
   ELSE
      process_available_ := Fnd_Boolean_API.DB_FALSE;
   END IF;
   -- Note: that Inventory_Part_In_Stock_API.Change_Handling_Unit_Id also is used by other projections that are connected to 
   -- repack and unattach functions but no need checking those for the attach process
...
BEGIN
   IF no_of_records_handled_ = 1 THEN
      message_ :=  Language_SYS.Translate_Constant(lu_name_, 'ATTACHOK: The stock record was attached to handling unit :P1.', NULL, process_message_);
   ELSE
      message_ := Language_SYS.Translate_Constant(lu_name_, 'ATTACHESOK: :P1 stock records was attached to handling unit(s).', NULL, no_of_records_handled_);
   END IF;
   RETURN message_;
...
IS
   ...
   activity_seq_                 NUMBER;
   handling_unit_id_             NUMBER; 
   quantity_to_attach_           NUMBER;
   catch_quantity_to_attach_     NUMBER;
   session_id_                   NUMBER;
   clob_out_data_                CLOB;
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
         ...
         ELSIF (name_ = 'QUANTITY') THEN
            quantity_to_attach_ := value_;
         ELSIF (name_ = 'CATCH_QUANTITY') THEN
            catch_quantity_to_attach_ := value_;
         END IF;
      END LOOP;

      clob_out_data_ := Message_SYS.Construct('OUTPUT_DATA');
      Message_SYS.Add_Attribute(clob_out_data_, 'CONTRACT' , contract_);
      Message_SYS.Add_Attribute(clob_out_data_, 'LOCATION_NO' , location_no_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'PART_NO' , part_no_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'CONFIGURATION_ID' , configuration_id_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'LOT_BATCH_NO' , lot_batch_no_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'ENG_CHG_LEVEL' , eng_chg_level_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'WAIV_DEV_REJ_NO' , waiv_dev_rej_no_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'ACTIVITY_SEQ' , activity_seq_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'SERIAL_NO' , serial_no_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'HANDLING_UNIT_ID' , 0); 
      Message_SYS.Add_Attribute(clob_out_data_, 'QUANTITY' , quantity_to_attach_); 
      Message_SYS.Add_Attribute(clob_out_data_, 'CATCH_QUANTITY' , catch_quantity_to_attach_);
      ...
      IF (Part_Catalog_API.Serial_Trak_Only_Rece_Issue_Db(part_no_) = Fnd_Boolean_API.DB_TRUE) THEN
         ...
         Message_SYS.Add_Attribute(clob_out_data_, 'PART_TRACKING_SESSION_ID' , session_id_);
      END IF;

      Inventory_Part_In_Stock_API.Change_Handling_Unit_Id(clob_out_data_, handling_unit_id_);
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (part_no_ IS NULL) THEN
         Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                            eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, handling_unit_id_, sscc_, 
                            alt_handling_unit_label_id_, gtin_no_, capture_session_id_, data_item_id_, use_applicable_ => FALSE);
         part_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, barcode_id_, 'PART_NO', capture_session_id_);
      END IF;
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
     ...
     -- fetch all necessary keys for all possible detail items below
      session_rec_          := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
      Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                         eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, handling_unit_id_, 
                         sscc_, alt_handling_unit_label_id_, gtin_no_, capture_session_id_, latest_data_item_id_, 
      ...
      IF (conf_item_detail_tab_.COUNT > 0) THEN
         FOR i IN conf_item_detail_tab_.FIRST..conf_item_detail_tab_.LAST LOOP
            ...
            ELSE  -- FEEDBACK ITEMS AS DETAILS
               IF (conf_item_detail_tab_(i).data_item_detail_id IN ('WAREHOUSE_ID', 'BAY_ID', 'TIER_ID', 'ROW_ID','BIN_ID', 'LOCATION_TYPE',
                                                                    'MIX_OF_PART_NUMBER_BLOCKED', 'MIX_OF_CONDITION_CODES_BLOCKED', 
                                                                    'MIX_OF_LOT_BATCH_NO_BLOCKED', 'LOCATION_NO_DESC')) THEN
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('CATCH_UNIT_MEAS', 'PART_DESCRIPTION', 'UNIT_MEAS', 'INPUT_CONV_FACTOR',
                                                                       'GTIN_IDENTIFICATION', 'GTIN_DEFAULT')) THEN
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('AVAILABILTY_CONTROL_ID', 'AVAILABILITY_CONTROL_DESCRIPTION', 'CATCH_QUANTITY_IN_TRANSIT',
                                                                       'CATCH_QUANTITY_ONHAND', 'EXPIRATION_DATE', 'FREEZE_FLAG', 'LAST_ACTIVITY_DATE', 
                                                                       'LAST_COUNT_DATE', 'OWNER', 'OWNER_NAME', 'OWNERSHIP', 'QUANTITY_IN_TRANSIT',
                                                                       'QUANTITY_ONHAND', 'QUANTITY_RESERVED', 'ROTABLE_PART_POOL_ID', 
                  ...
                  Data_Capture_Invent_Util_API.Add_Details_For_Inv_Stock_Rec(capture_session_id_  => capture_session_id_,
                                                                             ...
                                                                             waiv_dev_rej_no_     => waiv_dev_rej_no_,
                                                                             activity_seq_        => activity_seq_,
                                                                             handling_unit_id_    => 0);       -- only unattached stock records
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('PROJECT_ID', 'PROJECT_NAME', 'PROGRAM_ID', 'PROGRAM_DESCRIPTION', 'SUB_PROJECT_ID',
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('PARENT_HANDLING_UNIT_DESC', 'HANDLING_UNIT_TYPE_ADD_VOLUME', 
                                                                       'HANDLING_UNIT_TYPE_CATEG_ID', 'HANDLING_UNIT_TYPE_CATEG_DESC',
                                                                       'HANDLING_UNIT_TYPE_ID', 'HANDLING_UNIT_TYPE_DESC', 
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('HANDLING_UNIT_ACCESSORY_EXIST', 'HANDLING_UNIT_COMPOSITION', 'HANDLING_UNIT_DEPTH',
                                                                       ...
                                                                       'HANDLING_UNIT_MANUAL_VOLUME', 'HANDLING_UNIT_MIX_OF_PART_NO_BLOCKED', 
                                                                       ...
                                                                       'HANDLING_UNIT_UOM_VOLUME', 'HANDLING_UNIT_WIDTH', 'PARENT_HANDLING_UNIT_ID',
                                                                       'PARENT_SSCC', 'PARENT_ALT_HANDLING_UNIT_LABEL_ID', 'TOP_PARENT_HANDLING_UNIT_ID',
                                                                       'TOP_PARENT_SSCC', 'TOP_PARENT_ALT_HANDLING_UNIT_LABEL_ID', 'TOP_PARENT_HANDLING_UNIT_TYPE_ID', 
                                                                       'TOP_PARENT_HANDLING_UNIT_TYPE_DESC',
