-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptUnattachPartHu
--  Component:    INVENT
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: UNATTACH_PART_FROM_HANDL_UNIT
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
...
--  171103  DaZase  STRSC-13079, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
--  171103          of anything found later in that method. Changed size to 4000 on value_ in Execute_Process and detail_value_ in Add_Unique_Data_Item_Detail___.
--  171002  SURBLK  STRSC-11290, Implemented GTIN support, Adding GTIN, INPUT_UOM and INPUT_QUANTITY data items 
--  171002          and INPUT_CONV_FACTOR as feedback item and GTIN_IS_MANDATORY as detail item.
...
--  161221  SWiclk  LIM-9700, Added condition code as a data item. Modified Get_Sql_Where_Expression___() by adding capture_session_id_ and data_item_id_
...
--  160404  DaZase  LIM-4346, Created.
-----------------------------------------------------------------------------
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      ELSE
         unique_value_ := Inventory_Part_In_Stock_API.Get_Column_Value_If_Unique(no_unique_value_found_      => dummy_,
                                                                                 ...
                                                                                 column_name_                => wanted_data_item_id_,
                                                                                 sql_where_expression_       => Get_Sql_Where_Expression___(capture_session_id_,
     ...
     -- Add support for alternative handling unit keys
      ...
      IF use_unique_values_ THEN
         ...
         IF (part_no_ IS NULL) THEN
            part_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'PART_NO', capture_session_id_);
         END IF;
         IF (configuration_id_ IS NULL) THEN
            configuration_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'CONFIGURATION_ID', capture_session_id_);
         END IF;
         IF (lot_batch_no_ IS NULL) THEN
            lot_batch_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'LOT_BATCH_NO', capture_session_id_);
         END IF;
         IF (serial_no_ IS NULL) THEN
            serial_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'SERIAL_NO', capture_session_id_);
         END IF;
         IF (eng_chg_level_ IS NULL) THEN
            eng_chg_level_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'ENG_CHG_LEVEL', capture_session_id_);
         END IF;
         IF (waiv_dev_rej_no_ IS NULL) THEN
            waiv_dev_rej_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'WAIV_DEV_REJ_NO', capture_session_id_);
         END IF;
         IF (activity_seq_ IS NULL) THEN
            activity_seq_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'ACTIVITY_SEQ', capture_session_id_);
         END IF;
         IF (location_no_ IS NULL) THEN
            location_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'LOCATION_NO', capture_session_id_);
         END IF;
         IF (handling_unit_id_ IS NULL) THEN
            handling_unit_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'HANDLING_UNIT_ID', capture_session_id_);
         END IF;
         IF (alt_handling_unit_label_id_ IS NULL) THEN
            alt_handling_unit_label_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'ALT_HANDLING_UNIT_LABEL_ID', capture_session_id_);
         END IF;
         IF (barcode_id_ IS NULL AND Data_Capture_Invent_Util_API.Inventory_Barcode_Enabled(session_rec_.capture_process_id, session_rec_.capture_config_id)) THEN
            barcode_id_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'BARCODE_ID', capture_session_id_);
         END IF;
...
-- Trying to mimic dlgUnattachPartsFromHandlingUnit but here we also added handling unit id check 
-- and qty_in_transit check and location_type check for this process
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      sql_where_expression_ := ' AND HANDLING_UNIT_ID != 0 AND (QTY_ONHAND - QTY_RESERVED) > 0 AND QTY_IN_TRANSIT = 0 AND LOCATION_TYPE_DB IN (''PICKING'', ''F'', ''MANUFACTURING'') ';
      session_rec_     := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
      process_package_ := Data_Capture_Process_API.Get_Process_Package(session_rec_.capture_process_id);
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      -- Calling Invent Util for some validations (especially for BARCODE_ID and CATCH_QUANTITY)
      ...
      IF (data_item_id_ IN ('BARCODE_ID')) THEN
         ...
         IF (data_item_value_ IS NOT NULL) THEN 
            ...
            Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, 
                               waiv_dev_rej_no_, activity_seq_, barcode_id_, gtin_no_, handling_unit_id_, sscc_, alt_handling_unit_label_id_, 
      ...
      IF (data_item_id_ IN ('PART_NO', 'LOCATION_NO', 'CONFIGURATION_ID', 'LOT_BATCH_NO', 'SERIAL_NO', 'ENG_CHG_LEVEL', 
                            'WAIV_DEV_REJ_NO', 'ACTIVITY_SEQ', 'HANDLING_UNIT_ID', 'SSCC','ALT_HANDLING_UNIT_LABEL_ID', 'CONDITION_CODE')) THEN
         ...
         ELSE
            IF (data_item_id_ IN ('SSCC','ALT_HANDLING_UNIT_LABEL_ID', 'CONDITION_CODE')) THEN
      ...
      ELSIF (data_item_id_ IN ('QUANTITY')) THEN
         ...
         IF (qty_onhand_ IS NOT NULL AND qty_reserved_ IS NOT NULL ) THEN
            ...
            IF (data_item_value_ IS NOT NULL AND data_item_value_ > qty_available_) THEN
               Error_SYS.Record_General(lu_name_, 'TOMUCHQTYTOUNATTACH: Maximum available stock record quantity to be unattached is :P1.', Data_Capture_Common_Util_API.Get_Leading_Zero_For_Decimals(qty_available_));
            END IF;
         END IF;
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ IN ('BARCODE_ID')) THEN
         ...
         Get_Filter_Keys___(dummy_contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                            eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, gtin_no_, handling_unit_id_, sscc_, 
                            alt_handling_unit_label_id_, capture_session_id_, 
                            data_item_id_, data_item_value_ => NULL, use_unique_values_ => TRUE);
      ELSE
         Get_Filter_Keys___(dummy_contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                            eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, gtin_no_, handling_unit_id_, sscc_, 
                            alt_handling_unit_label_id_, capture_session_id_, data_item_id_);
      END IF;
      ...
      ELSIF (data_item_id_ IN ('PART_NO', 'CONFIGURATION_ID', 'LOCATION_NO', 'LOT_BATCH_NO', 'SERIAL_NO', 
                               ...
                               'HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID', 'CONDITION_CODE')) THEN

         IF (data_item_id_ IN ('HANDLING_UNIT_ID', 'SSCC', 'ALT_HANDLING_UNIT_LABEL_ID') AND
             barcode_id_ IS NOT NULL)  THEN
            -- Trying to fetch as many stock keys as possible from the barcode so LOV can be filtered more accurate
            Get_Filter_Keys___(dummy_contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                               eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, gtin_no_, handling_unit_id_, sscc_, 
                               alt_handling_unit_label_id_, capture_session_id_, 
                               data_item_id_, data_item_value_ => NULL, use_unique_values_ => TRUE);
      ...
      IF (automatic_value_ IS NULL) THEN
         ...
         ELSIF (data_item_id_ IN ('QUANTITY')) THEN
            ...
            ELSE
               ...
               -- get qty_available
               IF (qty_onhand_ IS NOT NULL AND qty_reserved_ IS NOT NULL ) THEN
                  automatic_value_ := qty_onhand_ - qty_reserved_;
               END IF;
            END IF;
...
BEGIN
   ...
   IF (Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('HandlingUnitsInReceiptHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('HandlingUnitHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseTierHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('WarehouseNavigatorHandling', 'ExecuteUnAttachFromHandlingUnit') OR 
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseRowHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseLocHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('InventoryPartInStockHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseBayHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('ReceiptHandling', 'ExecuteUnAttachFromHandlingUnit') OR
       Security_SYS.Is_Proj_Action_Available('HandlingUnitsInStockHandling', 'ExecuteUnAttachFromHandlingUnit') ) THEN
   ...
   -- repack and attach/connect functions but no need checking those for the unattach process
...
BEGIN
   IF no_of_records_handled_ = 1 THEN
      delimiter_ := INSTR(process_message_, '^', 1, 1);
      part_no_ := substr(process_message_, 0, delimiter_ - 1); 
      handling_unit_id_ := TO_NUMBER(substr(process_message_, delimiter_ + 1, length(process_message_)));
      message_ :=  Language_SYS.Translate_Constant(lu_name_, 'UNATTACHOK: The stock record or part of stock record :P1 was unattached from Handling Unit :P2.', NULL, part_no_, handling_unit_id_);
   ELSE
      message_ := Language_SYS.Translate_Constant(lu_name_, 'UNATTACHESOK: :P1 stock records was unattached from handling unit(s).', NULL, no_of_records_handled_);
   END IF;
   RETURN message_;
...
IS
   ...
   activity_seq_                 NUMBER;
   handling_unit_id_             NUMBER; 
   quantity_to_unattach_         NUMBER;
   catch_quantity_to_unattach_   NUMBER;
BEGIN
   ptr_ := NULL;
   WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
      ...
      ELSIF (name_ = 'QUANTITY') THEN
         quantity_to_unattach_ := value_;
      ELSIF (name_ = 'CATCH_QUANTITY') THEN
         catch_quantity_to_unattach_ := value_;
      END IF;
   END LOOP;
   ...
   Inventory_Part_In_Stock_API.Change_Handling_Unit_Id(contract_             => contract_,
                                                       ...
                                                       quantity_             => quantity_to_unattach_,
                                                       catch_quantity_       => catch_quantity_to_unattach_);


   process_message_ := part_no_ || '^' || handling_unit_id_;
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (part_no_ IS NULL) THEN
         Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                            eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, gtin_no_, handling_unit_id_, sscc_, 
                            alt_handling_unit_label_id_, capture_session_id_, data_item_id_, use_applicable_ => FALSE);
         part_no_ := Get_Unique_Data_Item_Value___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, eng_chg_level_, waiv_dev_rej_no_, activity_seq_, handling_unit_id_, alt_handling_unit_label_id_, barcode_id_, 'PART_NO', capture_session_id_);
      END IF; 
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
     ...
     -- fetch all necessary keys for all possible detail items below
      session_rec_          := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
      Get_Filter_Keys___(contract_, part_no_, location_no_, configuration_id_, lot_batch_no_, serial_no_, 
                         eng_chg_level_, waiv_dev_rej_no_, activity_seq_, barcode_id_, gtin_no_, handling_unit_id_, 
      ...
      IF (conf_item_detail_tab_.COUNT > 0) THEN
         FOR i IN conf_item_detail_tab_.FIRST..conf_item_detail_tab_.LAST LOOP
            ...
            ELSE  -- FEEDBACK ITEMS AS DETAILS
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('CATCH_UNIT_MEAS', 'PART_DESCRIPTION', 'UNIT_MEAS', 'INPUT_CONV_FACTOR')) THEN
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('PARENT_HANDLING_UNIT_DESC', 'HANDLING_UNIT_TYPE_ADD_VOLUME', 
                                                                       ...
                                                                       'HANDLING_UNIT_TYPE_STACKABLE', 'TOP_PARENT_HANDLING_UNIT_TYPE_ID', 
                                                                       'TOP_PARENT_HANDLING_UNIT_TYPE_DESC')) THEN
