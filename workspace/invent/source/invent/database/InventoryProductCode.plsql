-----------------------------------------------------------------------------
--
--  Logical unit: InventoryProductCode
--  Component:    INVENT
--
...
--  ------------------------------- Eagle ----------------------------------
--  070519  WaJalk  Made the column 'part_product_code' uppercase.
...
--  970312  MAGN    Changed tablename from mpc_product_code to inventory_product_code_tab.
...
PROCEDURE Insert_Lu_Data_Rec__ (
   newrec_ IN INVENTORY_PRODUCT_CODE_TAB%ROWTYPE)
IS
   dummy_ VARCHAR2(1);
   rec_   inventory_product_code_tab%ROWTYPE;
   CURSOR exist IS
      SELECT 'X'
      FROM INVENTORY_PRODUCT_CODE_TAB
      WHERE part_product_code = newrec_.part_product_code;
...
FUNCTION Get_Description (
   part_product_code_ IN VARCHAR2 ) RETURN VARCHAR2
IS
   temp_ inventory_product_code_tab.description%TYPE;
BEGIN
   IF (part_product_code_ IS NULL) THEN
      RETURN NULL;
   END IF;
   temp_ := substr(Basic_Data_Translation_API.Get_Basic_Data_Translation(module_, lu_name_,
      part_product_code_), 1, 35);
   ...
   SELECT description
      INTO  temp_
      FROM  inventory_product_code_tab
      WHERE part_product_code = part_product_code_;
   RETURN temp_;
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;
   WHEN too_many_rows THEN
      Raise_Too_Many_Rows___(part_product_code_, 'Get_Description');
END Get_Description;
