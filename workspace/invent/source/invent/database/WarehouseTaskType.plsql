...
--  161124  DaZase   Fixed so Get_Data_Capt_Item_Value_Desc actually shows the correct descriptions.
...
--  141006  JeLise   Added Create_Data_Capture_Lov.
-----------------------------------------------------------------------------
...
IS
BEGIN
   IF data_item_value_ IN ('CUSTOMER ORDER PICK LIST', 'TRANSPORT TASK', 'SHOP ORDER PICK LIST') THEN
      NULL;
   ELSE
...
BEGIN
   IF (data_item_value_ = 'CUSTOMER ORDER PICK LIST') THEN
      description_ := SUBSTR(Get_Client_Value(0),1,50); 
   ELSIF (data_item_value_ = 'TRANSPORT TASK') THEN
      description_ := SUBSTR(Get_Client_Value(1),1,50);
   ELSIF (data_item_value_ = 'SHOP ORDER PICK LIST') THEN
      description_ := SUBSTR(Get_Client_Value(2),1,50);
   END IF;   
   RETURN description_;
