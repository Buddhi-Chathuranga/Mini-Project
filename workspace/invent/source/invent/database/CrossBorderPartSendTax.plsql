-----------------------------------------------------------------------------
--
--  Logical unit: CrossBorderPartSendTax
--  Component:    INVENT
--
...
IS
   CURSOR get_line_rec IS
      ...
      AND    rowtype LIKE 'CrossBorderPartSendTax%';
...
BEGIN
   FOR rec_ IN get_line_rec LOOP
      IF (rec_.tax_code = newrec_.tax_code) THEN
         Error_SYS.Record_General(lu_name_, 'TAXCODEEXIST: Tax Code :P1 is already defined in Sender Country :P2.', newrec_.tax_code, Iso_Country_API.Decode(newrec_.sender_country));  
      END IF;

      IF (rec_.valid_from = newrec_.valid_from) THEN
         Error_SYS.Record_General(lu_name_, 'VALIDFROMEXIST: Valid From Date :P1 is already defined for Tax Code :P2 in Sender Country :P3.', newrec_.valid_from, rec_.tax_code, Iso_Country_API.Decode(newrec_.sender_country));  
      END IF;      
   END LOOP; 
...
FUNCTION Get_Latest_Sender_Tax_Code (
...
IS
   ...
   CURSOR sender_tax_for_all_parts IS
      SELECT tax_code
      FROM   cross_border_part_move_tax_tab
      WHERE  company = company_
      AND    sender_country = sender_country_
   ...
   sender_tax_code_  VARCHAR2(20);
   
BEGIN
   OPEN sender_tax_for_part;
   FETCH sender_tax_for_part INTO sender_tax_code_;
   IF sender_tax_for_part%NOTFOUND THEN
      OPEN sender_tax_for_all_parts;
      FETCH sender_tax_for_all_parts INTO sender_tax_code_;
      CLOSE sender_tax_for_all_parts;
   END IF;
   CLOSE sender_tax_for_part;
   RETURN sender_tax_code_;  
END Get_Latest_Sender_Tax_Code;
