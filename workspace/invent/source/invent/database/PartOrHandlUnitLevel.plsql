...
-- This method is used by DataCaptureMovePart
@ServerOnlyAccess
...
IS
BEGIN
   IF data_item_value_ IN ('PART', 'HANDLING_UNIT') THEN
      NULL;
   ELSE
...
BEGIN
   IF (data_item_value_ = 'PART') THEN
      description_ := SUBSTR(Get_Client_Value(0),1,50); 
   ELSIF (data_item_value_ = 'HANDLING_UNIT') THEN
