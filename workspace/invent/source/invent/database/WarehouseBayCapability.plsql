-----------------------------------------------------------------------------
--
--  Logical unit: WarehouseBayCapability
--  Component:    INVENT
--
...
--  110718  MaEelk  Added user allowed site filter to WAREHOUSE_BAY_CAPABILITY.
...
PROCEDURE Check_And_Delete___ (
   remrec_ IN WAREHOUSE_BAY_CAPABILITY_TAB%ROWTYPE )
IS
...
PROCEDURE Check_Insert___ (
   newrec_ IN OUT warehouse_bay_capability_tab%ROWTYPE,
...
IS
   ...
   objid_      WAREHOUSE_BAY_CAPABILITY.objid%TYPE;
   objversion_ WAREHOUSE_BAY_CAPABILITY.objversion%TYPE;
   newrec_     WAREHOUSE_BAY_CAPABILITY_TAB%ROWTYPE;
   indrec_     Indicator_Rec;
BEGIN

   IF (Check_Exist___(contract_, warehouse_id_, bay_id_, storage_capability_id_)) THEN
      Error_SYS.Record_General(lu_name_, 'WHBAYCAPLINEEXIST: You cannot remove this capability because it already exists on this level, please remove the capability on this level first');
   END IF;
...
IS
   remrec_ WAREHOUSE_BAY_CAPABILITY_TAB%ROWTYPE;
   ...
   ELSE
      IF (capability_tab_.COUNT > 0) THEN
         FOR i IN capability_tab_.FIRST..capability_tab_.LAST LOOP
            IF Check_Exist___(contract_, warehouse_id_, bay_id_, capability_tab_(i).storage_capability_id) THEN
               remrec_ := Lock_By_Keys___(contract_, warehouse_id_, bay_id_, capability_tab_(i).storage_capability_id);
...
IS
   ...
   objid_      WAREHOUSE_BAY_CAPABILITY.objid%TYPE;
   objversion_ WAREHOUSE_BAY_CAPABILITY.objversion%TYPE;
   oldrec_     WAREHOUSE_BAY_CAPABILITY_TAB%ROWTYPE;
   newrec_     WAREHOUSE_BAY_CAPABILITY_TAB%ROWTYPE;
   indrec_     Indicator_Rec;
BEGIN

   oldrec_ := Get_Object_By_Keys___(contract_, warehouse_id_, bay_id_, storage_capability_id_);
