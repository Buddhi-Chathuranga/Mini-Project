-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptCountHandlUnit
--  Component:    INVENT
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: COUNT_HANDLING_UNIT
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
...
--  171213  SURBLK  STRSC-14757, Added Customized process message to Get_Process_Execution_Message when subsequent process exist.
--  171127  DaZase  STRSC-14754, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
--  171127          of anything found later in that method. Changed size to 4000 on value_ in Execute_Process and detail_value_ in Add_Unique_Data_Item_Detail___.
--  171122  CKumlk  Modified Create_List_Of_Values , Execute_Process and Validate_Data_Item to Handle data item ACTION.
...
--  161216  SWiclk  LIM-9690, Modified Add_Details_For_Latest_Item() in order to get LAST_COUNT_DATE.
--  160920  DaZase  LIM-8318, Added Level 2 feedback items.
--  160811  Erlise  Created.
-----------------------------------------------------------------------------
...
FUNCTION Get_Sql_Where_Expression___ ( 
   capture_session_id_  IN NUMBER, 
   validate_            IN BOOLEAN := TRUE ) RETURN VARCHAR2
IS
   session_rec_  Data_Capture_Common_Util_API.Session_Rec;
...
--      IF validate_ THEN 
--         return_value_ := return_value_ || ' AND  total_reserved_qty = 0 ';
--      END IF ; 
   $END
...
IS
   ...
   sql_where_expression_          VARCHAR2(2000) := Get_Sql_Where_Expression___(capture_session_id_, FALSE);
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ IN ('ACTION')) THEN
         Count_Handling_Unit_Action_API.Create_Data_Capture_Lov(capture_session_id_ => capture_session_id_);
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      ELSIF data_item_id_ = 'ACTION' THEN
         Count_Handling_Unit_Action_API.Exist_Db(data_item_value_) ;
...
IS
   message_ VARCHAR2(200):= NULL;
BEGIN
   IF process_message_ = 'CONTINUE_PROCESS' THEN
      IF no_of_records_handled_ = 1 THEN
         message_ :=  Language_SYS.Translate_Constant(lu_name_, 'COUNTINGOK: The counting result was saved.');
      ELSE
         message_ := Language_SYS.Translate_Constant(lu_name_, 'COUNTINGSOK: :P1 counting results were saved.', NULL, no_of_records_handled_);
      END IF;
   ELSIF process_message_ = 'SUBSEQ_PROCESS_NOT_EXIST' THEN
      message_ :=  Language_SYS.Translate_Constant(lu_name_, 'NOSUBSEQ: Subsequent process Count Inventory Part is expected to be configured for this Action.');
   END IF;
   RETURN message_;
...
IS
   ...
   action_                    VARCHAR2(20);
   note_                      VARCHAR2(2000);
   count_all_to_zero_         VARCHAR2(5);
   state_                     VARCHAR2(2000);
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      session_rec_               Data_Capture_Session_API.Public_Rec;
      config_rec_                Data_Capture_Config_API.Public_Rec;
   $END
   next_process_id_           VARCHAR2(30);
   next_capture_config_id_    NUMBER;
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ptr_ := NULL;
      WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
         ...
         ELSIF (name_ = 'ACTION') THEN
            action_ := value_;
            IF (action_ = Count_Handling_Unit_Action_API.DB_EXISTS) THEN
               count_all_to_zero_ := FND_BOOLEAN_API.DB_FALSE;
            ELSIF (action_ = Count_Handling_Unit_Action_API.DB_DOES_NOT_EXIST) then
               count_all_to_zero_ := FND_BOOLEAN_API.DB_TRUE;        
            END IF;   
      ...
      IF count_all_to_zero_ IS NOT NULL THEN
         Handling_Unit_API.New_Counting_Result(state_            => state_,
                                               handling_unit_id_ => handling_unit_id_, 
                                               notes_            => note_, 
                                               count_all_to_zero_=> count_all_to_zero_);
      END IF;

      IF action_ = Count_Handling_Unit_Action_API.DB_COUNT_PART AND Data_Capture_Session_API.Is_Completed(capture_session_id_) THEN
         session_rec_        := Data_Capture_Session_API.Get(capture_session_id_); 
         IF (Data_Capture_Config_API.Get_Process_Completion_Acti_Db(session_rec_.capture_process_id, session_rec_.capture_config_id) = Data_Capt_Proc_Cmpl_Action_API.DB_CREATE_NEW_SESSION) THEN
            config_rec_ := Data_Capture_Config_API.Get(session_rec_.capture_process_id, session_rec_.capture_config_id);
            IF ((config_rec_.subsequent_capture_process_id IS NOT NULL AND config_rec_.subsequent_capture_config_id IS NOT NULL)
                OR (config_rec_.subseq_ctrl_data_item_id IS NOT NULL) OR (config_rec_.subseq_ctrl_feedback_item_id IS NOT NULL)) THEN

                  Data_Capture_Session_Line_API.Get_Subsequent_Process_Config(subsequent_capture_process_id_ => next_process_id_,
                                                                              subsequent_capture_config_id_  => next_capture_config_id_,
                                                                              current_capture_process_id_    => session_rec_.capture_process_id,
                                                                              current_capture_config_id_     => session_rec_.capture_config_id,
                                                                              current_subseq_process_id_     => config_rec_.subsequent_capture_process_id,
                                                                              current_subseq_config_id_      => config_rec_.subsequent_capture_config_id,
                                                                              subseq_ctrl_data_item_id_      => config_rec_.subseq_ctrl_data_item_id,
                                                                              subseq_ctrl_feedback_item_id_  => config_rec_.subseq_ctrl_feedback_item_id,
                                                                              current_session_id_            => capture_session_id_);

                  IF next_process_id_ = 'COUNT_PART' THEN                                                            
                     process_message_ := 'SUBSEQ_PROCESS_EXIST';
                  ELSE
                     process_message_ := 'SUBSEQ_PROCESS_NOT_EXIST';
                  END IF;
            END IF;
         END IF;
      ELSE
         process_message_ := 'CONTINUE_PROCESS';
      END IF;
   $ELSE
      NULL;
   $END
...
BEGIN
   -- Check to see that API method Handling_Unit_API.New_Counting_Result is granted thru any of following projections/actions
   IF (Security_SYS.Is_Proj_Action_Available('CountHandlingUnits', 'CountZero') OR
       Security_SYS.Is_Proj_Action_Available('CountHandlingUnits', 'CountWithoutDifferences')) THEN
      ...
      IF (conf_item_detail_tab_.COUNT > 0) THEN
         FOR i IN conf_item_detail_tab_.FIRST..conf_item_detail_tab_.LAST LOOP
            ...
            ELSE  
               ...
               IF (conf_item_detail_tab_(i).data_item_detail_id IN ('PARENT_HANDLING_UNIT_DESC', 
                                                                    'HANDLING_UNIT_TYPE_ID', 
                                                                    'HANDLING_UNIT_TYPE_DESC',
                                                                    'HANDLING_UNIT_TYPE_CATEG_ID', 
                                                                    'HANDLING_UNIT_TYPE_CATEG_DESC', 
                                                                    'HANDLING_UNIT_TYPE_STACKABLE', 
                                                                    'HANDLING_UNIT_TYPE_ADD_VOLUME', 
                                                                    'HANDLING_UNIT_TYPE_MAX_VOL_CAP', 
                                                                    'HANDLING_UNIT_TYPE_MAX_WGT_CAP',
                                                                    'HANDLING_UNIT_TYPE_NO_OF_LBLS',
                                                                    'TOP_PARENT_HANDLING_UNIT_TYPE_ID', 
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('HANDLING_UNIT_ACCESSORY_EXIST', 
                                                                       'HANDLING_UNIT_WAREHOUSE_ID',
                                                                       'HANDLING_UNIT_BAY_ID', 
                                                                       'HANDLING_UNIT_BIN_ID',
                                                                       'HANDLING_UNIT_ROW_ID',
                                                                       'HANDLING_UNIT_TIER_ID',
                                                                       'HANDLING_UNIT_COMPOSITION', 
                                                                       'HANDLING_UNIT_GEN_SSCC', 
                                                                       'HANDLING_UNIT_WIDTH', 
                                                                       'HANDLING_UNIT_HEIGHT', 
                                                                       'HANDLING_UNIT_DEPTH', 
                                                                       'HANDLING_UNIT_LOCATION_TYPE',
                                                                       'HANDLING_UNIT_LOCATION_NO_DESC',
                                                                       'HANDLING_UNIT_MANUAL_GROSS_WEIGHT', 
                                                                       'HANDLING_UNIT_MANUAL_VOLUME', 
                                                                       'HANDLING_UNIT_NET_WEIGHT',
                                                                       'HANDLING_UNIT_OPERATIVE_VOLUME', 
                                                                       'HANDLING_UNIT_OPERATIVE_GROSS_WEIGHT', 
                                                                       'HANDLING_UNIT_TARE_WEIGHT', 
                                                                       'HANDLING_UNIT_UOM_LENGTH',
                                                                       'HANDLING_UNIT_UOM_WEIGHT', 
                                                                       'HANDLING_UNIT_UOM_VOLUME', 
                                                                       'HANDLING_UNIT_MIX_OF_PART_NO_BLOCKED', 
                                                                       'HANDLING_UNIT_MIX_OF_CONDITION_CODE_BLOCKED', 
                                                                       'HANDLING_UNIT_MIX_OF_LOT_BATCH_NUMBERS_BLOCKED', 
                                                                       'HANDLING_UNIT_NO_OF_LBLS', 
                                                                       'HANDLING_UNIT_PRINT_LBL', 
                                                                       'HANDLING_UNIT_STRUCTURE_LEVEL', 
                                                                       'PARENT_HANDLING_UNIT_ID', 
                                                                       'PARENT_SSCC', 
                                                                       'PARENT_ALT_HANDLING_UNIT_LABEL_ID', 
                                                                       'TOP_PARENT_HANDLING_UNIT_ID', 
                                                                       'TOP_PARENT_SSCC', 
                                                                       'TOP_PARENT_ALT_HANDLING_UNIT_LABEL_ID', 
                                                                       'LEVEL_2_HANDLING_UNIT_ID', 
                                                                       'LEVEL_2_SSCC', 
                                                                       'LEVEL_2_ALT_HANDLING_UNIT_LABEL_ID',
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id IN ('PART_NO', 'SERIAL_NO', 'LOT_BATCH_NO', 'CONFIGURATION_ID', 'ENG_CHG_LEVEL', 
                                                                       ...
                                                                       'EXPIRATION_DATE', 'LAST_ACTIVITY_DATE', 
