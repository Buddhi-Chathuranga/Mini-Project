-----------------------------------------------------------------------------
--
--  Logical unit: DataCapRecHuFromTrans
--  Component:    INVENT
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: RECEIVE_HU_FROM_TRANSIT
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  200812  DaZase  SC2020R1-7510, Replaced Is_Method_Available with Is_Proj_Action_Available in Is_Process_Available to support new projection security concept.
--  180222  RuLiLk  STRSC-16860, Modified method Validate_Data_Item to validate if GS1 barcodes are mandatory.
--  171030  DaZase  STRSC-13047, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
...
--  160108  KhVese  LIM-2927, Created. 
-----------------------------------------------------------------------------
...
BEGIN
   IF no_of_records_handled_ = 1 THEN
      message_ := Language_SYS.Translate_Constant(lu_name_, 'MOVEHANDLINGUNIT: The stock has been successfully received.');
   ELSE
      message_ := Language_SYS.Translate_Constant(lu_name_, 'MOVEHANDLINGUNITS: :P1 stocks have been successfully received.', NULL, no_of_records_handled_);
   END IF;
   RETURN message_;
...
BEGIN
   ...
   Handling_Unit_API.Receive_From_Inventory_Transit(handling_unit_id_ => handling_unit_id_);
...
BEGIN
   -- Check to see that API method Handling_Unit_API.Receive_From_Inventory_Transit is granted thru any of following projections/actions
   IF (Security_SYS.Is_Proj_Action_Available('HandlingUnitHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('WarehouseNavigatorHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('HandlingUnitsInReceiptHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseTierHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseRowHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseLocHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('InventLocationWarehouseBayHandling', 'ReceiveFromInventoryTransit') OR 
       Security_SYS.Is_Proj_Action_Available('HandlingUnitsInStockHandling', 'ReceiveFromInventoryTransit')) THEN
...
BEGIN
   ...
   RETURN ' AND  Handling_Unit_API.Is_In_Inventory_Transit(handling_unit_id) = ''TRUE'' 
