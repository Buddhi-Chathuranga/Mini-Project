...
PROCEDURE Validate_Related_Uuids (
   company_                 IN VARCHAR2,
...
IS    
   ...
   FROM   Uuid_Information
   WHERE  company = company_
   AND    source_ref_type_db  = Uuid_Source_Ref_Type_API.DB_INVOICE
   AND    UPPER(uuid_number) = UPPER(uuid_number_);
  
   count_ NUMBER;
BEGIN 
   IF (Finance_Lib_API.Fin_Length(related_uuid_number_) IS NOT NULL) THEN
      FOR i IN
        (SELECT trim(regexp_substr(related_uuid_number_, '[^;]+', 1, LEVEL)) l
         FROM dual
         CONNECT BY LEVEL <= regexp_count(related_uuid_number_, ';') +1 )
      LOOP
         OPEN get_count (company_, i.l);
         FETCH get_count INTO count_;
         CLOSE get_count;
         IF (count_ = 0) THEN
            Error_SYS.Record_General(lu_name_, 'MXINVUUIDNOTEXIST: UUID Number ":P1" does not exist for any invoice in company :P2.', i.l, company_);
         END IF;         
      END LOOP;     
   END IF;   
END Validate_Related_Uuids;
-- gelr:mx_xml_doc_reporting, end