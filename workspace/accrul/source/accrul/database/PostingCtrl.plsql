...
--  210724  THPELK  LCS Merge Bug 159891, Added micro cache for Get_Control_Type.
...
BEGIN
   ...
   Posting_Ctrl_Public_API.Invalidate_Ctrl_Type_Cache__();
END Insert___;
...
BEGIN
   ...
   code_i_ := 0;
   code_j_ := 0;
   FOR rec_ IN fetch_code_part LOOP 
      IF rec_.code_part = 'A' THEN
         account_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'B' THEN
        code_b_  := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'C' THEN
         code_c_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'D' THEN
        code_d_  := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'E' THEN
        code_e_  := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'F' THEN
         code_f_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'G' THEN
         code_g_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'H' THEN
         code_h_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'I' THEN
         code_i_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      ELSIF rec_.code_part = 'J' THEN
         code_j_ := Get_Code_Part_Exist___(company_, posting_type_, rec_.code_part, control_type_, pc_valid_from_);
      END IF;
   END LOOP;
