...
--  210506  NiDalk  Bug 158933, Modified Fetch_And_Calculate_Tax to consider tax_date.
--  210601  Smallk  FI21R2-1421, Fixed issue with Vertex Fetch_Jurisdiction_Code(). 
-----------------------------------------------------------------------------
...
BEGIN        
   ...
   tax_date_        := NVL(ext_tax_param_in_rec_.tax_date, invoice_date_);
   ...
   END IF;
   IF (tax_date_ IS NOT NULL) THEN
      Plsqlap_Document_API.Add_Attribute(document_, 'TAX_DATE', TO_CHAR(tax_date_, 'YYYY-MM-DD'), line_element_id_);   
   END IF;
