...
--  211215  Hecolk   KEEP-5811, Added Get_Valid_Document_Type_Code 
-----------------------------------------------------------------------------
...
-- gelr:it_tax_reports, begin
FUNCTION Get_Nature_Of_Operation (
   company_     IN VARCHAR2,
   fee_code_    IN VARCHAR2 ) RETURN VARCHAR2
IS
   temp_ tax_code_texts_tab.nature_of_operation%TYPE;
   CURSOR get_nature_of_operation IS
      SELECT nature_of_operation
      FROM   tax_code_texts_tab
      WHERE  company = company_
      AND    fee_code = fee_code_;
BEGIN
   OPEN get_nature_of_operation;
   FETCH get_nature_of_operation INTO temp_;
   CLOSE get_nature_of_operation;
   RETURN Nature_Of_Operation_API.Decode(temp_);
END Get_Nature_Of_Operation;
-- gelr:it_tax_reports, end
...
FUNCTION Get_Valid_Document_Type_Code (
   company_     IN VARCHAR2,
   fee_code_    IN VARCHAR2,
   trans_date_  IN DATE ) RETURN VARCHAR2
IS
   document_type_code_  tax_code_texts_tab.document_type_code%TYPE;
   CURSOR get_valid_document_type_code IS
      SELECT document_type_code
      FROM tax_code_texts_tab
      WHERE company = company_
      AND fee_code = fee_code_
      AND trans_date_ BETWEEN valid_from AND valid_until;
BEGIN
   OPEN  get_valid_document_type_code;
   FETCH get_valid_document_type_code INTO document_type_code_;
   CLOSE get_valid_document_type_code;
   RETURN document_type_code_;
END Get_Valid_Document_Type_Code;
