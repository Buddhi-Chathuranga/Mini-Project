...
--  210514  NWeelk  FI21R2-458, Merged LCS bug 158397.
--  211018  ChJalk  SC21R2-5258, Modified the method Validate_Address to Remove special characters in ADDRESS_CODE.
--  211116  NiDalk  SC21R2-5776, Merged LCS bug 161382. Modified Create_Invoice_Avalara_Tax_Info_Req_Data___ to send net_curr_amount in invoice line instead of the net_dom ammount.
--  220101  NiDalk  SC21R2-7065, Merged LCS bug 162016, Modified Create_Invoice_Avalara_Tax_Info_Req_Data___ to send credit invoice positive values correctly.
--  220311  DhAplk  SCDEV-7853, Modified Create_Invoice_Avalara_Tax_Info_Req_Data___ function to add currency_code to avalara invoice request message.
--  220530  NWeelk  FIZ-17582, Modified the method Create_Base_Address_Item___ and Create_Line_Item___ to Remove special characters 
--  220530          in ADDRESS_CODE, LINE1, LINE2 and DESCRIPTION. 
--  220604  DhAplk  SCDEV-10605, Added currency_code to Create_Order_Request___ and modified Fetch_And_Calculate_Tax methods to pass currency_code as param for that method.
--  220909  DhAplk  SCDEV-13766, Modified Create_Invoice_Avalara_Tax_Info_Req_Data___ method to get line item description from Invoice line item description.
--  221213  NiDalk  SCDEV-18762, Merged LCS bug 165122. Modified Assign_Tax_Amount_Percentage___ to support special taxes for states such as COLORADO retail delivery fee where 
--  221213          there can be a tax even when txable amount is 0.
-----------------------------------------------------------------------------
...
BEGIN
   ...
   Plsqlap_Document_API.Add_Attribute(document_, 'CURRENCY_CODE', currency_code_, ord_req_element_id_);
...
BEGIN
   ...
   Plsqlap_Document_API.Add_Attribute(document_, 'ADDRESS_CODE', Rem_Special_Char(address_code_), element_id_);
   Plsqlap_Document_API.Add_Attribute(document_, 'LINE1', NVL(Rem_Special_Char(address1_), '*'), element_id_);
   Plsqlap_Document_API.Add_Attribute(document_, 'LINE2', Rem_Special_Char(address2_), element_id_);
...
BEGIN
   ...
   Plsqlap_Document_API.Add_Attribute(document_, 'DESCRIPTION', Rem_Special_Char(description_), element_id_);
...
BEGIN
   ...
   FOR j_ IN 1 .. 10 LOOP
      IF (Get_Next_Val_By_Tag_Name___(xml_trans_, 'JURIS_TYPE', j_, i_) = 'State') THEN
         ...
         ELSE
            -- To support special taxes for states such as COLORADO retail delivery fee where there can be a tax even when txable amount is 0.  
            IF Get_Next_Val_By_Tag_Name___(xml_trans_, 'TAX', j_, i_) != 0 AND Get_Next_Val_By_Tag_Name___(xml_trans_, 'RATE', j_, i_) != 0 THEN
               ext_tax_param_out_rec_.state_tax_amount     := NVL(ext_tax_param_out_rec_.state_tax_amount, 0) + Get_Next_Val_By_Tag_Name___(xml_trans_, 'TAX', j_, i_);
               ext_tax_param_out_rec_.state_tax_percentage := NVL(ext_tax_param_out_rec_.state_tax_percentage, 0) + Get_Next_Val_By_Tag_Name___(xml_trans_, 'RATE', j_, i_);
            END IF; 
         END IF;
...
IS
   ...
   tax_amount_                      NUMBER;
   credit_corr_invoice_             BOOLEAN := FALSE;
...
BEGIN
   $IF Component_Order_SYS.INSTALLED $THEN
      ...
      tax_request_rec_.currency_code := head_rec_.currency;
      ...
      credit_corr_invoice_ := (('CUSTORDCRE' = head_rec_.invoice_type) OR (def_co_cor_inv_type_ IS NOT NULL AND def_co_cor_inv_type_ = head_rec_.invoice_type) OR
                               ('CUSTCOLCRE' = head_rec_.invoice_type) OR (def_col_cor_inv_type_ IS NOT NULL AND def_col_cor_inv_type_ = head_rec_.invoice_type) );
      IF (credit_corr_invoice_) THEN
      ...
      FOR i_ IN 1..head_rec_.items.count LOOP
         ...
         IF (tax_liablity_ = 'TAX' AND taxable_prod_ = 'Y' AND taxable_customer_ = 'Y') THEN
            ...
            line_item_rec_.description := Rem_Special_Char(invoice_line_item_rec_.description);
            IF (credit_corr_invoice_ AND line_item_rec_.qty < 0) THEN
               line_item_rec_.qty := invoice_line_item_rec_.invoiced_qty * -1;
            ELSE 
            ...
            correction_invoice_ := Invoice_API.Is_Correction_Invoice(invoice_line_item_rec_.company, invoice_line_item_rec_.invoice_id);
            tax_amount_         := Customer_Order_Inv_Item_API.Get_Vat_Curr_Amount(invoice_line_item_rec_.company, invoice_line_item_rec_.invoice_id, invoice_line_item_rec_.item_id, Party_Type_API.Decode('CUSTOMER'), head_rec_.identity);
            -- line_item_rec_.amount is a string and invoice_line_item_rec_.net_curr_amount is a number. Do we need to 
            ...
            line_item_rec_.amount := invoice_line_item_rec_.net_curr_amount;
            IF (correction_invoice_ = 'TRUE' AND invoice_line_item_rec_.prel_update_allowed = 'FALSE') THEN 
               IF (tax_amount_ != 0) THEN 
                  line_item_rec_.customer_usage_type := 'Taxable';
               ELSE
                  line_item_rec_.exemption_no        := 1;
                  line_item_rec_.customer_usage_type := NULL;
               END IF;
            ELSE
...
BEGIN
   ...
   Create_Order_Request___(document_, ord_req_element_id_, invoice_date_, invoice_no_, ext_tax_param_in_rec_.company, exemption_no_, ext_tax_param_in_rec_.identity, ext_tax_param_in_rec_.currency_code);
...
BEGIN
   ...
   document_ := Plsqlap_Document_API.New_Document('ORDER_TAX_INFO_REQUEST');
    --Add login and version info
   Set_Login_And_Version_Info___(document_, document_.root_id);
   Create_Order_Request___(document_, ord_req_element_id_, invoice_date_, invoice_no_, ext_tax_param_in_arr_(1).company, exemption_no_, ext_tax_param_in_arr_(1).identity, ext_tax_param_in_arr_(1).currency_code);
...
BEGIN   
   ...
   Set_Login_And_Version_Info___(document_, document_.root_id);      
   element_id_ := Plsqlap_Document_API.Add_Aggregate(document_, 'VALIDATE_REQUEST', 'ADDRESS', document_.root_id);
   Plsqlap_Document_API.Add_Attribute(document_,'ADDRESS_CODE', Rem_Special_Char(postal_addresses_(0).address_id), element_id_); 
