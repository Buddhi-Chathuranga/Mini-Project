...
--  210724  THPELK  Merged LCS Bug 159891, Added micro cache for Get_Control_Type.
-----------------------------------------------------------------------------
...
-- Cache for the Get_Control_Type method
TYPE posting_ctrl_type_tab IS TABLE OF control_type_value_table INDEX BY VARCHAR2(50);
micro_cache_posting_ctrl_rec_tab_     posting_ctrl_type_tab;
micro_cache_posting_ctrl_rec_time_    NUMBER := 0;
micro_cache_posting_ctrl_rec_user_    VARCHAR2(30);

TYPE Linked_Posting_Rec_Cache IS TABLE OF VARCHAR2(50) INDEX BY PLS_INTEGER;
micro_cache_rec_link_tab_              Linked_Posting_Rec_Cache;
micro_cache_rec_max_id_                PLS_INTEGER;
max_cached_rec_element_count_          CONSTANT NUMBER := 100; 
max_cached_rec_element_life_           CONSTANT NUMBER := 60;

-------------------- LU SPECIFIC IMPLEMENTATION METHODS ---------------------
FUNCTION Get_Control_Type___ (   
   posting_type_ IN  VARCHAR2,
   company_      IN  VARCHAR2,
   date_         IN  DATE ) RETURN CONTROL_TYPE_VALUE_TABLE
IS
   ct_value_tbl_  CONTROL_TYPE_VALUE_TABLE; 
...
PROCEDURE Invalidate_Ctrl_Type_Cache__
IS
BEGIN
   micro_cache_posting_ctrl_rec_tab_.delete;
   micro_cache_posting_ctrl_rec_time_ := 0;         
   micro_cache_rec_link_tab_.delete;
   micro_cache_rec_max_id_ := 0;   
END Invalidate_Ctrl_Type_Cache__;
...
IS
   ...
   req_id_              VARCHAR2(50) := company_ ||'^'|| posting_type_ ||'^'|| TO_CHAR(date_, 'YYYY-MM-DD');
   local_ct_value_tab_  control_type_value_table;
   empty_ct_value_tab_  control_type_value_table;
   not_found            EXCEPTION;     
   
   
BEGIN
   time_    := Database_SYS.Get_Time_Offset;
   expired_ := ((time_ - micro_cache_posting_ctrl_rec_time_) > max_cached_rec_element_life_);        
      
   IF (expired_ OR (micro_cache_posting_ctrl_rec_user_ IS NULL) OR (micro_cache_posting_ctrl_rec_user_ != Fnd_Session_API.Get_Fnd_User)) THEN
      Invalidate_Ctrl_Type_Cache__;
      micro_cache_posting_ctrl_rec_user_ := Fnd_Session_API.Get_Fnd_User;
   END IF;      

   IF (NOT micro_cache_posting_ctrl_rec_tab_.exists(req_id_)) THEN
      
      local_ct_value_tab_ := Get_Control_Type___(posting_type_, company_, date_);
                  
      IF (local_ct_value_tab_.COUNT = 0) THEN         
         RAISE not_found;
      END IF;      

      IF (micro_cache_posting_ctrl_rec_tab_.count >= max_cached_rec_element_count_) THEN         
         DECLARE
            random_  NUMBER := NULL;
         ...
         BEGIN
            ...
            micro_cache_posting_ctrl_rec_tab_.delete(element_);
      ...
      END IF;      
      micro_cache_posting_ctrl_rec_tab_(req_id_) := local_ct_value_tab_;
      micro_cache_posting_ctrl_rec_time_ := time_;
   END IF;
   ct_value_tbl_ := micro_cache_posting_ctrl_rec_tab_(req_id_);        
EXCEPTION
   WHEN not_found THEN      
      micro_cache_posting_ctrl_rec_tab_.delete(req_id_);
      micro_cache_posting_ctrl_rec_time_ := time_;
      ct_value_tbl_ := empty_ct_value_tab_;
