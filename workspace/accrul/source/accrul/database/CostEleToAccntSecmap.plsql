...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  220810  SaWmlk  FIZ-18803, Moved error message related to copy elements, from client to Copy_Elements_To_Sec_Map__().
...
IS
BEGIN
   IF (Accounting_Code_Parts_API.Get_Base_For_Followup_Element(company_) != 'A') THEN
      Error_SYS.Record_General(lu_name_, 'SECMAPCOPYELEMENTALLOWED: Copying Cost/Revenue Elements to Secondary Mapping is only allowed for Code Part A.');
   END IF;
