...
--  220420  Jadulk   FIZ-16867, Modified in Check_Common___ to handle null values for missed mandotory columns.
--  221219  Jadulk   FIZ-21840, Added Tax_Code_In_Source_Tax___ procedure to improve the performance on validating records of the Tax Codes page.
-----------------------------------------------------------------------------
...
BEGIN   
   ...
   IF (NOT remrec_.valid_from IS NULL) THEN      
      ...
      Tax_Code_In_Source_Tax___(remrec_.company, remrec_.fee_code);
...
BEGIN
   ...
   newrec_.eu_operation             := NVL(newrec_.eu_operation, 'FALSE');
   newrec_.tax_character            := NVL(newrec_.tax_character, 'NORMAL');
   IF (App_Context_SYS.Find_Value(lu_name_||'.copy_to_company_', 'FALSE') != 'TRUE') THEN
...
-- This procedure has been introduced specifically for validationg tax codes used in Customer Order Lines.
PROCEDURE Tax_Code_In_Source_Tax___ (
   company_  IN VARCHAR2,
   tax_code_ IN VARCHAR2 )
IS
   codes_   NUMBER;
   
   CURSOR check_tax_codes IS
      SELECT 1
      FROM   SOURCE_TAX_ITEM_TAB
      WHERE  tax_code = tax_code_
      AND    company  = company_
      AND source_ref_type = Tax_Source_API.DB_CUSTOMER_ORDER_LINE;
BEGIN  
   OPEN check_tax_codes;
   FETCH check_tax_codes INTO codes_;
   IF (check_tax_codes%FOUND) THEN
      CLOSE check_tax_codes;
      Error_SYS.Record_General(lu_name_, 'SOURCETAXEXISTS: The Statutory Fee :P1 is used by another object( :P2 ).', tax_code_, Tax_Source_API.Decode(Tax_Source_API.DB_CUSTOMER_ORDER_LINE));
   END IF;
   CLOSE check_tax_codes;
END Tax_Code_In_Source_Tax___;
