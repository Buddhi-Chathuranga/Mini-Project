...
--  220706  reanpl  FIDEV-10562, Removed parameter in call to Tax_Book_API.Get_Tax_Book_Info
...
BEGIN
   ...
   IF ((Company_Localization_Info_API.Get_Parameter_Value_Db(rowrec_.company, 'SAFT_POLAND') = Fnd_Boolean_API.DB_TRUE OR 
        Company_Localization_Info_API.Get_Parameter_Value_Db(rowrec_.company, 'HU_VAT_CONTROL_STATEMENT') = Fnd_Boolean_API.DB_TRUE)) THEN 
      taxrec_.text            := rowrec_.text; 
      taxrec_.party_type_id   := rowrec_.party_type_id;
      taxrec_.party_type      := rowrec_.party_type;
   END IF;
...
BEGIN   
   ...
   Approve_Workflow_Edit_Error___(newrec_);
END Check_Common___;
...
BEGIN
   ...
   ELSE
      ...
      newrec_.tax_base_amount               := ROUND(newrec_.tax_base_amount, base_curr_rounding_);
      newrec_.currency_tax_base_amount      := ROUND(newrec_.currency_tax_base_amount, trans_curr_rounding_);
      newrec_.parallel_curr_tax_base_amount := ROUND(newrec_.parallel_curr_tax_base_amount, third_curr_rounding_);
   END IF;
   newrec_.year_period_key := newrec_.accounting_year*100 + newrec_.accounting_period;
...
BEGIN
   ...
   END IF;   
   Approve_Workflow_Edit_Error___(remrec_);
   super(remrec_);
END Check_Delete___;
...
PROCEDURE Approve_Workflow_Edit_Error___ (
   newrec_ IN voucher_row_tab%ROWTYPE )
IS
  state_    VARCHAR2(100); 
BEGIN
   IF (Voucher_Candidate_Approver_API.Get_Approver_Count__( newrec_.company, newrec_.accounting_year, newrec_.voucher_type, newrec_.voucher_no)> 0) THEN
      ...
      END IF;
   END IF;
END Approve_Workflow_Edit_Error___;
...
BEGIN
   ...
   WHILE (TRUE) LOOP
      ...
      $IF Component_Intled_SYS.INSTALLED $THEN      
         Internal_Hold_Voucher_Row_API.Update_Trans_Code(company_, voucher_type_, voucher_no_, accounting_year_, lu_rec_.row_no );
      $END
