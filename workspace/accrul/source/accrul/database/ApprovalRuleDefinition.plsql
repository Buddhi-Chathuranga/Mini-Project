...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  210816  Jadulk  FI21R2-3207, Modified code in Insert_Approvers_To_Voucher__.
--  220725  Jadulk  FIZ-17580, Modified code in Get_Relevant_Rule_For_Vou__.
-----------------------------------------------------------------------------
...
BEGIN
   ...
   IF ((voucher_no_ = 0 AND approval_rule_id_ IS NOT NULL) OR (voucher_no_ >0)) THEN
...
BEGIN
   IF (Voucher_Type_API.Get_Use_Approval_Workflow(company_,voucher_type_) = 'TRUE') THEN     
      IF(Voucher_API.Is_Cancellation_Voucher__(company_, voucher_type_, accounting_year_, voucher_no_) = 'TRUE')THEN
         voucher_debit_sum_ := ABS(Voucher_API.Get_Sum_For_Cancel_Vou(company_, accounting_year_, voucher_type_, voucher_no_));     
      ELSE
