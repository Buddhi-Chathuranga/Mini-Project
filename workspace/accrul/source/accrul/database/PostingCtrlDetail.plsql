...
--  220729  NEKOLK  AMDEV-4321:Restructuring of the Resource/Employee Setup development work 
--                  AMDEV-4983:made changes in Validate_Control_Type_Value__, added MAINT_PERSON_RESOURCE_API call .
-----------------------------------------------------------------------------
...
BEGIN
   ...
   IF (pkg_name_ = 'WO_TIME_TYPE_API') THEN
      procedure_name_:= 'Exist_Posting_Ctrl';
   END IF;   
   --Used by MSC1- to Old resource detail setup
   IF (pkg_name_ = 'MAINT_PERSON_EMPLOYEE_API') THEN
      procedure_name_ := 'Person_Employee_Exist';
   END IF;
   
   --Used by MSC1
   IF (pkg_name_ = 'MAINT_PERSON_RESOURCE_API') THEN
      procedure_name_ := 'Person_Employee_Exist';
   END IF;
   ...
   IF (pkg_name_ = 'INVENT_POSTING_CTRL_API') THEN
      procedure_name_:= 'Control_Type_Value_Exist';
   END IF;
   IF (pkg_name_ = 'JT_TASK_ACCOUNTING_API') THEN
      procedure_name_:= 'Exist_Company';
   END IF; 
