...
--  210728  Jadulk  FI21R2-3207, Added Remove_Vou_Approve_Rule__, Get_Credit_Debit_Totals and Set_Approver_Rule.
--  210729  Jadulk  FI21R2-3207, Modified Insert_Approve_Details__.
--  210816  Jadulk  FI21R2-3207, Modified Set_Approver_Rule and Check_Update___.
--  220121  Alwolk  FI21R2-7763, Merged Bug 161589, Corrected in Voucher_End() Cancel_ section.
--  220315  Alwolk  FIDEV-9480, Merged Bug 162744, Corrected in Balance_Parallel_Diff___().
--  220617  Jadulk  FIZ-17696, Modified Cancel Approve__() to support interim voucher functionality.
--  220621  Jadulk  FIZ-17696, Overrode Ready_Approve__(), Ready_To_Update__() and Delay_Update__() to support interim voucher functionality.
--  220704  Jadulk  FIZ-17696, Added Validate_Voucher_State___(), Approved_To_Partial_Ok___() to support interim voucher functionality.
--  220725  Jadulk  FIZ-17580, Added Get_Sum_For_Cancel_Vou().
--  220804  Jadulk  FIDEV-12014, Modified Ready_To_Update__().
--  220812  Jadulk  FIDEV-12251, Modified Voucher_End().
--  220909  supwlk  FIZ-18875, Modified Interim_Voucher().
-----------------------------------------------------------------------------
...
IS
   ...
   CURSOR get_voucher_row (any_line_ IN VARCHAR2) IS
      SELECT
         ...
         rowid, company, voucher_date, row_no,
   ...
   IF (function_group_ NOT IN ('0', 'A', 'B', 'C', 'CB', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'M', 'N', 'P', 'PPC', 'Q', 'R', 'U', 'X', 'YE', 'Z')) THEN
      ...
      IF (par_curr_balance_ != 0) THEN
         ...
         IF found_row_ THEN  
            ...
            Internal_Hold_Voucher_Row_API.Balance_Parallel_Diff_For_Il(company_,
                                                                       voucher_type_,
                                                                       accounting_year_,
                                                                       voucher_no_,
                                                                       row_rec_.row_no,
                                                                       row_rec_.third_currency_debit_amount,
                                                                       row_rec_.third_currency_credit_amount,
                                                                       row_rec_.parallel_currency_rate);  
            $END
         END IF;
      END IF;                                          
   END IF;
...
BEGIN
   ...
   Validate_Balance_Row_Exist___(rec_);
...
IS
   user_group_of_user_  user_group_member_finance_tab.user_group%type;
BEGIN
   Validate_Voucher_State___(rec_, 'Waiting');
   user_group_of_user_ := User_Group_Member_Finance_API.Get_User_Group (rec_.company,
...
IS
BEGIN
   Validate_Voucher_State___(rec_, 'AwaitingApproval');
   Validate_Balance_Row_Exist___(rec_);
   RETURN TRUE;
END Awaiting_Ok___;


FUNCTION Partially_Approved_Ok___ (
   rec_  IN VOUCHER_TAB%ROWTYPE ) RETURN BOOLEAN
IS
BEGIN
   Validate_Voucher_State___(rec_, 'PartiallyApproved');
   RETURN TRUE;
END Partially_Approved_Ok___;
...
BEGIN
   ...
   IF (acc_balance_ != 0) THEN
      Error_SYS.Record_General( 'Voucher', 'VOUNOTBALANCED: The Voucher :P1 is Not Balanced', rec_.voucher_no );
   END IF;  

   IF (par_curr_balance_ != 0) THEN
      Error_SYS.Record_General( 'Voucher', 'VOUNOTBALANCEDPAR: The Voucher :P1 is Not Balanced in parallel currency', rec_.voucher_no );
   END IF;  
...
BEGIN   
   ...
   IF (newrec_.approval_rule_id IS NOT NULL) THEN
      IF ((Validate_SYS.Is_Changed(oldrec_.two_approver_required,newrec_.two_approver_required)) OR (Validate_SYS.Is_Changed(oldrec_.sequential_approval, newrec_.sequential_approval))) THEN
         Error_SYS.Record_General(lu_name_, 'APPROVALDISABLED: Approval parameters can be modified only when an assigning rule is not applied.');
      END IF;
   END IF;
...
-- Note: Check whether row exist and balanced.
PROCEDURE Validate_Balance_Row_Exist___(
   rec_     IN voucher_tab%ROWTYPE)
IS
BEGIN
   Check_No_Rows_In_Voucher___(rec_);
   Validate_Voucher_In_Balance___(rec_);
END Validate_Balance_Row_Exist___;

PROCEDURE Check_No_Rows_In_Voucher___(
   rec_     IN voucher_tab%ROWTYPE)
IS
   row_exist_  NUMBER;
...
BEGIN   	  
   OPEN find_vou_rows;
   FETCH find_vou_rows INTO row_exist_;
   ...
   IF (NOT found_) THEN
      Error_SYS.Record_General( lu_name_, 'NOVOUCHERROW: There are no voucher rows in current voucher');
   END IF;
END Check_No_Rows_In_Voucher___;   


PROCEDURE Validate_Voucher_State___(
   rec_                 IN  voucher_tab%ROWTYPE,
   new_voucher_status_  IN  VARCHAR2)
IS
BEGIN 
   IF ( rec_.interim_voucher = 'Y' AND (rec_.function_group IN ('M', 'K', 'Q') AND NOT((rec_.rowstate = 'Error' AND new_voucher_status_ = 'Confirmed') OR (rec_.voucher_updated = 'Y'))) 
                                   OR (rec_.function_group = 'R' AND rec_.rowstate = 'Confirmed' AND new_voucher_status_ IN ('Waiting', 'AwaitingApproval','Error'))) THEN
      Error_SYS.Record_General(lu_name_, 'ISINTERIM: The voucher is connected to an interim voucher and cannot be changed.');
   END IF;
END Validate_Voucher_State___;  
...
IS
   ...
   CURSOR get_vou IS
      SELECT rowid, to_char(rowversion,'YYYYMMDDHH24MISS'), user_group, function_group, rowstate, ignore_approve_workflow
      FROM   voucher_tab
      WHERE  transfer_id  = transfer_id_;
...
BEGIN
   ...
   ELSE
      super(info_, objid_, objversion_, attr_, action_);
   END IF;
   IF( NOT Is_Approval_Workflow___(rec_, attr_))THEN
      IF action_ = 'DO' THEN
         rec_ := Get_Object_By_Id___(objid_);
...
BEGIN
   ...
   IF (NOT manual_found_) THEN      
      ...
      IF (cancel_option_) THEN
         ...
         END;
      ELSE
         IF (is_approval_workflow_ = 'TRUE' OR voucher_type_rec_.separate_user_approval = 'TRUE') THEN
            Ready_Approve__( info_save_,
                             objid_,
                             objversion_,
                             attr_,
      ...
      IF (is_approval_workflow_ != 'TRUE' AND authorize_level_ = 'Approved' AND voucher_type_rec_.separate_user_approval != 'TRUE' ) THEN  
         UPDATE   voucher_tab
            SET   approval_date           = SYSDATE,
...
BEGIN
   ...
   $IF Component_Intled_SYS.INSTALLED $THEN
      ...
      Internal_Hold_Voucher_API.Update_Interim_Voucher(company_, voucher_type_, voucher_no_, accounting_year_ );                                                    
      END IF;
   $END
...
BEGIN
   ...
   ELSE
      CLOSE get_rows_no;
      rec_  := Get_Object_By_Keys___(company_, accounting_year_, voucher_type_, voucher_no_);
      IF (rec_.rowstate IN (Voucher_API.DB_CONFIRMED, Voucher_API.DB_AWAITING_APPROVAL)) THEN
         Validate_Balance_Row_Exist___(rec_);
      END IF;
      result_ := 'TRUE';
   END IF;
...
FUNCTION Get_Sum_For_Cancel_Vou (
   company_            IN VARCHAR2,
   accounting_year_    IN NUMBER,
   voucher_type_       IN VARCHAR2,
   voucher_no_         IN NUMBER ) RETURN NUMBER
IS
   ...
   CURSOR get_amount_ IS
      ...
      AND   v.accounting_Year = accounting_year_
      AND   v.voucher_No      = voucher_no_
      AND (trans_code IN ('CANCELLATION-MANUAL', 'CANCELLATION-EXTERNAL', 'CANCELLATION-External', 'CANCELLATION-AP1', 'CANCELLATION-AP2', 'CANCELLATION-AP3', 'CANCELLATION-AP4') OR trans_code LIKE '%CANCELLATION-MANUAL' );
...
IS
   new_credit_total_  NUMBER;
   new_debit_total_   NUMBER;

BEGIN
   Get_Credit_Debit_Totals(new_credit_total_, new_debit_total_, company_, voucher_type_, accounting_year_, voucher_no_);

   IF ((NVL(old_debit_total_,0) != new_debit_total_) OR (NVL(old_credit_total_,0) != new_credit_total_))THEN     
      Approval_Rule_Definition_API.Insert_Approvers_To_Voucher__(company_,
                                                                 accounting_year_,
                                                                 voucher_type_,
                                                                 voucher_no_);
   END IF;
END Set_Approver_Rule;

PROCEDURE Remove_Vou_Approve_Rule__(
   company_                IN VARCHAR2,
   accounting_year_        IN NUMBER,
   voucher_type_           IN VARCHAR2,
   voucher_no_             IN NUMBER)
IS
   ...
   newrec_    voucher_tab%ROWTYPE;
BEGIN
   oldrec_ := Lock_By_Keys___(company_, accounting_year_, voucher_type_, voucher_no_);
   newrec_ := oldrec_;
   
   newrec_.approval_rule_id := NULL;  
   Modify__(newrec_);
END Remove_Vou_Approve_Rule__;

@UncheckedAccess
PROCEDURE Get_Credit_Debit_Totals(
   credit_total_            OUT NUMBER,
   debit_total_             OUT NUMBER,
   company_                 IN VARCHAR2,
   voucher_type_            IN VARCHAR2,
   accounting_year_         IN NUMBER,
   voucher_no_              IN NUMBER)
IS 
   CURSOR get_totals IS
      SELECT SUM(NVL(debet_amount,0))  sum_cur_debit, 
             SUM(NVL(credit_amount,0)) sum_cur_credit
      FROM Accounting_Code_Part_Value_Tab a,
           Voucher_Row_Tab v
      WHERE a.company         = v.company
      AND   a.code_part       = 'A'
...
BEGIN
   OPEN get_totals;
   FETCH get_totals INTO debit_total_, credit_total_;
   CLOSE get_totals;
END Get_Credit_Debit_Totals;
