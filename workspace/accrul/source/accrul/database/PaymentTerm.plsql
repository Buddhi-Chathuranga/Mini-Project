...
--  210402  Gevelk  FI21R2-499, Merged LCS bug 158590: Calling Enterp_Comp_Connect_V170_API.Insert_Company_Translation for the instance of new record creation 
--                  in order to correct erroneous translation created for the logged in session language.
...
-------------------- PUBLIC DECLARATIONS ------------------------------------
TYPE Pay_Installment_Data_Tab IS TABLE OF Pay_Installment_Data_Rec INDEX BY BINARY_INTEGER;

TYPE Pay_Installment_Data_Rec IS RECORD
...
BEGIN
   ...
   IF (source_rec_.rowversion IS NOT NULL AND old_target_rec_.rowversion IS NULL) THEN
      ...
      New___(target_rec_);
      -- Correcting erroneous translation created for the logged in session language. This cannot be moved inside Copy_Comp_To_Comp_Trans since it has to execute only when new record is created.
      Enterp_Comp_Connect_V170_API.Insert_Company_Translation(target_company_, 
                                                              module_, 
                                                              lu_name_,
                                                              pay_term_id_,
                                                              NVL(Fnd_Session_API.Get_Language, 'en'),
                                                              Enterp_Comp_Connect_V170_API.Get_Company_Translation(source_company_, 
                                                                                                                   module_, 
                                                                                                                   lu_name_,
                                                                                                                   pay_term_id_),
                                                              target_rec_.description);
...
PROCEDURE Get_Installment_Data (
   ...
   date_               IN DATE )
IS
   pay_installment_data_tab_ Pay_Installment_Data_Tab;
BEGIN
   Get_Installment_Data(pay_installment_data_tab_,
                        company_,
                        identity_,
                        party_type_db_,
                        pay_term_id_,
   ...
   IF (pay_installment_data_tab_.count > 0) THEN
      FOR count_ IN pay_installment_data_tab_.FIRST..pay_installment_data_tab_.LAST LOOP
         Client_SYS.Add_To_Attr('INSTALLMENT_ID', pay_installment_data_tab_(count_).installment_id, attr_);
         Client_SYS.Add_To_Attr('DUE_DATE', pay_installment_data_tab_(count_).due_date, attr_);
         Client_SYS.Add_To_Attr('CURR_AMOUNT', pay_installment_data_tab_(count_).curr_amount, attr_);
      END LOOP;  
   END IF;   
...
PROCEDURE Get_Installment_Data (
   pay_installment_data_tab_       OUT   Pay_Installment_Data_Tab,
   company_                        IN    VARCHAR2,
   identity_                       IN    VARCHAR2,
   party_type_db_                  IN    VARCHAR2,
   pay_term_id_                    IN    VARCHAR2,
...
IS
   ...
   rounding_              NUMBER;
   calc_due_date_         DATE;
   tax_distribution_db_   VARCHAR2(50);
   percentage_            NUMBER;
   amount_                NUMBER;
...
BEGIN
   ...
   END IF;
   tax_distribution_db_ := Payment_Term_API.Get_Vat_Distribution_Db(company_, pay_term_id_);
   ...
   FOR rec_ IN get_pay_term_details LOOP
      ...
      IF (tax_distribution_db_= 'EVEN') THEN   -- Even
         amount_ := (net_curr_amount_+ tax_curr_amount_)* (percentage_ / 100);
         net_amount_ := net_curr_amount_* (percentage_ / 100);
      ELSIF (tax_distribution_db_= 'ALLTAXONFIRSTINST') THEN  -- All on first installment
         IF (count_rec_ = 1) THEN
            net_amount_ := net_curr_amount_* (percentage_ / 100);
            amount_ := net_amount_ + nvl(tax_curr_amount_,0);                        
         ELSE
            amount_ := net_curr_amount_* (percentage_ / 100);
            net_amount_ := amount_;            
         END IF;
      ELSIF (tax_distribution_db_= 'FIRSTINSTONLYTAX') THEN  -- First installment only Tax
         IF (count_rec_ = 1) THEN
            amount_ := nvl(tax_curr_amount_,0);
            net_amount_ := 0;            
         ELSE
            amount_ := net_curr_amount_* (percentage_ / 100);
            net_amount_ := amount_;            
      ...
      net_amount_     := ROUND(net_amount_, rounding_);
      sum_net_amount_ := sum_net_amount_ + net_amount_;

      -- for the last installment adjust amount
      IF count_rec_ = count_pay_det_ THEN
         amount_ := amount_ + ((net_curr_amount_+ tax_curr_amount_) - sum_amount_);
         net_amount_ := net_amount_ + (net_curr_amount_ - sum_net_amount_);         
      END IF;
      IF (nvl(amount_,0) != 0) THEN
         ...
         calc_due_date_ := Payment_Term_Details_API.Calc_Due_Date(
                              ...
                              temp_identity_,
                              party_type_db_);
         pay_installment_data_tab_(count_rec_).installment_id := rec_.installment_number;
         pay_installment_data_tab_(count_rec_).due_date := calc_due_date_;
         pay_installment_data_tab_(count_rec_).net_curr_amount := net_amount_;
         pay_installment_data_tab_(count_rec_).curr_amount := amount_;                  
      END IF;
   END LOOP;
END Get_Installment_Data;
