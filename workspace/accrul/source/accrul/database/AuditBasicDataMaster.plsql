...
IS
BEGIN
   IF indrec_.reporting_currency THEN
      IF newrec_.reporting_currency = Audit_Reporting_Currency_API.DB_ACCOUNTING_CURRENCY THEN
         newrec_.default_currency_code := Currency_Code_API.Get_Currency_Code(newrec_.company);
      ELSIF newrec_.reporting_currency = Audit_Reporting_Currency_API.DB_PARALLEL_CURRENCY THEN
         newrec_.default_currency_code := Company_Finance_API.Get_Parallel_Acc_Currency(newrec_.company);
      END IF;
   END IF;
