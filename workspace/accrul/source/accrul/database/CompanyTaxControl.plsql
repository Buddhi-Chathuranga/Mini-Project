...
--  210728  PraWlk  FI21R2-3290, Added new method Set_Tax_Cal_method_to_Not_Used().
-----------------------------------------------------------------------------
...
BEGIN 
   IF (newrec_.external_tax_cal_method != 'NOT_USED') THEN 
      ...
      END IF;
   END IF;
   IF (newrec_.external_tax_cal_method IN (External_Tax_Calc_Method_API.DB_AVALARA_SALES_TAX, External_Tax_Calc_Method_API.DB_AVALARA_TAX_BRAZIL)) THEN
...
PROCEDURE Set_Tax_Cal_Method_To_Not_Used (
   company_  IN VARCHAR2 )
IS
   newrec_     company_tax_control_tab%ROWTYPE;
BEGIN
   newrec_ := Get_Object_By_Keys___(company_);
   newrec_.external_tax_cal_method := External_Tax_Calc_Method_API.DB_NOT_USED; 
   Modify___(newrec_);
END Set_Tax_Cal_Method_To_Not_Used;
