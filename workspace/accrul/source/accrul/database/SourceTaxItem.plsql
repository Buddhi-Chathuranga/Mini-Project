...
-- 210706   PraWlk  FI21R2-925, Added cst_code and legal_tax_class to the public interfaces.
-- 211215   Kgamlk  FI21R2-7201, Added tax_category1 and tax_category2 to Source Tax Item.
-- 220103   Kgamlk  FI21R2-7201, Added tax_category1 and tax_category2 to invoice line level.
-----------------------------------------------------------------------------
...
TYPE source_public_rec IS RECORD
   (company                        source_tax_item_tab.company%TYPE,
    ...
    cst_code                       source_tax_item_tab.cst_code%TYPE,
    legal_tax_class                source_tax_item_tab.legal_tax_class%TYPE,
    -- gelr:br_external_tax_integration, end
    tax_category1                  source_tax_item_tab.tax_category1%TYPE,
    tax_category2                  source_tax_item_tab.tax_category2%TYPE);
...
@UncheckedAccess
FUNCTION Get_Line_Tax_Category1 ( 
   company_                      IN  VARCHAR2,
   source_ref_type_              IN  VARCHAR2,
   source_ref1_                  IN  VARCHAR2,
   source_ref2_                  IN  VARCHAR2,
   ...
   source_ref5_                  IN  VARCHAR2 ) RETURN VARCHAR2
IS
   tax_category1_on_line_  source_tax_item_tab.tax_category1%TYPE;
   is_multiple_tax_        VARCHAR2(5);
   CURSOR get_tax_category1_ref IS
      SELECT tax_category1 
      FROM   source_tax_item_tab
      WHERE  company         = company_
      AND    source_ref_type = source_ref_type_
      AND    source_ref1     = source_ref1_
...
BEGIN
   is_multiple_tax_ := Multiple_Tax_Items_Exist(company_, source_ref_type_, source_ref1_, source_ref2_, source_ref3_, source_ref4_, source_ref5_);
   IF (is_multiple_tax_ = Fnd_Boolean_API.DB_TRUE) THEN
      tax_category1_on_line_ := NULL;
   ELSE
      IF (source_ref2_ IS NULL OR source_ref3_ IS NULL OR source_ref4_ IS NULL OR source_ref5_ IS NULL) THEN 
         OPEN  get_tax_category1;
         FETCH get_tax_category1 INTO tax_category1_on_line_;
         CLOSE get_tax_category1;           
      ELSE
         OPEN  get_tax_category1_ref;
         FETCH get_tax_category1_ref INTO tax_category1_on_line_;
         CLOSE get_tax_category1_ref;        
      END IF;
   END IF;
   RETURN tax_category1_on_line_;   
END Get_Line_Tax_Category1;


@IgnoreUnitTest TrivialFunction
@UncheckedAccess
FUNCTION Get_Line_Tax_Category2 ( 
   company_                      IN  VARCHAR2,
   source_ref_type_              IN  VARCHAR2,
   source_ref1_                  IN  VARCHAR2,
   source_ref2_                  IN  VARCHAR2,
   ...
   source_ref5_                  IN  VARCHAR2 ) RETURN VARCHAR2
IS
   tax_category2_on_line_    source_tax_item_tab.tax_category2%TYPE;
   is_multiple_tax_          VARCHAR2(5);
   CURSOR get_tax_category2_ref IS
      SELECT tax_category2
      FROM   source_tax_item_tab
      WHERE  company         = company_
      AND    source_ref_type = source_ref_type_
      AND    source_ref1     = source_ref1_
...
BEGIN
   is_multiple_tax_ := Multiple_Tax_Items_Exist(company_, source_ref_type_, source_ref1_, source_ref2_, source_ref3_, source_ref4_, source_ref5_);
   IF (is_multiple_tax_ = Fnd_Boolean_API.DB_TRUE) THEN
      tax_category2_on_line_ := NULL;
   ELSE
      IF (source_ref2_ IS NULL OR source_ref3_ IS NULL OR source_ref4_ IS NULL OR source_ref5_ IS NULL) THEN 
         OPEN  get_tax_category2;
         FETCH get_tax_category2 INTO tax_category2_on_line_;
         CLOSE get_tax_category2;           
      ELSE
         OPEN  get_tax_category2_ref;
         FETCH get_tax_category2_ref INTO tax_category2_on_line_;
         CLOSE get_tax_category2_ref;        
      END IF;
   END IF;
   RETURN tax_category2_on_line_;   
END Get_Line_Tax_Category2;
...
BEGIN
   ...
   -- gelr:br_external_tax_integration, begin
   outrec_.cst_code                    := inrec_.cst_code;
   outrec_.legal_tax_class             := inrec_.legal_tax_class;
   -- gelr:br_external_tax_integration, end
   outrec_.tax_category1               := inrec_.tax_category1;
   outrec_.tax_category2               := inrec_.tax_category2;
...
BEGIN
   ...
   -- gelr:br_external_tax_integration, begin
   outrec_.cst_code                    := cst_code_;
   outrec_.legal_tax_class             := legal_tax_class_;
   -- gelr:br_external_tax_integration, end
   outrec_.tax_category1               := tax_category1_;
   outrec_.tax_category2               := tax_category2_;
...
BEGIN
   ...
   FOR dummy_ IN 1..count_ LOOP
      ...
      -- gelr:br_external_tax_integration, begin   
      ELSIF (m_s_names_(dummy_) = 'CST_CODE') THEN
         tax_rec_(index_).cst_code := m_s_values_(dummy_);
      ELSIF (m_s_names_(dummy_) = 'LEGAL_TAX_CLASS') THEN
         tax_rec_(index_).legal_tax_class := m_s_values_(dummy_);
      -- gelr:br_external_tax_integration, end
      ELSIF (m_s_names_(dummy_) = 'TAX_CATEGORY1') THEN
         tax_rec_(index_).tax_category1 := m_s_values_(dummy_);
      ELSIF (m_s_names_(dummy_) = 'TAX_CATEGORY2') THEN
         tax_rec_(index_).tax_category2 := m_s_values_(dummy_);
      ELSE
         NULL;
      END IF;
   END LOOP;
...
BEGIN
   IF (Message_SYS.Get_Name(msg_) = 'TAX_INFORMATION') THEN 
      ...
      Message_SYS.Add_Attribute(msg_, 'TAX_CATEGORY1', tax_rec_.tax_category1);
      Message_SYS.Add_Attribute(msg_, 'TAX_CATEGORY2', tax_rec_.tax_category2);
   END IF;
