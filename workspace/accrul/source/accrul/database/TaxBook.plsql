...
--  210910  NWeelk FI21R2-1632, Merged Bug 158941, It should be possible to assign one tax book, tax series and tax number for the customer and supplier invoices.
--  220706  reanpl FIDEV-10562, Removed def_tax_book_ from Get_Tax_Book_Info and Get_Tax_Book_Series_Id___, removed validations from Get_Tax_Book_Info
-----------------------------------------------------------------------------
...
IS
   ...
   CURSOR get_ip10_tax_series_no IS
      SELECT ip10_tax_series_no
      FROM   invoice_item_tab
      WHERE  company = company_
      AND    invoice_id = invoice_id_
      AND    ip10_tax_series_no IS NOT NULL;
   $END 	
BEGIN
   ...
   $IF Component_Taxled_SYS.INSTALLED $THEN
      ...
      END IF;      
   $END
   IF (fee_type_ = Fee_Type_API.DB_CALCULATED_TAX AND party_type_ = Party_Type_API.DB_SUPPLIER) THEN
      -- Fetch the applicable tax book
      ip10_tax_book_id_ := Get_Tax_Book_For_Tax_Code___(company_, fee_code_, Tax_Direction_Sp_API.DB_DISBURSED);
      IF (ip10_tax_book_id_ IS NULL) THEN
         ip10_tax_book_id_ := Get_Tax_Book_For_Inv_Type___(company_, party_type_, invoice_type_, Tax_Direction_Sp_API.DB_DISBURSED);
      END IF;
      IF (ip10_tax_book_id_ IS NULL) THEN
         ip10_tax_book_id_ := Get_Tax_Book_For_Inv_Series___(company_, inv_series_id_, Tax_Direction_Sp_API.DB_DISBURSED);
      END IF;
      -- Fetch the applicable tax series
      IF (ip10_tax_book_id_ IS NOT NULL) THEN
         ip10_tax_series_id_ := Get_Tax_Series_Id(company_, ip10_tax_book_id_);
         IF (ip10_tax_series_id_ IS NULL) THEN
            Error_SYS.Record_General(lu_name_, 'NOTAXSERIES: Tax Book does not have a Tax Series.');
         END IF;
      END IF;
      -- Fetch the next tax series no
      $IF Component_Invoic_SYS.INSTALLED AND Component_Taxled_SYS.INSTALLED $THEN
         OPEN get_ip10_tax_series_no;
         FETCH get_ip10_tax_series_no INTO ip10_tax_series_no_;
         IF (get_ip10_tax_series_no%NOTFOUND) THEN
            ip10_tax_series_no_ := Tax_Number_Series_API.Get_Next_Number(company_, ip10_tax_series_id_, tax_trans_date_);   
         END IF;
         CLOSE get_ip10_tax_series_no;
      $ELSE
         NULL;
      $END
   END IF;
   IF (fee_type_ != Fee_Type_API.DB_CALCULATED_TAX AND tax_series_no_ IS NULL) OR (fee_type_ = Fee_Type_API.DB_CALCULATED_TAX AND ip10_tax_series_no_ IS NULL) THEN
...
BEGIN
   ...
   Get_Tax_Book_Info___(tax_book_id_, tax_series_id_, tax_series_no_, ip10_tax_book_id_, ip10_tax_series_id_, ip10_tax_series_no_, company_, fee_code_, tax_trans_date_, party_type_, invoice_type_, direction_ , inv_series_id_, invoice_id_, ref_item_id_, tax_rec_.fee_type);
...
BEGIN
   $IF Component_Invoic_SYS.INSTALLED $THEN
      ...
      END IF;
      tax_series_id_ := Get_Tax_Book_Series_Id___(company_, party_type_ , invoice_type_, fee_code_ , inv_series_id_, direction_);
      IF (party_type_ = Party_Type_API.DB_SUPPLIER) THEN
         tax_rec_ := Statutory_Fee_API.Fetch_Validate_Tax_Code_Rec (company_, fee_code_, SYSDATE, 'FALSE', 'FALSE', 'FETCH_AND_VALIDATE');
         IF (tax_rec_.fee_type = Fee_Type_API.DB_CALCULATED_TAX) THEN
            ip10_tax_series_id_ := Get_Tax_Book_Series_Id___(company_, party_type_ , invoice_type_, fee_code_ , inv_series_id_, Tax_Direction_Sp_API.DB_DISBURSED);
         END IF;
      END IF;
   $ELSE
      NULL;
