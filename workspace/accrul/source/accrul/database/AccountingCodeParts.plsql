...
BEGIN
   IF code_part_ IS NOT NULL THEN
      IF code_part_ IN ('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J') THEN
         IF ( number_of_char_ < 1 OR number_of_char_ > 10 ) THEN
            Error_SYS.Appl_General(lu_name_, 'INVALMAXNO10: Max number of characters must be between 1 and 10.');
         END IF;
      END IF;
   END IF;
...
BEGIN    
   ...
   Val_Max_Number_Of_Char___(newrec_.code_part, newrec_.max_number_of_char);
