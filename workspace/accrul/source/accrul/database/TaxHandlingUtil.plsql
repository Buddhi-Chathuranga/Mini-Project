...
--  210710  Dakplk  FI21R2-2075, Merged Bug 159689, Modified the record type tax_assistant_label_rec to increase the lengths. 
--  210910  MalLlk  SC21R2-1980, Added parameter part_move_tax_direction_db to record type fetch_default_key_rec and parameter to Create_Fetch_Default_Key_Rec,
--  210910          to set that value. Modified Fetch_Tax_Code_On_Object___ to fetch the tax code from INVENT when object type is 'INV_MOVE_PART'.
--  210923  PraWlk  FI21R2-4180, Removed default NULL from parameters cst_code and legal_tax_class in Add_Tax_Code_Info() and passed NULL for 
--  210923          those two parameters from the calling methods accordingly.   
--  211215  Kgamlk  FI21R2-7201, Added tax_category1 and tax_category2 to Source Tax Item.
--  211216  HasTlk  SC21R2-6456, Added TAX_DOCUMENT_LINE_API into Fetch_For_Tax_Line_Assistant function
--  220103  Paralk  FI21R2-7654, Handled visibility of Tax Category 1 and Tax Category 2
--  220201  HasTlk  SC21R2-7281, Added TAX_DOCUMENT_LINE_API into Save_From_Tax_Line_Assistant
--  220519  ShKolk  FIDEV-9051, Added FISCAL_NOTE_HEAD_API into Fetch_For_Tax_Line_Assistant
--  220719  MaEelk  SCDEV-12653, Modified Field_Visible_Tax_Line_Assis and added parameter source_key_rec_ to pass it to
--  220919          Tax_Handling_Discom_Util_API.Field_Visible_Tax_Line_Assis.
--  220815  DhAplk  Modified Field_Visible_Tax_Line_Assis to handle visible lines when package_name is FISCAL_NOTE_HEAD_API.
-----------------------------------------------------------------------------
...
TYPE tax_information_rec IS RECORD                  
   (tax_code                     VARCHAR2(20),
    ...
    tax_category1                VARCHAR2(20),
    tax_category2                VARCHAR2(20));
...
-- INVENTORY TRANSACTION HIST - INV_MOVE_PART
...
TYPE fetch_default_key_rec IS RECORD (
   ...
   part_move_tax_direction_db VARCHAR2(8)); -- only for invent module. values: SENDER/RECEIVER
...
TYPE tax_assistant_label_rec IS RECORD ( 
   tax_lines_label                      VARCHAR2(100),
   list_tax_percentage_label            VARCHAR2(100),
   list_ded_percentage_label            VARCHAR2(100),
   list_tax_amount_label                VARCHAR2(100),
   list_tax_dom_amount_label            VARCHAR2(100),
   list_tax_parallel_amt_label          VARCHAR2(100),   
   list_tax_base_curr_amt_label         VARCHAR2(100),
   list_tax_base_dom_amount_label       VARCHAR2(100),
   list_tax_base_para_amt_label         VARCHAR2(100),
   list_non_ded_tax_amt_label           VARCHAR2(100),
   list_non_ded_tax_dom_amt_lbl         VARCHAR2(100),
   list_non_ded_tax_para_amt_lbl        VARCHAR2(100),
   list_total_tax_curr_amt_label        VARCHAR2(100),
   list_total_tax_dom_amt_label         VARCHAR2(100),
   list_total_tax_para_amount_label     VARCHAR2(100),
   list_cst_code_label                  VARCHAR2(100),
   group_gross_curr_amount_label        VARCHAR2(100),   
   group_net_curr_amount_label          VARCHAR2(100),
   group_vat_curr_amount_label          VARCHAR2(100),
   group_non_ded_curr_amt_label         VARCHAR2(100),
   group_total_tax_curr_amt_lbl         VARCHAR2(100),
   group_cost_curr_amount_label         VARCHAR2(100));        
...
TYPE tax_assis_field_visibility_rec IS RECORD (
   ...
   cst_code_visible                 VARCHAR2(5),
   legal_tax_class_visible          VARCHAR2(5),
   tax_type_category_visible        VARCHAR2(5),
   tax_category_visible             VARCHAR2(5));         
...
BEGIN
   ...
   $IF Component_Invent_SYS.INSTALLED $THEN
      IF (tax_source_object_rec_.object_type = 'INV_MOVE_PART') THEN
         Tax_Handling_Invent_Util_API.Fetch_Tax_Code_On_Object(tax_source_object_rec_, company_, identity_rec_.supply_country_db, identity_rec_.delivery_country_db, tax_liability_date_, fetch_default_key_rec_.part_move_tax_direction_db);
      END IF;
   $END
...
BEGIN
   ...
   ELSE
      FOR i IN 1 .. default_tax_info_table_.COUNT LOOP
         Add_Tax_Code_Info(tax_info_table_, company_, identity_rec_.party_type_db, default_tax_info_table_(i).tax_code, NULL, NULL, validation_rec_.fetch_validate_action, NULL, tax_liability_date_, NULL, NULL);
      END LOOP;      
   END IF;   
...
BEGIN
   ...
   ELSE   
      ...
      FOR i IN 1 .. default_tax_info_table_.COUNT LOOP
         Add_Tax_Code_Info(tax_info_table_, company_, party_type_db_, default_tax_info_table_(i).tax_code, tax_calc_structure_id_, default_tax_info_table_(i).tax_calc_structure_item_id, validation_rec_.fetch_validate_action, NULL, tax_liability_date_, NULL, NULL);
      END LOOP; 
   END IF;
...
BEGIN
   ...
   IF (tax_free_tax_code_ IS NOT NULL) THEN    
      Add_Tax_Code_Info(tax_info_table_, company_, identity_rec_.party_type_db, tax_free_tax_code_, NULL, NULL, fetch_validate_action_, NULL, tax_liability_date_, NULL, NULL);    
   END IF;    
   ...
   IF (Message_SYS.Get_Name(tax_code_msg_) = 'TAX_CODES') THEN
      ...
      FOR i IN 1..count_  LOOP
         ...
         ELSIF (m_s_names_(i) = 'TAX_PERCENTAGE') THEN
            ...
            Add_Tax_Code_Info(tax_info_table_, company_, party_type_db_, tax_code_, tax_calc_structure_id_, tax_calc_structure_item_id_, fetch_validate_action_, tax_percentage_, tax_liability_date_, NULL, NULL);
         END IF;       
...
BEGIN
   IF (tax_code_ IS NOT NULL) THEN
      IF (saved_tax_code_ IS NOT NULL AND saved_tax_code_ = tax_code_) THEN
         Add_Tax_Code_Info(tax_info_table_, company_, party_type_db_, saved_tax_code_, NULL, NULL, validation_rec_.fetch_validate_action, saved_tax_percentage_, tax_liability_date_, NULL, NULL);
         IF (tax_info_table_.COUNT = 1) THEN
            tax_info_table_(1).tax_base_curr_amount := saved_tax_base_curr_amount_;
         END IF;
      ELSE
         Add_Tax_Code_Info(tax_info_table_, company_, party_type_db_, tax_code_, NULL, NULL, validation_rec_.fetch_validate_action, NULL, tax_liability_date_, NULL, NULL);
      END IF;
   ELSE
...
IS
   ...
   br_tax_percentage_      NUMBER := 0;
BEGIN
   IF (tax_info_table_(index_).tax_calc_structure_id IS NOT NULL) THEN
   ...
   ELSE
      FOR i IN 1 .. tax_info_table_.COUNT LOOP
         tax_in_tax_base_ := Statutory_Fee_API.Get_Tax_In_Tax_Base_Db(company_, tax_info_table_(i).tax_code);
         IF (tax_in_tax_base_ = Fnd_Boolean_API.DB_TRUE) THEN
            br_tax_percentage_ := br_tax_percentage_ + tax_info_table_(i).tax_percentage;
         END IF;
      END LOOP;
      tax_calc_base_amount_ := (tax_calc_base_amount_ * 100) / (100 - br_tax_percentage_);
   END IF;
...
BEGIN
   ...
   IF (tax_source_object_rec_.is_taxable_db = Fnd_Boolean_API.DB_FALSE) THEN
      IF (tax_source_object_rec_.object_tax_code IS NOT NULL) THEN    
         Add_Tax_Code_Info(tax_info_table_, company_, identity_rec_.party_type_db, tax_source_object_rec_.object_tax_code, NULL, NULL, validation_rec_.fetch_validate_action, NULL, tax_liability_date_, NULL, NULL);    
      END IF; 
   ...
   FOR i IN 1 .. tax_info_table_.COUNT LOOP
      IF (tax_info_table_(i).tax_curr_amount IS NULL) THEN
         IF (tax_info_table_(i).tax_calc_structure_id IS NULL) THEN   
            Set_Tax_Calc_Base_Info___(tax_calc_base_percent_, tax_calc_base_amount_, use_tax_calc_base_amount_, tax_info_table_, line_amount_rec_, company_, i);
         ELSE
...
FUNCTION Create_Fetch_Default_Key_Rec (
   ...
   attr_                       IN VARCHAR2,
   connect_charge_seq_no_      IN NUMBER,
   part_move_tax_direction_db_ IN VARCHAR2 ) RETURN fetch_default_key_rec
IS 
...
BEGIN
   ...
   fetch_default_key_rec_.part_move_tax_direction_db := part_move_tax_direction_db_;
   ...
   IF (tax_info_table_.COUNT > 0) THEN        
      ...
      FOR i IN 1 .. tax_info_table_.COUNT LOOP
         IF (Message_SYS.Get_Name(tax_msg_) = 'TAX_INFORMATION') THEN
            ...
            Message_SYS.Add_Attribute(tax_msg_, 'CST_CODE',                    tax_info_table_(i).cst_code);
            Message_SYS.Add_Attribute(tax_msg_, 'LEGAL_TAX_CLASS',             tax_info_table_(i).legal_tax_class);
            -- gelr:br_external_tax_integration, end
            Message_SYS.Add_Attribute(tax_msg_, 'TAX_CATEGORY1',               tax_info_table_(i).tax_category1);
            Message_SYS.Add_Attribute(tax_msg_, 'TAX_CATEGORY2',               tax_info_table_(i).tax_category2);
...
-- gelr:br_external_tax_integration, added cst_code_, legal_tax_class_
...
BEGIN
   ...
   IF (tax_code_rec_.fee_code IS NOT NULL) THEN
      ...
      tax_info_table_(index_).tax_category1              := tax_code_rec_.tax_category1;
      tax_info_table_(index_).tax_category2              := tax_code_rec_.tax_category2;
      -- gelr:extend_tax_code_and_tax_struct, begin
      IF (Company_Localization_Info_API.Get_Parameter_Value_Db(company_, 'EXTEND_TAX_CODE_AND_TAX_STRUCT') = Fnd_Boolean_API.DB_TRUE) THEN
      ...
      -- gelr:extend_tax_code_and_tax_struct, end
      -- gelr:br_external_tax_integration, begin
      IF (Company_Localization_Info_API.Get_Parameter_Value_Db(company_, 'BR_EXTERNAL_TAX_INTEGRATION') = Fnd_Boolean_API.DB_TRUE) THEN
         tax_info_table_(index_).cst_code        := cst_code_;
         tax_info_table_(index_).legal_tax_class := legal_tax_class_;
      END IF;
      -- gelr:br_external_tax_integration, end
   END IF;
...
BEGIN 
   ...
   ELSIF (package_name_ IN ('PURCH_CHANGE_ORDER_CHARGE_API', 'PURCHASE_ORDER_CHARGE_API', 'PURCH_ORDER_LINE_PART_API', 'PURCHASE_CHANGE_ORDER_LINE_API', 'PURCH_ORDER_LINE_NOPART_API', 'PURCHASE_REQUISITION_LINE_API', 'PURCHASE_REQUISITION_LINE_NOPART_API', 'PURCHASE_CHANGE_ORDER_LINE_NOPART_API', 'PURCHASE_QUOTATION_APPROVAL_LINE_API', 'PURCHASE_QUOTATION_LINE_ORDER_API', 'PURCHASE_QUOTATION_LINE_NOPART_ORDER_API')) THEN   
      ...
      $ELSE
         NULL;
      $END
   ELSIF (package_name_ IN ('TAX_DOCUMENT_LINE_API')) THEN   
      $IF Component_Discom_SYS.INSTALLED $THEN 
         source_info_rec_ :=  Tax_Handling_Discom_Util_API.Fetch_For_Tax_Line_Assistant(attr_);          
      $ELSE
         NULL;
      $END
   ELSIF (package_name_ IN ('FISCAL_NOTE_HEAD_API')) THEN   
      $IF Component_Erep_SYS.INSTALLED $THEN 
         source_info_rec_ :=  Tax_Handling_Erep_Util_API.Fetch_For_Tax_Line_Assistant(attr_);          
      $ELSE
         NULL;
      $END
   ELSIF (package_name_ IN ('CHANGE_CONTRACT_ITEM_API', 'CHANGE_SUB_CONTRACT_ITEM_API')) THEN 
      $IF Component_Change_SYS.INSTALLED $THEN   
         source_info_rec_ := Tax_Handling_Change_Util_API.Fetch_For_Tax_Line_Assistant(package_name_, attr_);         
      $ELSE
         NULL;
      $END
   END IF;
...
BEGIN
   ...
   IF ((tax_calc_structure_id_ IS NOT NULL) OR (Company_Localization_Info_API.Get_Parameter_Value_Db(company_, 'EXTEND_TAX_CODE_AND_TAX_STRUCT') = 'TRUE')) THEN
   ...
   -- gelr:br_external_tax_integration, begin
   IF (Company_Localization_Info_API.Get_Parameter_Value_Db(company_, 'OUTGOING_FISCAL_NOTE') = 'TRUE') THEN
      field_visibility_rec_.cst_code_visible             := Fnd_Boolean_API.DB_TRUE;
      field_visibility_rec_.legal_tax_class_visible      := Fnd_Boolean_API.DB_TRUE;    
   ELSE
      field_visibility_rec_.cst_code_visible             := Fnd_Boolean_API.DB_FALSE;
      field_visibility_rec_.legal_tax_class_visible      := Fnd_Boolean_API.DB_FALSE; 
   END IF;
   IF (Company_Localization_Info_API.Get_Parameter_Value_Db(company_, 'EXTEND_TAX_CODE_AND_TAX_STRUCT') = 'TRUE') THEN
      field_visibility_rec_.tax_type_category_visible    := Fnd_Boolean_API.DB_TRUE;
   ELSE
      field_visibility_rec_.tax_type_category_visible    := Fnd_Boolean_API.DB_FALSE;
   END IF;
   field_visibility_rec_.tax_category_visible            := Fnd_Boolean_API.DB_FALSE;   
   -- gelr:br_external_tax_integration, end
   -- Special handling of field visible nature relevant to each module/package. 
   IF (package_name_ IN ('MAN_SUPP_INVOICE_API', 'INSTANT_INVOICE_TEMPLATE_API', 'PROJECT_INVOICE_API', 'SUPPLIER_INVOICE_TEMPLATE_API', 'INSTANT_INVOICE_API', 'MAN_CUST_INVOICE_API')) THEN
      $IF Component_Invoic_SYS.INSTALLED $THEN
   ...
   ELSIF (package_name_ IN ('TAX_DOCUMENT_LINE_API')) THEN
      $IF Component_Discom_SYS.INSTALLED $THEN
         Tax_Handling_Discom_Util_API.Field_Visible_Tax_Line_Assis(field_visibility_rec_,   
                                                                   package_name_,
                                                                   company_,
                                                                   source_key_rec_);
      $ELSE
   ...
   ELSIF (package_name_ IN ('CHANGE_CONTRACT_ITEM_API', 'CHANGE_SUB_CONTRACT_ITEM_API')) THEN
      $IF Component_Change_SYS.INSTALLED $THEN 
         Tax_Handling_Change_Util_API.Field_Visible_Tax_Line_Assis(field_visibility_rec_, package_name_);         
      $ELSE
         NULL;
      $END
   ELSIF (package_name_ IN ('FISCAL_NOTE_HEAD_API')) THEN
      $IF Component_Erep_SYS.INSTALLED $THEN
         Tax_Handling_Erep_Util_API.Field_Visible_Tax_Line_Assis(field_visibility_rec_,   
                                                                 package_name_,
                                                                 company_,
                                                                 source_key_rec_);
      $ELSE
...
BEGIN
   ...
   ELSIF (package_name_ IN ('CHANGE_CONTRACT_ITEM_API', 'CHANGE_SUB_CONTRACT_ITEM_API')) THEN
      $IF Component_Change_SYS.INSTALLED $THEN 
         Tax_Handling_Change_Util_API.Field_Editable_Tax_Line_Assis(field_editable_rec_, package_name_);
      $ELSE
         NULL;
      $END
   ELSE
...
BEGIN
   ...
   -- gelr:br_external_tax_integration, begin
   label_rec_.list_cst_code_label := Language_SYS.Translate_Constant(lu_name_, 'LIST_CST_CODE_LABEL: CST Code');
   -- gelr:br_external_tax_integration, end
...
BEGIN
   IF (package_name_ IN ('INSTANT_INVOICE_API', 'OUTGOING_SUPPLIER_INVOICE_API', 'MAN_SUPP_INVOICE_API', 'MAN_CUST_INVOICE_API', 'PROJECT_INVOICE_API')) THEN
      $IF Component_Invoic_SYS.INSTALLED $THEN 
...
BEGIN 
   ...
   ELSIF (package_name_ IN ('TAX_DOCUMENT_LINE_API')) THEN   
      $IF Component_Discom_SYS.INSTALLED $THEN 
         Tax_Handling_Discom_Util_API.Save_From_Tax_Line_Assistant(company_, source_key_rec_, tax_info_table_);
      $ELSE
         NULL;
      $END
   ELSIF (package_name_ IN ('FISCAL_NOTE_HEAD_API')) THEN
      $IF Component_Erep_SYS.INSTALLED $THEN
         Tax_Handling_Erep_Util_API.Save_From_Tax_Line_Assistant(company_, source_key_rec_, tax_info_table_);
      $ELSE
         NULL;
      $END
   ELSIF (package_name_ IN ('CHANGE_CONTRACT_ITEM_API', 'CHANGE_SUB_CONTRACT_ITEM_API')) THEN    
      $IF Component_Change_SYS.INSTALLED $THEN 
         Tax_Handling_Change_Util_API.Save_From_Tax_Line_Assistant(company_, source_key_rec_, tax_info_table_, calc_base_, package_name_, transaction_date_);       
      $ELSE
         NULL;
      $END
   END IF;
...
IS  
BEGIN
   IF (package_name_ IN ('INSTANT_INVOICE_API', 'OUTGOING_SUPPLIER_INVOICE_API', 'MAN_SUPP_INVOICE_API', 'MAN_CUST_INVOICE_API', 'CUSTOMER_ORDER_INV_ITEM_API', 'CUSTOMER_ORDER_INV_HEAD_API', 'SUPPLIER_INVOICE_TEMPLATE_API', 'INSTANT_INVOICE_TEMPLATE_API', 'PROJECT_INVOICE_API', 'PROJECT_INVOICE_ITEM_API')) THEN
      $IF Component_Invoic_SYS.INSTALLED $THEN 
   ...
   ELSIF (package_name_ IN ('CHANGE_CONTRACT_ITEM_API', 'CHANGE_SUB_CONTRACT_ITEM_API')) THEN
      $IF Component_Change_SYS.INSTALLED $THEN
         Tax_Handling_Change_Util_API.Tax_Line_Assis_Set_To_Def(tax_info_table_,
                                                                       line_amount_rec_, 
                                                                       tax_calc_structure_id_,
                                                                       company_, 
                                                                       package_name_,
