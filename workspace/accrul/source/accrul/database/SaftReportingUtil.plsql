...
--  210922  THPELK  Bug 160917, merged APP10 correction. Corrected in Add_Master_Files_Info___.
--  211130  Lakhlk  Bug 160917, merged APP10 correction, Corrected in Generate_File_Name__.
-----------------------------------------------------------------------------
...
PROCEDURE Add_Master_Files_Info___(
   ...
   to_date_       IN     DATE)
IS
   country_db_             Audit_Basic_Data_Master_Tab.country%TYPE;
   report_type_db_         Audit_Basic_Data_Master_Tab.report_type%TYPE;
   reporting_currency_db_  Audit_Basic_Data_Master_Tab.reporting_currency%TYPE;
BEGIN
   ...
   $IF Component_Payled_SYS.INSTALLED $THEN
      ...
      report_type_db_        := Audit_Report_Types_API.Encode(report_type_);
      reporting_currency_db_ := Audit_Basic_Data_Master_API.Get_Reporting_Currency_Db(company_, country_, report_type_);
   
      Saft_Reporting_Payled_Util_API.Fetch_Qualified_Info(company_, from_date_, to_date_, reporting_currency_db_);
...
BEGIN
   ...
   IF (batch_ = 'TRUE') THEN
      IF (error_exists_ = 'TRUE') THEN
         error_text_ := Language_SYS.Translate_Constant(lu_name_, 'XMLERROROCCURED: An error occurred while generating the XML file. Please refer the partially generated XML file for more details.');
      END IF;
      Insert_Batch_File_Info___(company_, load_file_id_, report_type_, file_template_id_ ,clob_out_, path_, error_exists_, error_text_);
   ELSE
...
PROCEDURE Insert_Batch_File_Info___(
   company_          IN VARCHAR2,
   load_file_id_     IN NUMBER,
   report_type_      IN VARCHAR2,
   ...
   error_exist_      IN VARCHAR2,
   error_text_       IN VARCHAR2)
IS
   batch_file_rec_   Audit_Batch_File_Info_Tab%ROWTYPE;
BEGIN
   batch_file_rec_.company       := company_;
   batch_file_rec_.load_file_id  := load_file_id_;
   batch_file_rec_.report_type   := report_type_;
   batch_file_rec_.template_id   := template_id_;
   batch_file_rec_.xml_data      := xml_data_;  
   batch_file_rec_.xml_file_name := xml_file_name_;
   batch_file_rec_.error_exist   := NVL(error_exist_, 'FALSE');
   batch_file_rec_.error_text    := error_text_;
   Audit_Batch_File_Info_API.Insert_Record(batch_file_rec_);
END Insert_Batch_File_Info___;
...
IS
   ...
   association_no_   COMPANY_TAB.association_no%TYPE;
BEGIN
   association_no_ := Company_API.Get_Association_No(company_name_);
   IF (association_no_ IS NOT NULL) THEN 
      file_name_ := file_type_ || '_' || association_no_ || '_' || timestamp_ || '_' || file_no_ || format_;
   ELSE
