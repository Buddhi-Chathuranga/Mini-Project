...
BEGIN
   IF newrec_.master_company IS NULL AND oldrec_.master_company IS NOT NULL THEN
      Client_SYS.Add_Warning(lu_name_, 'MASTERREMOVED: All existing Group Account mapping will be removed.');
   ...
   ELSIF newrec_.master_company IS NOT NULL AND newrec_.master_company != oldrec_.master_company THEN
      Client_SYS.Add_Warning(lu_name_, 'MASTERUPDATE: Existing Group Account mapping might not be relevant if you change Company. If so, please review existing mapping. Alternatively first remove the old Company and save, which will remove all existing mapping.');
   END IF;   
