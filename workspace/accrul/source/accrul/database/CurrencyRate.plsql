...
BEGIN
   ...
   IF (error_text_ IS NULL) THEN
      ...
      Get_Ref_Currency___(ref_currency_code_ ,valid_from_date_, company_, to_currency_code_,currency_type_ ,valid_from_);
      ...
      IF (error_text_ IS NULL) THEN
         ...
         Get_Ref_Currency___(ref_currency_code_ ,valid_from_date_, company_, from_currency_code_,currency_type_ ,valid_from_);
...
PROCEDURE Get_Ref_Currency___ (
   ref_currency_code_      OUT CURRENCY_RATE_TAB.ref_currency_code%TYPE,
...
BEGIN
  SELECT a.ref_currency_code, a.valid_from
      INTO ref_currency_code_ , valid_from_date_
...
EXCEPTION
   WHEN no_data_found THEN
      ref_currency_code_ := NULL;
      valid_from_date_   := NULL;
   --too many rows exception is handled using rownum = 1
END Get_Ref_Currency___;
...
BEGIN
   ...
   END IF;
   Handle_Centrlized_Rate_Del___(remrec_);
   super(remrec_);
END Check_Delete___;


PROCEDURE Handle_Centrlized_Rate_Del___(
   remrec_ IN CURRENCY_RATE_TAB%ROWTYPE )
IS
   another_rate_exist_ NUMBER;
   
   CURSOR another_rate_exist IS
   SELECT 1
   FROM Currency_Rate_Tab
   WHERE company       = remrec_.company
   AND   currency_type = remrec_.currency_type
   AND   currency_code = remrec_.currency_code
   AND   valid_from   != remrec_.valid_from;
   
   CURSOR get_target_companies IS
   SELECT target_company, target_curr_rate_type
   FROM Target_Comp_Curr_Rate_Type_Tab
   WHERE source_company       = remrec_.company
   AND  source_curr_rate_type = remrec_.currency_type;
   
BEGIN
   -- is the currenct company a source company and state is active.
   IF (NVL(Source_Comp_Curr_Rate_Type_API.Get_Objstate(remrec_.company, remrec_.currency_type), ' ') = Source_Comp_Curr_Rate_Type_API.DB_ACTIVE) THEN
      -- Rate already exist for another date.
      OPEN another_rate_exist;
      FETCH another_rate_exist INTO another_rate_exist_;
      IF another_rate_exist%FOUND THEN
         CLOSE another_rate_exist;
         RETURN;
      ELSE
         CLOSE another_rate_exist;
         -- Process target companies.
         FOR target_ IN get_target_companies LOOP
            -- Get target company ref currency.
            IF Currency_Type_API.Get_Ref_Currency_Code(target_.target_company, target_.target_curr_rate_type) = remrec_.currency_code THEN
               -- source currency is targets ref currency.
               Client_SYS.Add_Warning(lu_name_, 'RATEUSEDINCENTRALIZED: Currency Rate defined for Currency Code :P1 will be used for triangulation calculation. Currency rates will not be updated for Target Company :P2 - Target Currency Rate Type :P3 in Centralized Currency Rate Handling.', remrec_.currency_code, target_.target_company, target_.target_curr_rate_type);
            END IF;
         END LOOP;
      END IF;
   END IF;
END Handle_Centrlized_Rate_Del___;
...
@IgnoreUnitTest TrivialFunction
FUNCTION Check_Exist_Curr_Code___ (
   company_       IN  VARCHAR2,
   currency_code_ IN  VARCHAR2,
   currency_type_ IN  CURRENCY_RATE_TAB.currency_type%TYPE) RETURN BOOLEAN
IS   
   dummy_ NUMBER;
BEGIN   
   IF (company_ IS NULL OR currency_type_ IS NULL OR currency_code_ IS NULL) THEN
      RETURN FALSE;
   END IF;
...
EXCEPTION
   WHEN no_data_found THEN
       RETURN FALSE;
  -- too_many_rows is handled using rownum = 1;
END Check_Exist_Curr_Code___;


PROCEDURE Check_If_Curr_Code_Exists___ (
   error_text_    OUT VARCHAR2,
   company_       IN  VARCHAR2,
   currency_type_ IN  VARCHAR2,
   currency_code_ IN  VARCHAR2,
...
BEGIN
   ...
   IF (NOT Check_Exist_Curr_Code___( company_ , currency_code_ , curr_type_)) THEN
      ...
      END IF;  
   END IF;
END Check_If_Curr_Code_Exists___;
...
IS   
BEGIN
  Check_If_Curr_Code_Exists___(error_text_ , company_ , currency_type_ , currency_code_ ,ignore_errors_);
...
BEGIN
   ...
   --Check currency code exist for currency type
   Check_If_Curr_Code_Exists___(error_text_, company_, currency_type_, base_currency_code_, TRUE);

   IF (error_text_ IS NULL) THEN
      --Check transaction currency code exist for currency type
      Check_If_Curr_Code_Exists___(error_text_, company_, currency_type_, currency_code_, TRUE);
