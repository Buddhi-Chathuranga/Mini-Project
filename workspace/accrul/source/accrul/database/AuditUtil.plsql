...
BEGIN
   ...
   IF batch_ = 'TRUE' THEN
      -- write data file      
      Insert_Batch_File_Info___(company_, view_, lob_xml_data_, lob_data_, view_ || '_I.XML', view_ || '.txt');
   END IF;
EXCEPTION
   WHEN OTHERS THEN
...
BEGIN
   ...
   IF batch_ = 'TRUE' THEN
      -- write data file
      Insert_Batch_File_Info___(company_, view_, lob_xml_data_, lob_data_, view_ || '_I.XML', view_);
   END IF;
EXCEPTION
   WHEN OTHERS THEN
      IF (DBMS_SQL.IS_OPEN(cursor_)) THEN
...
PROCEDURE Insert_Batch_File_Info___(
   ...
   text_file_name_   IN VARCHAR2)
IS
   batch_file_rec_   Audit_Batch_File_Info_Tab%ROWTYPE;
BEGIN
   batch_file_rec_.company          := company_;
   batch_file_rec_.report_type      := Audit_Report_Types_API.DB_GD_PD_U;
   batch_file_rec_.audit_source     := audit_source_;
   batch_file_rec_.xml_data         := xml_data_;
   batch_file_rec_.text_data        := text_data_;
   batch_file_rec_.xml_file_name    := xml_file_name_;      
   batch_file_rec_.text_file_name   := text_file_name_;   
