...
--  210602  THPELK   FI21R2-1505 - Get_Min_Open_User_Grp_Date().
-----------------------------------------------------------------------------
...
@SecurityCheck Company.UserAuthorized(company_)
FUNCTION Get_Min_Open_User_Grp_Date (
   company_    IN VARCHAR2,
   user_group_ IN VARCHAR2,
   date_       IN DATE,
   ledger_id_  IN VARCHAR2 DEFAULT '00' ) RETURN DATE
IS  
   current_accounting_year_   NUMBER;
   current_accounting_period_ NUMBER;
   earliest_open_acc_year_    NUMBER;
   earliest_open_acc_period_  NUMBER;
   first_viable_posting_date_ DATE;   
   user_group_use_            user_group_period_tab.user_group%TYPE;
   date_in_                   DATE;
   ...
   CURSOR get_previous_periods IS
      ...
      FROM   user_group_period_tab
      WHERE  company           = company_
      AND    user_group        = user_group_use_
      AND    ledger_id         = ledger_id_
      AND  ((accounting_year *100) + accounting_period) <= ((current_accounting_year_ * 100)+ current_accounting_period_)
      ORDER BY accounting_year DESC, accounting_period DESC;
BEGIN
   user_group_use_ := user_group_;
   IF ( user_group_use_ IS NULL ) THEN
      user_group_use_ := User_Group_Member_Finance_API.Get_User_Group(company_, Fnd_Session_API.Get_Fnd_User);
   END IF;
   date_in_ := date_;
   IF ( date_in_ IS NULL ) THEN 
      date_in_ := SYSDATE;
   END IF;
   date_in_ := TRUNC(date_in_);
   Accounting_Period_API.Get_Accounting_Year(current_accounting_year_, current_accounting_period_, company_, date_in_ , user_group_use_);
     
   FOR rec_ IN get_previous_periods LOOP
     IF (rec_.period_status = 'C') THEN
       EXIT;
     END IF;
     earliest_open_acc_year_   := rec_.accounting_year;
     earliest_open_acc_period_ := rec_.accounting_period; 
   END LOOP;
     
   first_viable_posting_date_ := Accounting_Period_API.Get_Date_From (company_, earliest_open_acc_year_, earliest_open_acc_period_);
   RETURN first_viable_posting_date_;
END Get_Min_Open_User_Grp_Date;
