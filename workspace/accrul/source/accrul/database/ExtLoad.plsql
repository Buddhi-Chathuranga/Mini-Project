...
--  210331  Tkavlk FI21R2-487, Merged LCS Bug 158698, Modified Start_Ext_Vouchers.
--  220303  TiRalk FIDEV-8937, Merged LCS Bug 162320, Modified Start_Ext_Vouchers to show correct load ids, companies for multi companies.
--  220616  supwlk FIDEV-11198, Modified Start_Ext_Vouchers method.
--  221005  TiRalk FIZ-20046, Modified Start_Ext_Vouchers to calculate currency_debit_amount and currency_credit_amount properly when currency amount is zero.
-----------------------------------------------------------------------------
...
IS
   ...
   parallel_amount_without_rounding_  NUMBER;
...
BEGIN
   ...
   FOR trans_rec_ IN get_ext_file_trans LOOP
      ...
      IF old_load_id_ IS NULL OR tst_company_ != rec_.company THEN
      ...
      IF (rec_.voucher_type IS NULL) THEN
         rec_.voucher_type                := Ext_Parameters_API.Get_Voucher_Type(rec_.company, load_type_);
      END IF;
      ...
      -- This logic caters the scenario where transaction currency is zero together with a negative amount value
      IF (rec_.currency_amount = 0) THEN
         IF (rec_.debet_amount IS NOT NULL) THEN
            rec_.currency_debet_amount := rec_.currency_amount;
            rec_.currency_credit_amount := NULL;
         ELSE
            rec_.currency_credit_amount  := rec_.currency_amount;
            rec_.currency_debet_amount := NULL;
         END IF;
      END IF;
      ...
      IF (third_currency_code_ IS NOT NULL AND rec_.third_currency_amount IS NULL) THEN 
         IF (rec_.amount IS NOT NULL) THEN
            ...
            END IF;
            IF (rec_.currency_debet_amount IS NOT NULL AND rec_.currency_debet_amount = 0) THEN
               rec_.third_currency_debit_amount := Currency_Amount_API.Calculate_Parallel_Curr_Amount(rec_.company,
                                                                                                      tst_curr_date_,
                                                                                                      rec_.debet_amount,
                                                                                                      rec_.currency_debet_amount,
                                                                                                      base_currency_,
                                                                                                      rec_.currency_code,
               ...
               rec_.third_currency_debit_amount    := ROUND(rec_.third_currency_debit_amount, third_currency_round_);               
               rec_.third_currency_amount          := rec_.third_currency_debit_amount - NVL(rec_.third_currency_credit_amount, 0);
            ELSIF (rec_.currency_credit_amount IS NOT NULL AND rec_.currency_credit_amount = 0) THEN
               rec_.third_currency_credit_amount := Currency_Amount_API.Calculate_Parallel_Curr_Amount(rec_.company,
                                                                                                      tst_curr_date_,
                                                                                                      rec_.credit_amount,
               ...
               rec_.third_currency_credit_amount   := ROUND(rec_.third_currency_credit_amount, third_currency_round_);               
               rec_.third_currency_amount          := NVL(rec_.third_currency_debit_amount, 0) - rec_.third_currency_credit_amount;
            ELSE
               ...
               parallel_amount_without_rounding_ := rec_.third_currency_amount;
         ...
         IF rec_.correction = 'FALSE' THEN
            IF (rec_.third_currency_amount IS NOT NULL AND rec_.third_currency_debit_amount IS NULL AND rec_.third_currency_credit_amount IS NULL) THEN
               IF ((parallel_amount_without_rounding_ < 0)) THEN
                  rec_.third_currency_credit_amount := rec_.third_currency_amount * -1;
               ELSE
                  rec_.third_currency_debit_amount  := rec_.third_currency_amount;
               END IF; 
   ...
   ELSE
      ...
      FOR load_rec_ IN get_load_id LOOP
         ...
         IF (ext_load_status_tab_.EXISTS(ext_load_state_)) THEN
            ext_load_status_tab_(ext_load_state_).company_load_id_col := ext_load_status_tab_(ext_load_state_).company_load_id_col || ',' || load_rec_.company || '/' || load_rec_.load_id;
         ELSE
