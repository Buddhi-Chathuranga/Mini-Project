...
--  220422  Jadulk   FIZ-16874, Modified in Import___ to inline voucher type user group data with the calender data.
-----------------------------------------------------------------------------
...
IS
   ...
   CURSOR get_cal_min_max IS
      SELECT MIN(accounting_year), MAX(accounting_year)
        FROM Accounting_Year_Tab
       WHERE company = crecomp_rec_.company;
BEGIN
   ...
   ELSE
      ...
      IF (NOT any_rows_) THEN
         IF (crecomp_rec_.user_defined = 'TRUE') THEN
            ...
            END LOOP;
         ELSE
            OPEN get_cal_min_max;
            FETCH get_cal_min_max INTO min_year_, max_year_;
            CLOSE get_cal_min_max;
            
            FOR rec_ IN get_data LOOP            
               IF(rec_.n1 >= min_year_ AND rec_.n1<= max_year_)THEN
                  i_ := i_ + 1;
                  @ApproveTransactionStatement(2014-03-24,dipelk)
                  SAVEPOINT make_company_insert;
                  data_found_ := TRUE;
