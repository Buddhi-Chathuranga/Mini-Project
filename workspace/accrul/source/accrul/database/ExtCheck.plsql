...
--  210331  Tkavlk  FI21R2-446, Merged LCS Bug 158358, Corrected in Process_By_Load_Group_Item___.
--  210810  Alwolk  FI21R2-3690, Merged LCS Bug 160389, Modified Validate_Transaction_Data__.
-----------------------------------------------------------------------------
   ...
   LOOP
         ...
         IF (error_ = FALSE) THEN
            IF (old_trans_rec_.load_group_item IS NOT NULL) THEN
               ...
               IF (ext_voucher_diff_ = 'N') AND (amount_ != 0 OR (third_curr_amount_ !=0 AND third_curr_null_ = 'TRUE')) THEN
                  ...
                  Ext_Transactions_API.Update_Load_Error(old_trans_rec_.company,
                                                         ...
                                                         err_msg_);
               ELSE
                  IF (ext_voucher_diff_ = 'N' AND amount_ = 0 AND third_curr_amount_ !=0 AND third_curr_null_= 'FALSE') THEN
            ...
            IF (err_msg_ IS NULL) AND (stat_acc_ = 'FALSE') THEN
               ...
               ELSE
                  amount_ := nvl(new_trans_rec_.debet_amount,-new_trans_rec_.credit_amount);
               END IF;
               IF (NVL(new_trans_rec_.third_currency_debit_amount,0) != 0 OR nvl(new_trans_rec_.third_currency_credit_amount,0) != 0 OR NVL(new_trans_rec_.third_currency_tax_amount,0) != 0) THEN
                  IF (tax_type_ NOT IN (Fee_Type_API.DB_CALCULATED_TAX)) THEN                   
                     IF (def_amount_method_ = 'NET') THEN           
                        third_curr_amount_ := NVL(nvl(new_trans_rec_.third_currency_debit_amount,-new_trans_rec_.third_currency_credit_amount),0) + nvl(new_trans_rec_.third_currency_tax_amount,0);              
                     ELSE
      ...
      ELSIF (error_ = FALSE ) AND (old_trans_rec_.load_group_item = new_trans_rec_.load_group_item) AND (stat_acc_ = 'FALSE') THEN
         ...
         IF (NVL(new_trans_rec_.third_currency_debit_amount,0) != 0 OR nvl(new_trans_rec_.third_currency_credit_amount,0) != 0 OR nvl(new_trans_rec_.third_currency_tax_amount,0) != 0)  THEN
            IF (tax_type_ NOT IN (Fee_Type_API.DB_CALCULATED_TAX)) THEN
               IF (def_amount_method_ = 'NET') THEN
                  third_curr_amount_ := third_curr_amount_ + NVL(nvl(new_trans_rec_.third_currency_debit_amount,-new_trans_rec_.third_currency_credit_amount),0) + nvl(new_trans_rec_.third_currency_tax_amount,0);
               ELSE
                  IF (NVL(new_trans_rec_.third_currency_debit_amount,0) != 0 OR nvl(new_trans_rec_.third_currency_credit_amount,0) != 0) THEN
...
BEGIN
   ...
   ELSIF (project_origin_ = 'JOB') THEN
      IF (NVL(function_group_, ' ') NOT IN ('M', 'K', 'Q','A','P','R','Z','D')) THEN
   ...
   IF (NVL(rec_.project_activity_id,0) <> 0) THEN
      $IF Component_Proj_SYS.INSTALLED $THEN
         IF (project_origin_ = 'PROJECT') THEN
            IF NVL(function_group_, ' ') != 'P' THEN
               IF NVL(function_group_, ' ') IN ('I', 'J', 'T', 'V', 'TF') THEN
         ...
         IF (NVL(function_group_, ' ') != 'P' AND project_origin_ IN ('FINPROJECT','JOB')) THEN
            $IF Component_Genled_SYS.INSTALLED $THEN
...
IS
   ...
   mismatch_third_credit_debit_    BOOLEAN := FALSE;
...
BEGIN
   ...
   -- Note: Validation to check whether Parallel Currency Amount is in the same side as Accounting Currency.
   mismatch_third_credit_debit_ := FALSE;
   IF (third_currency_ IS NOT NULL) THEN
      IF (transaction_rec_.credit_amount IS NOT NULL AND transaction_rec_.third_currency_credit_amount IS NULL) THEN
         mismatch_third_credit_debit_ := TRUE;
      ELSIF (transaction_rec_.debet_amount IS NOT NULL AND transaction_rec_.third_currency_debit_amount IS NULL) THEN
         mismatch_third_credit_debit_ := TRUE;
      ELSIF (transaction_rec_.currency_credit_amount IS NOT NULL AND transaction_rec_.third_currency_credit_amount IS NULL) THEN
         mismatch_third_credit_debit_ := TRUE;
      ELSIF (transaction_rec_.currency_debet_amount IS NOT NULL AND transaction_rec_.third_currency_debit_amount IS NULL) THEN
         mismatch_third_credit_debit_ := TRUE;
      END IF;          
   END IF;
      
   IF mismatch_third_credit_debit_ THEN
      BEGIN
         Error_SYS.Record_General('ExtCheck', 'THIRD_CREDIT_DEBIT_MISMATCH: Parallel currency amount should be in the same side as accounting currency.');
      EXCEPTION
         WHEN OTHERS THEN
            err_msg_ := SQLERRM;
      END;
