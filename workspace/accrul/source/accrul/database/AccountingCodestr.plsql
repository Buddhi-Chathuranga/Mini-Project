...
--  210721  Umdolk FI21R2-3681, Merged Bug 160226, Added new method Validate_Codepart_Demands and changed Validate_Codeparts___. 
-----------------------------------------------------------------------------
...
PROCEDURE Validate_Codeparts___ (
   ...
   req_string_in_             IN VARCHAR2 DEFAULT NULL,
   curr_balance_in_           IN VARCHAR2 DEFAULT NULL)
IS
...
BEGIN   
   IF (validate_account_) THEN
      ...
      END IF;
   ELSE
      IF (req_string_in_ IS NOT NULL) THEN
         req_string_ := req_string_in_;
      END IF;
      IF (curr_balance_in_ IS NOT NULL) THEN
         curr_balance_ := curr_balance_in_;
      END IF;
   END IF;
...
IS
   code_part_value_param_  VARCHAR2(4000);
BEGIN
   code_part_value_param_  := code_part_value_;
   IF code_part_value_param_ = CHR(0) THEN
      code_part_value_param_ := NULL;
   END IF;
   IF (posting_type_ IS NOT NULL ) THEN
      IF (validate_budget_) THEN
         Error_SYS.Appl_General(lu_name_,'CODEMISSINGBUDPOSTING: The value :P2 used for the code part :P1 and posting type :P3 is either missing, is a budget/planning code part value or has invalid time interval.',
                                Accounting_Code_Parts_API.Get_Name(company_,code_part_), code_part_value_param_, posting_type_);
      ELSE      
         Error_SYS.Appl_General(lu_name_,'CODEMISSINGPOSTING: :P1 :P2 is missing or has invalid time interval for posting type :P3.', Accounting_Code_Parts_API.Get_Name(company_,code_part_), code_part_value_param_, posting_type_);
      END IF;
   END IF;
   IF (validate_budget_) THEN
      Error_SYS.Appl_General(lu_name_,'CODEMISSINGBUD: The value :P2 used for the code part :P1 is either missing, is a budget/planning code part value or has invalid time interval.',
                             Accounting_Code_Parts_API.Get_Name(company_,code_part_), code_part_value_param_);
   ELSE      
      Error_SYS.Appl_General(lu_name_,'CODEMISSING: :P1 :P2 is missing or has invalid time interval', Accounting_Code_Parts_API.Get_Name(company_,code_part_), code_part_value_param_);
   END IF;
...
BEGIN
   ...
   IF (code_part_ != 'A' AND (Instr(this_code_part_value_, '%') > 0 OR Instr(this_code_part_value_, '_') > 0 )) THEN
      NULL;
...
-- This method will only validate code part demands for the given req_string
PROCEDURE Validate_Codepart_Demands (
   codestring_rec_            IN CodestrRec,
   company_                   IN VARCHAR2,
   req_string_                IN VARCHAR2,
   curr_balance_              IN VARCHAR2,
   check_demands_quantity_    BOOLEAN DEFAULT TRUE,
   check_demands_process_     BOOLEAN DEFAULT TRUE,
   check_demands_text_        BOOLEAN DEFAULT TRUE)
IS
BEGIN      
   Validate_Codeparts___( codestring_rec_, 
                          company_, 
                          NULL,
                          FALSE,
                          TRUE,
