-----------------------------------------------------------------------------
--
--  Logical unit: CreateConnectMessages
--  Component:    DISCOM
--
...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  221220  Avjalk  SCZ-20570, Modified the condition of inserting and activating the DESADV message class in Create_Connectivity_Messages___ to prevent parallel deployment errors.
...
-- Create_Connectivity_Messages___
--    Create Message Classes for ORDER, PURCH and RCEIPT, depending on their availabllity.
PROCEDURE Create_Connectivity_Messages___
IS
...
END Create_Connectivity_Messages___;

-- Insert_Prog_Translation___
--    Inserting PROG translations for connectivity messages
PROCEDURE Insert_Prog_Translation___
IS
   module_                VARCHAR2(10) := 'ORDER';
BEGIN
   IF Component_Order_SYS.INSTALLED THEN
      -- Inserting PROG translations for ORDER connectivity messages
   ...
   END IF;
   IF Component_Order_SYS.INSTALLED OR Component_Purch_SYS.INSTALLED THEN
      -- Inserting PROG translations for ORDER PURCH shared  connectivity messages...
      IF (Component_Purch_SYS.INSTALLED) THEN
         module_ := 'PURCH';
      END IF;
...
--    Create the Connectivity Messages for dynamic modules and register Object LU Connection Transformation for dynamic modules.
--    NOTE: This method is never called at runtime. This has been added to be called at deployment time only, when any of 
--          components Order, Purch, Rceipt or Shpmnt freshly installed. Because of dynamic dependency among them and ability 
--          to be installed during different upgrades, this will be called from those components when freshly deployed.      
PROCEDURE Register_Message_Classes
IS
BEGIN
   -- Create all Connectivity Messages for dynamic modules.
   Create_Connectivity_Messages___();

   -- Register Object LU Connection Transformation for dynamic modules.
   Insert_Prog_Translation___();

END Register_Message_Classes;
