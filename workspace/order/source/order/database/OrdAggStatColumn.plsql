...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  190429  LaThlk  SCUXXW4-19484, Removed the unused parameter from Get_Ord_Agg_Stat_Columns().
--  190419  LaThlk  SCUXXW4-9257, Added the function Get_Ord_Agg_Stat_Columns() to return columns as a stringify json.
...
--  990407  JakH  New template. Deleted get_col_detail, added public remove.
...
IS
   ...
   info_          VARCHAR2(2000);
BEGIN
   Get_Id_Version_By_Keys___ ( objid_ , objversion_ , company_, aggregate_id_ , column_name_  );
   Remove__ ( info_ , objid_ , objversion_ , 'DO');
END Remove;

FUNCTION Get_Ord_Agg_Stat_Columns (
   company_             IN VARCHAR2,
   aggregate_id_        IN NUMBER,
...
IS 
   ...
   CURSOR get_values IS
      SELECT column_name
      FROM   ORD_AGG_STAT_COLUMN
      WHERE  aggregate_id = aggregate_id_
      AND    company = company_;   
BEGIN
   selected_str_ := '[';
   FOR value_ IN get_values LOOP
      selected_str_ := selected_str_ || '"' || 'COLUMN_NAME=' || value_.column_name || '^ISSUE_ID=' || issue_id_ || '^",';
   END LOOP;
