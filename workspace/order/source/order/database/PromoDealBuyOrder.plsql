-----------------------------------------------------------------------------
--
--  Logical unit: PromoDealBuyOrder
--  Component:    ORDER
--
...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  101215  NaLrlk  Renamed the method Delete_All_Records_For_Order to Delete_Promo_Buy_For_Order.
...
-- New
--   Public interface for creating a new promotion deal buy order.
PROCEDURE New (
   campaign_id_ IN NUMBER,
   deal_id_     IN NUMBER,
   buy_id_      IN NUMBER,
...
IS
   attr_        VARCHAR2(2000);
   objid_       PROMO_DEAL_BUY_ORDER.objid%TYPE;
   objversion_  PROMO_DEAL_BUY_ORDER.objversion%TYPE;
   newrec_      PROMO_DEAL_BUY_ORDER_TAB%ROWTYPE;
...
BEGIN
   ...
   Get_Id_Version_By_Keys___(objid_, objversion_, campaign_id_, deal_id_, buy_id_, order_no_);
   Modify__(info_, objid_, objversion_, attr_, 'DO');
END Modify;


PROCEDURE Delete_Promo_Buy_For_Order (
   order_no_    IN VARCHAR2 )
IS
   remrec_      PROMO_DEAL_BUY_ORDER_TAB%ROWTYPE;
   objid_       PROMO_DEAL_BUY_ORDER.objid%TYPE;
   objversion_  PROMO_DEAL_BUY_ORDER.objversion%TYPE;
...
BEGIN
   ...
   END LOOP;
END Delete_Promo_Buy_For_Order;
