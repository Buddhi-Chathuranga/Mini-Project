...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  170626  ErRalk  Bug 135979, Changed the error message constant from ONLYONEMINAMOUNT to ONEOFAMOUNT and modified message content according to logic in Check_Before_Ins_And_Upd___ to eliminate duplication of message constant.
--  131029  RoJalk  Modified the view comments of contract to be mandatory and included NOT NULL checks.
--  111116  ChJalk  Modified the view SALES_PROMOTION_DEAL_GET to use User_Finance_Auth_Pub instead of Company_Finance_Auth_Pub.
--  111026  ChJalk  Modified the base view SALES_PROMOTION_DEAL_GET to use the user allowed company filter.
...
--  090611  RILASE  Added Deal_Is_Discount_Only.
--  090610  RILASE  Added New.
...
PROCEDURE Check_Before_Ins_And_Upd___(
   newrec_ IN SALES_PROMOTION_DEAL_GET_TAB%ROWTYPE )
IS
BEGIN
   ...
   IF ((newrec_.net_amount IS NOT NULL AND (newrec_.gross_amount IS NOT NULL OR newrec_.qty IS NOT NULL))
   OR (newrec_.gross_amount IS NOT NULL AND (newrec_.net_amount IS NOT NULL OR newrec_.qty IS NOT NULL))
   OR (newrec_.qty IS NOT NULL AND (newrec_.net_amount IS NOT NULL OR newrec_.gross_amount IS NOT NULL))) THEN
      Error_SYS.Record_General(lu_name_, 'ONLYONEMINAMOUNT: Only one of quantity, net amount or gross amount can be set.');
   END IF;

   IF (newrec_.net_amount IS NULL AND newrec_.gross_amount IS NULL AND newrec_.qty IS NULL
       AND Sales_Promotion_Deal_API.Get_Discount(newrec_.campaign_id, newrec_.deal_id) IS NULL) THEN
      Error_SYS.Record_General(lu_name_, 'ONEOFAMOUNT: One of quantity, net amount or gross amount must be set if the deal uses price or discount amount.');
   END IF;

   IF (newrec_.qty IS NOT NULL AND newrec_.price_unit_meas IS NULL) THEN
      Error_SYS.Record_General(lu_name_, 'UNITMEASMUSTBESET: Price unit of measure must be set if quantity is used.');
   END IF;
   ...
   IF (newrec_.qty <= 0) THEN
      Error_SYS.Record_General(lu_name_, 'QTYAMTLARGERTHANZERO: The quantity must be greater than zero.');
   END IF;

   IF (newrec_.net_amount <= 0) THEN
      Error_SYS.Record_General(lu_name_, 'NETAMTLARGERTHANZERO: The net amount must be greater than zero.');
   END IF;

   IF (newrec_.gross_amount <= 0) THEN
      Error_SYS.Record_General(lu_name_, 'GROSSAMTLARGERTHANZERO: The gross amount must be greater than zero.');
   END IF;

END Check_Before_Ins_And_Upd___;


PROCEDURE Get_Next_Get_Id___ (
   get_id_       IN OUT NUMBER,
   campaign_id_  IN     NUMBER,
   deal_id_      IN     NUMBER)
IS
   CURSOR get_next_get_id IS
   SELECT NVL(max(get_id + 1), 1)
   FROM   SALES_PROMOTION_DEAL_GET_TAB
   WHERE  campaign_id = campaign_id_
   AND    deal_id = deal_id_;
BEGIN
   OPEN  get_next_get_id;
   FETCH get_next_get_id INTO get_id_;
   CLOSE get_next_get_id;
END Get_Next_Get_Id___;


-- Exist_Catalog_No___
--   Returns whether or not a specific catalog no already connected to a
--   sales promotion deal get line.
...
PROCEDURE Insert___ (
   ...
   newrec_     IN OUT SALES_PROMOTION_DEAL_GET_TAB%ROWTYPE,
   attr_       IN OUT VARCHAR2 )
IS
BEGIN
   Get_Next_Get_Id___(newrec_.get_id, newrec_.campaign_id, newrec_.deal_id);
   Client_SYS.Add_To_Attr('GET_ID', newrec_.get_id, attr_);
...
PROCEDURE Check_Delete___ (
   remrec_ IN SALES_PROMOTION_DEAL_GET_TAB%ROWTYPE )
IS
BEGIN
   IF (Promo_Deal_Get_Order_API.Check_Exist_Any_Order(remrec_.campaign_id, remrec_.deal_id, remrec_.get_id)) THEN
   ...
   END IF;
   IF (Promo_Deal_Get_Quotation_API.Check_Exist_Any_Quotation(remrec_.campaign_id, remrec_.deal_id, remrec_.get_id)) THEN
...
BEGIN
   ...
   IF (newrec_.assortment_node_id IS NULL) THEN
      newrec_.assortment_id := '';
      indrec_.assortment_id := TRUE;
   END IF;
...
PROCEDURE Check_Update___ (
   oldrec_ IN     sales_promotion_deal_get_tab%ROWTYPE,
   newrec_ IN OUT sales_promotion_deal_get_tab%ROWTYPE,
   indrec_ IN OUT Indicator_Rec,
   attr_   IN OUT VARCHAR2 )
IS
...
IS
   ...
   objid_       SALES_PROMOTION_DEAL_GET.objid%TYPE;
   objversion_  SALES_PROMOTION_DEAL_GET.objversion%TYPE;
   newrec_      SALES_PROMOTION_DEAL_GET_TAB%ROWTYPE;
...
BEGIN
   ...
   Client_SYS.Add_To_Attr('QTY', qty_, attr_);
...
@UncheckedAccess
FUNCTION Deal_Is_Discount_Only(
   campaign_id_ IN NUMBER,
   deal_id_     IN NUMBER) RETURN BOOLEAN
IS
   dummy_ NUMBER;
   CURSOR get_has_disc_only_lines IS
      SELECT 1
      FROM   SALES_PROMOTION_DEAL_GET_TAB
      WHERE  campaign_id = campaign_id_
      AND    deal_id     = deal_id_
...
BEGIN
   -- Check if there are GET lines with qty, net amount AND gross amount that is NULL.
   OPEN get_has_disc_only_lines;
   FETCH get_has_disc_only_lines INTO dummy_;
   IF (get_has_disc_only_lines%FOUND) THEN
      CLOSE get_has_disc_only_lines;
      RETURN TRUE;
   END IF;
   CLOSE get_has_disc_only_lines;
   RETURN FALSE;
END Deal_Is_Discount_Only;
