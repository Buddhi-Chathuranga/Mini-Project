-----------------------------------------------------------------------------
--
--  Logical unit: RebatePeriodicAggLog
--  Component:    ORDER
--
...
--  140312  ShVese   Made the last_run_date a private attribute and removed the overloaded Get_Last_Run_Date method.
...
--  080328  JeLise   Changed prompt on the view from Rebate Period Agg Log to Rebate Periodic Agg Log.
--  080318  JeLise   Created
-----------------------------------------------------------------------------
...
PROCEDURE Insert___ (
   ...
   newrec_     IN OUT REBATE_PERIODIC_AGG_LOG_TAB%ROWTYPE,
...
PROCEDURE Modify (
   ...
   process_id_ IN     NUMBER )
IS
   newrec_     REBATE_PERIODIC_AGG_LOG_TAB%ROWTYPE;
   oldrec_     REBATE_PERIODIC_AGG_LOG_TAB%ROWTYPE;
   objid_      VARCHAR2(2000);
   objversion_ VARCHAR2(2000);
...
IS
   temp_ REBATE_PERIODIC_AGG_LOG_TAB.last_run_date%TYPE;
   CURSOR get_attr IS
