-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptStartPicking
--  Component:    ORDER
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: START_PICKING
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
...
--  171102  DaZase  STRSC-13075, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
...
BEGIN
   IF (wanted_data_item_id_ = 'PICK_LIST_NO_LEVEL')  THEN
   ...
   unique_value_ := Pick_Customer_Order_API.Get_Column_Value_If_Unique(contract_                   => contract_, 
                                                                       ...
                                                                       pick_list_no_level_db_      => pick_list_no_level_db_,
...
PROCEDURE Get_Filter_Keys___ (
   ...
   pick_list_no_level_db_         OUT VARCHAR2,
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      pick_list_no_level_db_ := Data_Capture_Session_API.Get_Predicted_Data_Item_Value(capture_session_id_, data_item_id_, data_item_value_, 'PICK_LIST_NO_LEVEL', session_rec_ , process_package_, use_applicable_);
      ...
      IF (use_unique_values_) THEN
         ...
         IF (pick_list_no_ IS NULL) THEN
            pick_list_no_ := Get_Unique_Data_Item_Value___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, 'PICK_LIST_NO');
         END IF;
         IF (location_no_ IS NULL) THEN
            location_no_ := Get_Unique_Data_Item_Value___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, 'LOCATION_NO');
         END IF;
         IF (pick_list_no_level_db_ IS NULL) THEN
            pick_list_no_level_db_ := Get_Unique_Data_Item_Value___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, 'PICK_LIST_NO_LEVEL');
         END IF;
      END IF;
   $ELSE
      NULL;
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      CASE (data_item_detail_id_)
         ...
         WHEN ('PICK_LIST_NO_LEVEL') THEN
            detail_value_ := pick_list_no_level_db_;
         ELSE
            NULL;
      END CASE;
...
IS
   ...
   pick_list_no_level_db_       VARCHAR2(20);
   sql_where_expression_        VARCHAR2(2000) DEFAULT NULL;
   column_name_                 VARCHAR2(30);
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      Get_Filter_Keys___(dummy_, pick_list_no_, location_no_, pick_list_no_level_db_, capture_session_id_, data_item_id_);
      IF (data_item_id_ IN ('LOCATION_NO', 'PICK_LIST_NO_LEVEL')) THEN
         IF (data_item_id_ = 'PICK_LIST_NO_LEVEL' AND pick_list_no_ IS NULL AND location_no_ IS NULL) THEN
            -- Pick List No Level is for some strange reason first in configuration then use the faster enumeration LOV.
            Part_Or_Handl_Unit_Level_API.Create_Data_Capture_Lov(capture_session_id_);
         ELSE
            IF (data_item_id_ = 'PICK_LIST_NO_LEVEL')  THEN
               column_name_ := 'PART_OR_HANDLING_UNIT';
            ELSE
            ...
            Pick_Customer_Order_API.Create_Data_Capture_Lov(contract_              => contract_, 
                                                            ...
                                                            sql_where_expression_  => sql_where_expression_);
         END IF;
      ELSIF (data_item_id_ = 'PICK_LIST_NO') THEN
         -- If PICK_LIST_NO is first data item in the configuration use faster PICK_LIST_NO-LOV method (using CUST_ORD_PICK_LIST_JOIN)
         IF (location_no_ IS NULL AND pick_list_no_level_db_ IS NULL) THEN
            Pick_Customer_Order_API.Create_Data_Capture_Lov(contract_           => contract_,
                                                            capture_session_id_ => capture_session_id_,
                                                            lov_type_db_        => lov_type_db_);
         ELSE
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN 
      ...
      IF (automatic_value_ IS NULL) THEN
         ...
         IF (data_item_id_ = 'PICK_LIST_NO') THEN
            ...
            ELSE
               Get_Filter_Keys___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, capture_session_id_, data_item_id_);
               automatic_value_ := Pick_Customer_Order_API.Get_Column_Value_If_Unique(contract_                   => contract_, 
                                                                                      pick_list_no_               => pick_list_no_, 
                                                                                      location_no_                => location_no_,
                                                                                      pick_list_no_level_db_      => pick_list_no_level_db_,
                                                                                      column_name_                => data_item_id_,
                                                                                      sql_where_expression_       => sql_where_expression_);
            END IF;
         ELSIF (data_item_id_ IN ('LOCATION_NO', 'PICK_LIST_NO_LEVEL')) THEN      
            Get_Filter_Keys___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, capture_session_id_, data_item_id_);
            IF (data_item_id_ = 'PICK_LIST_NO_LEVEL')  THEN
               column_name_ := 'PART_OR_HANDLING_UNIT';
            ELSE
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      Get_Filter_Keys___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, capture_session_id_, data_item_id_, data_item_value_);
      ...
      IF (data_item_id_ IN ('PICK_LIST_NO', 'LOCATION_NO', 'PICK_LIST_NO_LEVEL')) THEN      
         IF (data_item_id_ = 'PICK_LIST_NO_LEVEL')  THEN
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      session_rec_ := Data_Capture_Session_API.Get_Session_Rec(capture_session_id_);
      Get_Filter_Keys___(contract_, pick_list_no_, location_no_, pick_list_no_level_db_, capture_session_id_, 
      ...
      IF (conf_item_detail_tab_.COUNT > 0) THEN
         FOR i IN conf_item_detail_tab_.FIRST..conf_item_detail_tab_.LAST LOOP
            ...
            IF (conf_item_detail_tab_(i).item_type = Capture_Session_Item_Type_API.DB_DATA) THEN
               ...
               IF (conf_item_detail_tab_(i).data_item_detail_id IN ('PICK_LIST_NO','LOCATION_NO', 'PICK_LIST_NO_LEVEL')) THEN
...
BEGIN
   ...
   IF (Security_SYS.Is_Proj_Action_Available('WarehouseTaskHandling', 'PickSelected') OR
       ...
       Security_SYS.Is_Proj_Action_Available('MyWarehouseTaskHandling', 'UnreserveSelected') OR
       ...
       Security_SYS.Is_Proj_Action_Available('WarehouseTaskHandling', 'ReportPickAggregated') OR
