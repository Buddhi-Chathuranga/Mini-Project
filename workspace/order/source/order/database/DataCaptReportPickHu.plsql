-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptReportPickHu
--  Component:    ORDER
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: REPORT_PICKING_HU
--
--  Date    Sign    History
--  ------  ------  --------------------------------------------------------
--  211029  DaZase  Bug 161410, (SCZ-16770), Changes in Add_Details_For_Latest_Item to fix issues with PART_DESCRIPTION and CONDITION_CODE_DESCRIPTION.
--  211029  DaZase  Bug 161274 (SCZ-16463), This bug fix was not merged from Apps10 solution, instead here we did another solution where we instead of changing
--  211029          how aggregated_line_id_ worked we changed how the hu snapshot updates works in inventory (SC21R2-5917), so there is no changes in this process like Apps10.
--  201215  Dazase  Bug 157109 (SCZ-12924), Changes in Add_Details_For_Latest_Item to use new detail PICK_BY_CHOICE_DETAILS to control when unique filter keys should be fetched.
--  201209  DaZase  Bug 156990 (SCZ-12596), Changes in Validate_Data_Item for a validation against Pick_By_Choice_Blocked for item RES_HANDLING_UNIT_ID to increase performance.  
--  200827  DaZase  SC2020R1-7510, Replaced Is_Method_Available with Is_Proj_Action_Available/Is_Proj_Entity_Act_Available in Is_Process_Available to support new projection security concept.
--  191001  SURBLK  Added Raise_Shipment_Handling_Error___ to handle error messages and avoid code duplication.
--  190327  Budklk  Bug 149203 (SCZ-5817), Modified the method Post_Process_Action() to add finish_session in order to properly refresh the Hu_Snapshot_For_Refresh_API.
--  190327  ChBnlk  Bug 147260 (SCZ-3694), Modified Create_List_Of_Values() by reverting the correction done for bug 147105 since the original issue 
--  190327          is behaving accoring to the expected functionality. 
--  190228  ChBnlk  SCZ-3482 (Bug 147105), Modified Create_List_Of_Values() by passing the handling_unit_id to the LOV creation method when the data item is AGGREGATED_LINE_ID.
--  180227  DaZase  STRSC-16924, Changes in Add_Details_For_Latest_Item to support hu structure checks for LAST_LINE_ON_PICK_LIST.
--  180222  RuLiLk  STRSC-16860, Modified method Validate_Data_Item to validate if GS1 barcodes are mandatory.
--  180119  SWiclk  STRSC-15949, Modified Pack_To_Shipment_Handl_Unit___() by adding condition compilation to check WADACO is installed.
--  180118  MaAuse  STRSC-15862, Modified condition for call to Shipment_Flow_API.Start_Shipment_Flow in Post_Process_Action. Modified Execute_Process by
--  180118          adding call to Update_Shipment_Ids___, moved check of max weight and max volume to Pack_To_Shipment_Handl_Unit___. Modified Pick_Hu_By_Choice_And_Pack___
--  180118          and Pack_To_Shipment_Handl_Unit___ by adding parameter capture_session_id_. Replaced Add_Shipment_Id_To_Process_Message___ with the new
--  180118          methods Get_Shipment_Id_Message___, Update_Shipment_Ids___ and Clob_Attribute_Exist___.
...
--  171218          trigger_shipment_flow_ in call to Pick_Customer_Order_API.Pick_Reservations_HU__. Added new method Add_Shipment_To_Process_Message___.
--  171218          Modified Execute_Process and Post_Process_Action to be able to trigger shipment flow at the end of Execute_Process.
--  171113  JeLise  STRSC-13870, Added check on pick by choice blocked in Validate_Data_Item.
--  170918  DaZase  STRSC-11606, Added fetching of current data item from any previously scanned GS1 barcode in Get_Automatic_Data_Item_Value and made sure that value is used instead 
--  170918          of anything found later in that method. Changed size to 4000 on detail_value_ in Add_Unique_Data_Item_Detail___.
...
--  170523  KHVESE  STRSC-8590, Renamed method Validate_Ship_Location_No___ to Validate_Shipment_Data_Items___ and added more validation for shp_handling_unit_id_ and ship_location_no_.
--  170523          Modified Fixed_Value_Is_Applicable() to handle fixed value NULL FOR Shipment HANDLING_UNIT_ID, SSCC AND ALT_HANDLING_UNIT_ID WHEN fixed value is set to always or 
--  170523          when applicable and when reserved handling unit contains package component part or has qty attached to shipment.
--  170523          Modified method Validate_Data_Item() to validate shipment handling unit id, shipment sscc and shipment alt hu id when reserved handling unit contains component part 
--  170523          or has qty attached to shipment. Also extra check added to validate shipment handling unit location against shipment location that user is selected or scanned.
--  170518  Khvese  STRSC-8425, Modified method Get_Pick_By_Choice_Whr_Expr___.
--  170517  Khvese  STRSC-8255, Modified method Validate_Ship_Location_No___ to verify Shipment Location versus Shipment handling Unit Id's Location. 
--  170517          Added method Pack_To_Shipment_Handl_Unit___ to be called from Execute method and Pick_Hu_By_Choice_And_Pack___.
--  170517  Chfose  STRSC-8205, Modified Execute_Process to use new Pick_Customer_Order_API.Pick_Reservations_HU__ that takes a CLOB.
--  170329  Khvese  LIM-11284, Modified method Pick_Hu_By_Choice___ to handle pack to shipment and renamed it to Pick_Hu_By_Choice_And_Pack___, modified method Execute_Process
--  170329          to call Modify_Parent_Handling_Unit_Id (Pack to shipment) inside the condition when pick by choice is not allowed.
--  170329          Added methods Reset_Shp_Handl_Unit_Ids___, Get_Shp_Handl_Unit_Id_Tab___ and Get_Res_Handl_Unit_Id_Tab___ and modified Post_Process_Action.
--  170329  Khvese  LIM-11011, Modified method Add_Details_For_Latest_Item to handle Shipment handling unit realted feedback items.
--  170317  Khvese  LIM-11181, Rearenged Execute method and added some new methods to support process_message currectly 
--  170316  Khvese  LIM-11175, Added method Get_Inventory_Event_Id___() to construct message correctly 
...
--  170306  Khvese  LIM-11015, Added Possiblity to loop over handling unit when pick by choice is enabled.
--  170301  Khvese  LIM-5836, Added Pick by choice functionality.
...
--  170131  DaZase  LIM-2931, created.
-----------------------------------------------------------------------------
      ...
      IF use_unique_values_ THEN
         ...
         IF (pick_list_no_ IS NULL) THEN
            pick_list_no_ := Get_Unique_Data_Item_Value___(capture_session_id_, contract_, pick_list_no_, aggregated_line_id_, location_no_,
                                                           res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_, 'PICK_LIST_NO');
         END IF;
         IF (aggregated_line_id_ IS NULL) THEN
            aggregated_line_id_ := Get_Unique_Data_Item_Value___(capture_session_id_, contract_, pick_list_no_, aggregated_line_id_, location_no_,
                                                                 res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_, 'AGGREGATED_LINE_ID');
         END IF;
         IF (location_no_ IS NULL) THEN
            location_no_ := Get_Unique_Data_Item_Value___(capture_session_id_, contract_, pick_list_no_, aggregated_line_id_, location_no_,
                                                          res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_, 'LOCATION_NO');
         END IF;
         IF (res_handling_unit_id_ IS NULL) THEN
            res_handling_unit_id_ := Get_Unique_Data_Item_Value___(capture_session_id_, contract_, pick_list_no_, aggregated_line_id_, location_no_,
                                                                   res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_, 'RES_HANDLING_UNIT_ID');
         END IF;
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (detail_value_ IS NULL AND data_item_detail_id_ IN ('SHIP_LOCATION_NO') AND res_handling_unit_id_ IS NOT NULL) THEN
         shipment_id_ := Reserve_Customer_Order_API.Get_Shipment_Id(pick_list_no_, res_handling_unit_id_);
         order_no_  := Reserve_Customer_Order_API.Get_Order_No(pick_list_no_, res_handling_unit_id_);
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (pick_list_no_ IS NOT NULL AND res_handling_unit_id_ IS NOT NULL) THEN
         shipment_id_ := Reserve_Customer_Order_API.Get_Shipment_Id(pick_list_no_, res_handling_unit_id_); -- find unique shipment for this hu and its childs
...
BEGIN 
   ...
   IF (ship_location_no_ IS NOT NULL AND shp_handling_unit_id_ IS NOT NULL) THEN 
      ...
      END IF;
   END IF;
   IF Pick_Customer_Order_API.Hu_Has_Package_Component_Part(pick_list_no_, res_handling_unit_id_) = Fnd_Boolean_API.DB_TRUE AND shp_handling_unit_id_ IS NOT NULL THEN 
      Error_SYS.Record_General(lu_name_, 'PACKINGNOTALLOWED: Shipment Handling Unit Id must be empty since Handling Unit :P1 contains package component parts.', res_handling_unit_id_);
   END IF;
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      IF (data_item_id_ = 'ACTION') THEN
         ...
         IF (data_item_value_ = Pick_Handl_Unit_Action_API.DB_PICK_SUB_LEVEL) THEN
            Get_Filter_Keys___(contract_, pick_list_no_, aggregated_line_id_, location_no_, res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_,
                               shp_handling_unit_id_, shp_sscc_, shp_alt_handl_unit_label_id_, capture_session_id_, data_item_id_, data_item_value_);
            ...
            IF (res_handling_unit_id_ IS NOT NULL AND NOT Handling_Unit_API.Has_Children(res_handling_unit_id_) AND 
                pick_list_no_ IS NOT NULL AND Pick_Customer_Order_API.Handl_Unit_Exist_On_Pick_List(pick_list_no_,res_handling_unit_id_)) THEN
      ...
      ELSE
         ...
         IF (data_item_id_ IN ('PICK_LIST_NO', 'AGGREGATED_LINE_ID', 'LOCATION_NO')) THEN
         ...
         ELSIF (data_item_id_ IN ('SHP_HANDLING_UNIT_ID', 'SHP_SSCC', 'SHP_ALT_HANDLING_UNIT_LABEL_ID')) THEN
            IF (data_item_value_ IS NOT NULL) THEN
               ...
               END IF;
               IF (pick_list_no_ IS NOT NULL AND aggr_res_handling_unit_id_ IS NOT NULL AND 
                   Pick_Customer_Order_API.Hu_Has_Package_Component_Part(pick_list_no_, aggr_res_handling_unit_id_) = Fnd_Boolean_API.DB_TRUE) THEN 
                  Error_SYS.Record_General(lu_name_, 'PACKINGPKGNOTALLOWED: :P1 must be empty since Handling Unit :P2 contains package component part.', 
         ...
         ELSIF (data_item_id_ IN ('RES_HANDLING_UNIT_ID', 'RES_SSCC', 'RES_ALT_HANDLING_UNIT_LABEL_ID')) THEN
            ...
            ELSE
               IF (data_item_id_ = 'RES_HANDLING_UNIT_ID' AND data_item_value_ != aggr_res_handling_unit_id_) THEN
                  IF (Customer_Order_Reservation_API.Get_Pick_By_Choice_Blocked_Db(aggr_res_handling_unit_id_) = Fnd_Boolean_API.DB_TRUE) THEN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      ELSE
         Get_Filter_Keys___(dummy_, pick_list_no_, aggregated_line_id_, location_no_, res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_,
                            shp_handling_unit_id_, shp_sscc_, shp_alt_handl_unit_label_id_, capture_session_id_, data_item_id_);
         ...
         ELSIF (data_item_id_ = 'PICK_LIST_NO' AND aggregated_line_id_ IS NULL AND location_no_ IS NULL AND res_handling_unit_id_ IS NULL AND shp_handling_unit_id_ IS NULL) THEN   
            ...
            pick_list_sql_where_expr_ := ' AND EXISTS (SELECT 1 from Report_Pick_Handling_Unit
                                                       WHERE contract = coplj.contract
                                                       AND pick_list_no = coplj.pick_list_no
                                                       AND handling_unit_id IS NOT NULL  
                                                       AND process_control IS NULL -- filter out pick_parts and pick_sub_level lines
                                                       AND Pick_Customer_Order_API.Is_Fully_Picked(pick_list_no, handling_unit_id, location_no) = ''FALSE'') ';
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN 
      ...
      IF (automatic_value_ IS NULL) THEN
         ...
         IF (data_item_id_ = 'PICK_LIST_NO') THEN
            ...
            ELSE
               automatic_value_ := Get_Unique_Data_Item_Value___(capture_session_id_, contract_, pick_list_no_, aggregated_line_id_, location_no_,
                                                                 res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_, data_item_id_);
            END IF;
         ELSIF (data_item_id_ IN ('AGGREGATED_LINE_ID', 'LOCATION_NO', 'RES_HANDLING_UNIT_ID', 'RES_SSCC', 'RES_ALT_HANDLING_UNIT_LABEL_ID')) THEN
         ...
         ELSIF data_item_id_ = 'SHIP_LOCATION_NO' THEN
            ...
            shipment_id_ := Reserve_Customer_Order_API.Get_Shipment_Id(pick_list_no_, aggr_res_handling_unit_id_);
            order_no_  := Reserve_Customer_Order_API.Get_Order_No(pick_list_no_, aggr_res_handling_unit_id_);
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      Get_Filter_Keys___(contract_, pick_list_no_, aggregated_line_id_, location_no_, res_handling_unit_id_, res_sscc_, res_alt_handl_unit_label_id_,
                         shp_handling_unit_id_, shp_sscc_, shp_alt_handl_unit_label_id_, capture_session_id_, 
         ...
         FOR i IN conf_item_detail_tab_.FIRST..conf_item_detail_tab_.LAST LOOP
            ...
            ELSE  -- FEEDBACK ITEMS AS DETAILS
               ...
               ELSIF (conf_item_detail_tab_(i).data_item_detail_id = 'LAST_LINE_ON_PICK_LIST') THEN 
                  ...
                  IF (Pick_Customer_Order_API.Lines_Left_To_Pick(pick_list_no_) = 1 AND 
                  ...
                  ELSIF (action_ IN (Pick_Handl_Unit_Action_API.DB_PICK,
                     ...
                     IF ((Handling_Unit_API.Get_Root_Handling_Unit_Id(res_handling_unit_id_) = res_handling_unit_id_)  AND 
                        ...
                        IF (Pick_Customer_Order_API.Last_Hndl_Unit_Structure_On_PL(pick_list_no_, res_handling_unit_id_)) THEN
                        ...
                        END IF;
                     ELSIF (Pick_Customer_Order_API.Lines_Left_To_Pick(pick_list_no_) = 1 ) THEN
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      shipment_id_ := Reserve_Customer_Order_API.Get_Shipment_Id(pick_list_no_, reserve_rec_.handling_unit_id);
      ...
      IF (action_ = Pick_Handl_Unit_Action_API.DB_PICK) THEN -- no need to do checks if action is not PICK
         -- Doing some extra checks on ship_location_no
         Validate_Ship_Data_Items___(contract_, pick_list_no_, reserve_rec_.handling_unit_id, shp_handling_unit_id_, shipment_id_, ship_location_no_);
      END IF;

      IF (action_ = Pick_Handl_Unit_Action_API.DB_PICK OR 
          action_ = Pick_Handl_Unit_Action_API.DB_UNRESERVE_PICK_LIST_LINES) THEN
         ...
         ELSE 
            ...
            out_clob_ := Pick_Customer_Order_API.Pick_Reservations_HU__(message_               => pick_clob_,
...
BEGIN
   -- Check to see that API method Pick_Customer_Order_API.Pick_Reservations_HU__ is granted thru following projections/actions/entity actions
   IF (Security_SYS.Is_Proj_Action_Available('WarehouseTaskHandling', 'ReportPickAggregated') OR
       Security_SYS.Is_Proj_Entity_Act_Available('WarehouseTaskHandling', 'OutermostHandlingUnit', 'PickHandlingUnitReservation') OR
       Security_SYS.Is_Proj_Action_Available('ReportPickingOfPickListLines', 'ReportPickAggregated') OR
       Security_SYS.Is_Proj_Entity_Act_Available('ReportPickingOfPickListLines', 'OutermostHandlingUnit', 'PickHandlingUnitReservation') OR
       Security_SYS.Is_Proj_Action_Available('MyWarehouseTaskHandling', 'ReportPickAggregated') OR
       Security_SYS.Is_Proj_Entity_Act_Available('MyWarehouseTaskHandling', 'OutermostHandlingUnit', 'PickHandlingUnitReservation')) THEN
...
BEGIN
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ IN ('SHP_HANDLING_UNIT_ID', 'SHP_SSCC', 'SHP_ALT_HANDLING_UNIT_LABEL_ID', 'SHIP_LOCATION_NO')) THEN            
         ...
         unique_shipment_id_ := Reserve_Customer_Order_API.Get_Shipment_Id(pick_list_no_, aggr_res_handling_unit_id_); -- Get unique shipment_id for this hu structure
         order_no_  := Reserve_Customer_Order_API.Get_Order_No(pick_list_no_, aggr_res_handling_unit_id_);
         ...
         ELSIF (data_item_id_ IN ('SHP_HANDLING_UNIT_ID', 'SHP_SSCC', 'SHP_ALT_HANDLING_UNIT_LABEL_ID') AND 
               -- When handling unit contains component part packing is not allowed even if HU contains whole package.
                Pick_Customer_Order_API.Hu_Has_Package_Component_Part(pick_list_no_, aggr_res_handling_unit_id_) = Fnd_Boolean_API.DB_TRUE) THEN 
