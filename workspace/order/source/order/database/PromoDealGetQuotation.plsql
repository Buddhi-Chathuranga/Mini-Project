-----------------------------------------------------------------------------
--
--  Logical unit: PromoDealGetQuotation
--  Component:    ORDER
--
...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  101215  NaLrlk  Added procedure Delete_Promo_Get_For_Quote, New, Modify
--  101215          And Check_Exist_Any_Quotation.
...
-- Delete_Promo_Get_For_Quote
--   Remove all promotion get deals for specified quotation.
PROCEDURE Delete_Promo_Get_For_Quote (
   quotation_no_ IN VARCHAR2 )
IS
   objid_       PROMO_DEAL_GET_QUOTATION.objid%TYPE;
   objversion_  PROMO_DEAL_GET_QUOTATION.objversion%TYPE;
   remrec_      PROMO_DEAL_GET_QUOTATION_TAB%ROWTYPE;

   CURSOR get_rec IS
      SELECT campaign_id, deal_id, get_id
        FROM PROMO_DEAL_GET_QUOTATION_TAB
       WHERE quotation_no = quotation_no_;
BEGIN
   FOR rec_ IN get_rec LOOP
      Get_Id_Version_By_Keys___(objid_, objversion_, rec_.campaign_id, rec_.deal_id, rec_.get_id, quotation_no_);
      remrec_ := Lock_By_Id___(objid_, objversion_);
      Check_Delete___(remrec_);
      Delete___(objid_, remrec_);
   END LOOP;
END Delete_Promo_Get_For_Quote;


-- New
--   Public interface for creating a new promotion deal get quotation.
PROCEDURE New (
...
IS
   ...
   objid_       PROMO_DEAL_GET_QUOTATION.objid%TYPE;
   objversion_  PROMO_DEAL_GET_QUOTATION.objversion%TYPE;
   newrec_      PROMO_DEAL_GET_QUOTATION_TAB%ROWTYPE;
