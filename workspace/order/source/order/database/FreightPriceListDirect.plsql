...
--  111116  ChJalk   Modified the view FREIGHT_PRICE_LIST_DIRECT to use User_Finance_Auth_Pub instead of Company_Finance_Auth_Pub.
--  111025  ChJalk   Modified the view FREIGHT_PRICE_LIST_DIRECT to use the user allowed company filter.
--  110914  NaLrlk   Modified the method Unpack_Check_Insert___ to validate the supplier and freight.
--  110323  NiBalk   EANE-4853, Removed user allowed site filter from where clause of view FREIGHT_PRICE_LIST_DIRECT.
--  110203  Nekolk   EANE-3744  added where clause to View FREIGHT_PRICE_LIST_DIRECT.
--  100426  JeLise   Renamed zone_definition_id to freight_map_id.
--  091009  UtSwlk   Modified Copy_Freight_List__ to copy data in zone info tab.
...
--  080826  RoJalk   Modified Insert___ to call Freight_Price_List_Base_API.Post_Insert_Actions__.
...
BEGIN
   ...
   END IF;
   IF NOT Freight_Zone_Valid_Supp_API.Check_Exist(newrec_.freight_map_id, newrec_.supplier_id) THEN
      Error_SYS.Record_General(lu_name_, 'NOTVALIDSUPP: :P1 is not a valid supplier for a freight map ID :P2.', newrec_.supplier_id, newrec_.freight_map_id);
   END IF;
END Check_Insert___;
...
-- Copy_Freight_List__
--   Copies an Existing Direct Delivery Freight Price List to a new one.
...
BEGIN
   ...
   newrec_.supplier_id := copyrec_.supplier_id;
...
IS
BEGIN
   RETURN Freight_Price_List_Base_API.Get_Active_Freight_List_No(contract_, ship_via_code_, zone_def_id_, forwarder_id_, use_price_incl_tax_, vendor_no_);
