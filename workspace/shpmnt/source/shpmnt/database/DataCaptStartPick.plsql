-----------------------------------------------------------------------------
--
--  Logical unit: DataCaptStartPick
--  Component:    SHPMNT
--
--  IFS Developer Studio Template Version 3.0
--
--  Supported process: START_PICK
--
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
...
BEGIN
   $IF Component_Wadaco_SYS.INSTALLED $THEN
      ...
      IF (data_item_id_ IN ('LOCATION_NO', 'PICK_LIST_NO_LEVEL', 'PICK_LIST_NO')) THEN
         IF (data_item_id_ = 'PICK_LIST_NO_LEVEL' AND pick_list_no_ IS NULL AND location_no_ IS NULL) THEN
            -- when pick_list_no_level is the first data item
            Part_Or_Handl_Unit_Level_API.Create_Data_Capture_Lov(capture_session_id_);
         ELSIF (data_item_id_ = 'PICK_LIST_NO' AND location_no_ IS NULL AND pick_list_no_level_db_ IS NULL) THEN
            -- when pick_list_no is the first data item
   ...
   $IF Component_Wadaco_SYS.INSTALLED $THEN 
      ...
      IF (automatic_value_ IS NULL) THEN
         IF (data_item_id_ IN ('PICK_LIST_NO', 'LOCATION_NO', 'PICK_LIST_NO_LEVEL')) THEN
            -- TODO fetch warehouse_task_id if avaliable
            ...
            ELSE
               column_name_ := data_item_id_;
            END IF;
            automatic_value_ := Pick_Shipment_API.Get_Column_Value_If_Unique(contract_                   => contract_, 
                                                                 pick_list_no_               => pick_list_no_, 
                                                                 location_no_                => location_no_,
                                                                 pick_list_no_level_db_      => pick_list_no_level_db_,
                                                                 column_name_                => column_name_,
...
BEGIN
   IF (Security_SYS.Is_Proj_Action_Available('ReportPickingOfPickListLines', 'PickSelected') OR
       ...
       Security_SYS.Is_Proj_Entity_Act_Available('ReportPickingOfPickListLines', 'OutermostHandlingUnit', 'PickHandlingUnitReservation')) THEN
